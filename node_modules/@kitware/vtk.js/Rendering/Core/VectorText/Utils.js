...
//#region Sources/Rendering/Core/VectorText/Utils.js
/**
...
function computeSidesUV(vertices, iA, iB, iC, iD) {
	...
	if (Math.abs(ay - by) < Math.abs(ax - bx)) return [
		[ax, 1 - az],
		[bx, 1 - bz],
		[cx, 1 - cz],
		[dx, 1 - dz]
		...
		lineTo(x, y) {
			...
			curves.push({
				...
				getPoints(resolution) {
					const points = [];
					for (let i = 0; i <= resolution; i++) points.push(this.getPointAt(i / resolution));
					return points;
				}
			});
			currentPoint[0] = x;
		...
		*/
		getPoints(divisions) {
			...
			for (let i = 0; i < curves.length; i++) {
				...
				if (curve.curveType === "EllipseCurve") resolution = divisions * 2;
				else if (curve.curveType === "LineCurve") resolution = 1;
		...
		*/
		extractPoints(divisions) {
			return {
				...
				holes: this.holes.map((hole) => hole.getPoints(divisions))
			};
		},
		/**
...
function getBoundingSize(shapes, depth, curveSegments) {
	...
	for (let i = 0; i < shapes.length; i++) {
		const shapePoints = shapes[i].extractPoints(curveSegments);
...
function isClockWise(points) {
	...
	for (let p = n - 1, q = 0; q < n; p = q++) sum += points[p][0] * points[q][1] - points[q][0] * points[p][1];
	return sum * .5 < 0;
}
/**
...
function computeBevelVector(pt, prev, next) {
	...
	} else if (vPrevX > 0 && vNextX > 0 || vPrevX < 0 && vNextX < 0 || Math.sign(vPrevY) === Math.sign(vNextY)) {
		tx = -vPrevY;
		ty = vPrevX;
...
function addTriangle(layers, a, b, c, verticesArray, uvArray, colorArray, color) {
	...
	computeFacesUV(verticesArray, nextIndex - 3, nextIndex - 2, nextIndex - 1).forEach((uv) => {
		uvArray.push(uv[0], uv[1]);
	});
	if (colorArray && color) for (let i = 0; i < 3; ++i) colorArray.push(color[0] * 255, color[1] * 255, color[2] * 255);
}
/**
...
function addQuad(layers, a, b, c, d, verticesArray, uvArray, colorArray, color) {
	[
		...
		d,
		c
	].forEach((i) => verticesArray.push(layers[i * 3], layers[i * 3 + 1], layers[i * 3 + 2]));
	const nextIndex = verticesArray.length / 3;
...
function buildWalls(layers, contour, layerOffset, vlen, steps, bevelSegments, verticesArray, uvArray, colorArray, color) {
	...
	for (let i = 0; i < contour.length; i++) {
		...
		for (let s = 0; s < totalLayers; s++) {
			...
			addQuad(layers, layerOffset + j + slen1, layerOffset + k + slen1, layerOffset + k + slen2, layerOffset + j + slen2, verticesArray, uvArray, colorArray, color);
		}
	}
}
/**
