import { cross, multiplyAccumulate, multiplyScalar, normalize, signedAngleBetweenVectors, subtract } from "../../../Common/Core/Math.js";
import Plane_default from "../../../Common/DataModel/Plane.js";
import BoundingBox_default, { STATIC } from "../../../Common/DataModel/BoundingBox.js";
...
import { planeNameToViewType, planeNames, viewTypeToPlaneName } from "./Constants.js";
//#region Sources/Widgets/Widgets3D/ResliceCursorWidget/helpers.js
var EPSILON = 1e-6;
/**
...
function boundPlane(bounds, origin, p1, p2) {
	...
	BoundingBox_default.addBounds(inflatedBounds, bounds[0] + eps[0], bounds[1] + eps[0], bounds[2] + eps[1], bounds[3] + eps[1], bounds[4] + eps[2], bounds[5] + eps[2]);
	BoundingBox_default.addBounds(inflatedBounds, bounds[0] - eps[0], bounds[1] - eps[0], bounds[2] - eps[1], bounds[3] - eps[1], bounds[4] - eps[2], bounds[5] - eps[2]);
	const plane = Plane_default.newInstance();
	plane.setOrigin(...origin);
	plane.setNormal(...n);
	const cubeSource = CubeSource_default.newInstance();
	cubeSource.setBounds(inflatedBounds);
	const cutter = Cutter_default.newInstance();
	cutter.setCutFunction(plane);
	cutter.setInputConnection(cubeSource.getOutputPort());
	const cutBounds = cutter.getOutputData();
	if (cutBounds.getNumberOfPoints() === 0) return false;
...
function boundPoint(inPoint, v1, v2, bounds) {
	...
	for (let i = 0; i < 3; i++) {
		...
		if (inPoint[i] < bounds[i * 2]) axisOffset = absT[i] > EPSILON ? (bounds[2 * i] - inPoint[i]) / t[i] : 0;
		else if (inPoint[i] > bounds[2 * i + 1]) axisOffset = absT[i] > EPSILON ? (bounds[2 * i + 1] - inPoint[i]) / t[i] : 0;
		if (useT1) {
			if (Math.abs(axisOffset) > Math.abs(o1)) o1 = axisOffset;
		} else if (Math.abs(axisOffset) > Math.abs(o2)) o2 = axisOffset;
	}
	const outPoint = [
		inPoint[0],
		inPoint[1],
	...
	if (o1 !== 0) multiplyAccumulate(outPoint, v1, o1, outPoint);
	if (o2 !== 0) multiplyAccumulate(outPoint, v2, o2, outPoint);
	return outPoint;
}
function boundPointOnPlane(p1, p2, bounds) {
	...
	BoundingBox_default.intersectBox(bounds, p1, dir12, out, [
		0,
		0,
		0
	]);
...
function rotateVector(vectorToBeRotated, axis, angle) {
	...
	MatrixBuilder_default.buildFromRadian().rotate(angle, axis).apply(rotatedVector);
...
function getPlanesLineNames(planes = planeNames) {
	...
	planes.forEach((plane) => {
		planes.forEach((inPlane) => {
			if (plane !== inPlane) lines.push(`${plane}in${inPlane}`);
		});
	});
	return lines;
}
function getLineNames(widgetState) {
	return getPlanesLineNames(Object.keys(widgetState.getPlanes()).map((viewType) => viewTypeToPlaneName[viewType]));
}
/**
...
function getOtherLineName(widgetState, lineName) {
	...
	return `${getPlaneNames(widgetState).find((planeName) => planeName !== linePlaneName && planeName !== lineInPlaneName)}in${lineInPlaneName}`;
}
...
function updateState(widgetState, scaleInPixels, rotationHandlePosition) {
	...
	const pdLength = BoundingBox_default.getDiagonalLength(bounds);
...
function transformPlane(planeToTransform, targetCenter, targetNormal, targetViewUp) {
	...
	const angle = signedAngleBetweenVectors(subtract(planeToTransform.getPoint2(), planeToTransform.getOrigin(), []), targetViewUp, targetNormal);
