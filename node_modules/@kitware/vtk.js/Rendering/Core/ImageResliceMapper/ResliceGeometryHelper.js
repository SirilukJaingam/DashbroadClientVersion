...
//#region Sources/Rendering/Core/ImageResliceMapper/ResliceGeometryHelper.js
var tmpAxisAlignedN = [
	0,
	0,
	0
...
function isVectorAxisAligned(n) {
	Math_default.normalize(n);
	for (let i = 0; i < 3; ++i) {
		vec3.zero(tmpAxisAlignedN);
		tmpAxisAlignedN[i] = 1;
		const dotP = Math_default.dot(n, tmpAxisAlignedN);
		if (dotP < -.999999 || dotP > .999999) return [true, i];
	}
	return [false, 2];
}
...
function computeResliceGeometryState(renderable, firstImageData) {
	let resGeomString = "";
	...
	if (slicePD) resGeomString = `PolyData${slicePD.getMTime()}`;
	else if (slicePlane) {
		...
		slicePlane = Plane_default.newInstance();
		...
		if (firstImageData) bds = imageBounds;
		slicePlane.setOrigin(bds[0], bds[2], .5 * (bds[4] + bds[5]));
		renderable.setSlicePlane(slicePlane);
		resGeomString = `Plane${slicePlane.getMTime()}Image${firstImageData?.getMTime?.() ?? 0}`;
	}
	return {
		resGeomString,
		slicePD,
...
function computeObliqueSliceGeometryData(firstImageData, slicePlane, outlineFilter, cutter, lineToSurfaceFilter) {
...
function computeOrthoSliceGeometryData(firstImageData, slicePlane, orthoAxis, transform) {
	const points = new Float32Array(12);
	const indexSpacePlaneOrigin = firstImageData.worldToIndex(slicePlane.getOrigin(), [
		0,
		0,
		0
	]);
...
//#endregion
export { computeObliqueSliceGeometryData, computeOrthoSliceGeometryData, computeResliceGeometryState, fillPackedNormals, isVectorAxisAligned };

//# sourceMappingURL=ResliceGeometryHelper.js.map