...
import caseTable_default from "./ImageMarchingCubes/caseTable.js";
//#region Sources/Filters/General/ImageMarchingCubes/index.js
...
function vtkImageMarchingCubes(publicAPI, model) {
	...
	publicAPI.getVoxelScalars = (i, j, k, slice, dims, origin, spacing, s) => {
		...
		ids[6] = ids[4] + dims[0];
		ids[7] = ids[6] + 1;
		for (let ii = 0; ii < 8; ++ii) voxelScalars[ii] = s[ids[ii]];
	};
	...
	publicAPI.produceTriangles = (cVal, i, j, k, extent, slice, dims, origin, spacing, scalars, points, tris, normals) => {
		...
		let index = 0;
		for (let idx = 0; idx < 8; idx++) if (voxelScalars[VERT_MAP[idx]] >= cVal) index |= CASE_MASK[idx];
		const voxelTris = caseTable_default.getCase(index);
		if (voxelTris[0] < 0) return;
		publicAPI.getVoxelPoints(i + extent[0], j + extent[2], k + extent[4], origin, spacing);
		if (model.computeNormals) publicAPI.getVoxelGradients(i, j, k, dims, slice, spacing, scalars);
		for (let idx = 0; voxelTris[idx] >= 0; idx += 3) {
			tris.push(3);
			for (let eid = 0; eid < 3; eid++) {
				const edgeVerts = caseTable_default.getEdge(voxelTris[idx + eid]);
				pId = void 0;
	...
	publicAPI.requestData = (inData, outData) => {
		...
		for (let k = 0; k < dims[2] - 1; ++k) for (let j = 0; j < dims[1] - 1; ++j) for (let i = 0; i < dims[0] - 1; ++i) publicAPI.produceTriangles(model.contourValue, i, j, k, extent, slice, dims, origin, spacing, s, pBuffer, tBuffer, nBuffer);
...
var newInstance = macros_default.newInstance(extend, "vtkImageMarchingCubes");
var ImageMarchingCubes_default = {
	newInstance,
	extend
};
//#endregion
export { ImageMarchingCubes_default as default, extend, newInstance };

//# sourceMappingURL=ImageMarchingCubes.js.map