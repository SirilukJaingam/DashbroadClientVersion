...
import StorageBuffer_default from "./StorageBuffer.js";
import { addClipPlaneEntries, getClippingPlaneEquationsInCoords } from "./Helpers/ClippingPlanes.js";
import { mat4 } from "gl-matrix";
//#region Sources/Rendering/WebGPU/VolumePassFSQ/index.js
var volFragTemplate = `
//VTK::Renderer::Dec
...
function vtkWebGPUVolumePassFSQ(publicAPI, model) {
	...
	publicAPI.replaceShaderPosition = (hash, pipeline, vertexInput) => {
		...
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", ["output.tcoordVS = vec2<f32>(vertexBC.x * 0.5 + 0.5, 1.0 - vertexBC.y * 0.5 - 0.5);", "output.Position = vec4<f32>(vertexBC, 1.0);"]).result;
		vDesc.setCode(code);
	...
	publicAPI.replaceShaderVolume = (hash, pipeline, vertexInput) => {
		...
		const clipInit = [];
		const traverseCalls = [];
		for (let i = 0; i < model.volumes.length; i++) if (model.volumes[i].getRenderable().getMapper().getBlendMode() === BlendMode.COMPOSITE_BLEND) {
			clipInit.push(`  var tpos${i}: vec4<f32> = volumeSSBO.values[${i}].SCTCMatrix*minPosSC;`);
			clipInit.push(`  var tpos2_${i}: vec4<f32> = volumeSSBO.values[${i}].SCTCMatrix*(minPosSC + rayStepSC);`);
			clipInit.push(`  var tstep${i}: vec4<f32> = tpos2_${i} - tpos${i};`);
			clipInit.push(`  var rayBounds${i}: vec2<f32> = intersectRayBoundsWithClipPlanes(${i}, minPosSC, rayStepSC, adjustBounds(tpos${i}, tstep${i}, numSteps));`);
			compositeCalls.push(`    if (curDist >= rayBounds${i}.x * mapperUBO.SampleDistance && curDist <= rayBounds${i}.y * mapperUBO.SampleDistance) {`);
		...
		code = ShaderCache_default.substitute(code, "//VTK::Volume::CompositeCalls", compositeCalls).result;
		code = ShaderCache_default.substitute(code, "//VTK::Volume::TraverseCalls", traverseCalls).result;
		code = ShaderCache_default.substitute(code, "//VTK::Volume::TraverseInit", clipInit).result;
		code = ShaderCache_default.substitute(code, "//VTK::Volume::TraverseDec", [`var<private> traverseVals: array<vec4<f32>,${model.volumes.length}>;`]).result;
		...
		for (let vidx = 0; vidx < model.volumes.length; vidx++) {
			...
			if (blendMode === BlendMode.COMPOSITE_BLEND) compositeWhileTraversing = true;
			else if (blendMode === BlendMode.MAXIMUM_INTENSITY_BLEND) code = ShaderCache_default.substitute(code, "//VTK::Volume::Loop", [
				`    traverseMax(volTexture${vidx}, ${vidx}, ${vidx}, rayLengthSC, minPosSC, rayStepSC);`,
				`    computedColor = traverseVals[${vidx}];`,
				"//VTK::Volume::Loop"
			]).result;
			else if (blendMode === BlendMode.MINIMUM_INTENSITY_BLEND) code = ShaderCache_default.substitute(code, "//VTK::Volume::Loop", [
				`    traverseMin(volTexture${vidx}, ${vidx}, ${vidx}, rayLengthSC, minPosSC, rayStepSC);`,
				`    computedColor = traverseVals[${vidx}];`,
				"//VTK::Volume::Loop"
			]).result;
			else if (blendMode === BlendMode.AVERAGE_INTENSITY_BLEND) code = ShaderCache_default.substitute(code, "//VTK::Volume::Loop", [
				`    traverseAverage(volTexture${vidx}, ${vidx}, ${vidx}, rayLengthSC, minPosSC, rayStepSC);`,
				`    computedColor = traverseVals[${vidx}];`,
				"//VTK::Volume::Loop"
			]).result;
			else if (blendMode === BlendMode.ADDITIVE_INTENSITY_BLEND) code = ShaderCache_default.substitute(code, "//VTK::Volume::Loop", [
				`    traverseAdditive(volTexture${vidx}, ${vidx}, ${vidx}, rayLengthSC, minPosSC, rayStepSC);`,
				`    computedColor = traverseVals[${vidx}];`,
				"//VTK::Volume::Loop"
			]).result;
		}
		if (compositeWhileTraversing) code = ShaderCache_default.substitute(code, "//VTK::Volume::Loop", ["    computedColor = composite(rayLengthSC, minPosSC, rayStepSC);"]).result;
		fDesc.setCode(code);
	};
	...
	publicAPI.updateLUTImage = (device) => {
		...
		if (mtime < model.lutBuildTime.getMTime()) return;
		model.numRows = 0;
		model.rowStarts = [];
		for (let vidx = 0; vidx < model.volumes.length; vidx++) {
			...
			const numComp = (image.getPointData() && image.getPointData().getScalars()).getNumberOfComponents();
		...
		for (let vidx = 0; vidx < model.volumes.length; vidx++) {
			...
			for (let c = 0; c < numIComps; ++c) {
				...
				for (let i = 0; i < rowLength; ++i) {
					...
					for (let co = 0; co < 4; co++) colorArray[ioffset + (rowLength + i) * 4 + co] = colorArray[ioffset + i * 4 + co];
				}
		...
		}
		{
			...
			const tview = device.getTextureManager().getTexture(treq).createView("tfunTexture");
		...
		}
		{
			...
			const tview = device.getTextureManager().getTexture(treq).createView("ofunTexture");
	...
	publicAPI.updateSSBO = (device) => {
		...
		for (let i = 0; i < model.volumes.length; i++) {
			...
			const image = volMapr.getInputData();
			mtime = Math.max(mtime, vol.getMTime(), image.getMTime(), volMapr.getMTime(), volMapr.getClippingPlanesMTime());
		}
		if (mtime < model.SSBO.getSendTime()) return;
		...
		const clipPlaneArrays = Array.from({ length: 6 }, () => new Float64Array(model.volumes.length * 4));
		...
		for (let vidx = 0; vidx < model.volumes.length; vidx++) {
			...
			mat4.multiply(tmpMat4, tmp2Mat4, tmpMat4);
			for (let j = 0; j < 16; j++) marray[vidx * 16 + j] = tmpMat4[j];
			...
			const clipPlaneCount = getClippingPlaneEquationsInCoords(volMapr, tmp2Mat4, model.clipPlanes);
		...
		for (let i = 0; i < 6; i++) model.SSBO.setAllInstancesFromArray(`clipPlane${i}`, clipPlaneArrays[i]);
	...
	publicAPI.updateBuffers = () => {
		...
		for (let i = 0; i < model.volumes.length; i++) {
			const sd = model.volumes[i].getRenderable().getMapper().getSampleDistance();
			if (sd < sampleDist) sampleDist = sd;
		}
		...
		for (let vidx = 0; vidx < model.volumes.length; vidx++) {
			const image = model.volumes[vidx].getRenderable().getMapper().getInputData();
		...
		if (model.volumes.length < model.lastVolumeLength) for (let i = model.volumes.length; i < model.lastVolumeLength; i++) model.textureViews.pop();
		...
		if (!model.clampSampler) {
			model.clampSampler = Sampler_default.newInstance({ label: "clampSampler" });
	...
	publicAPI.setVolumes = (val) => {
		...
		for (let i = 0; i < val.length; i++) if (val[i] !== model.volumes[i]) {
			model.volumes = [...val];
			publicAPI.modified();
			return;
		}
...
function extend(publicAPI, model, initialValues = {}) {
	...
	model.SSBO = StorageBuffer_default.newInstance({ label: "volumeSSBO" });
	model.componentSSBO = StorageBuffer_default.newInstance({ label: "componentSSBO" });
	model.lutBuildTime = {};
	macros_default.obj(model.lutBuildTime, { mtime: 0 });
...
var newInstance = macros_default.newInstance(extend, "vtkWebGPUVolumePassFSQ");
var VolumePassFSQ_default = {
	newInstance,
	extend
};
//#endregion
export { VolumePassFSQ_default as default, extend, newInstance };

//# sourceMappingURL=VolumePassFSQ.js.map