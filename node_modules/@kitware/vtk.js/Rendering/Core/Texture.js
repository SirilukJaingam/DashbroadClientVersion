...
//#region Sources/Rendering/Core/Texture/index.js
function vtkTexture(publicAPI, model) {
	...
	publicAPI.setJsImageData = (imageData) => {
		if (model.jsImageData === imageData) return;
		if (imageData !== null) {
	...
	publicAPI.setImageBitmap = (imageBitmap) => {
		if (model.imageBitmap === imageBitmap) return;
		if (imageBitmap !== null) {
			publicAPI.setInputData(null);
			publicAPI.setInputConnection(null);
			model.image = null;
	...
	publicAPI.setImage = (image) => {
		...
		if (image.complete) publicAPI.imageLoaded();
		else image.addEventListener("load", publicAPI.imageLoaded);
		publicAPI.modified();
	};
	...
	publicAPI.getInputAsJsImageData = () => {
		...
		if (model.jsImageData) return model.jsImageData;
		if (model.imageBitmap) return model.imageBitmap;
		if (model.canvas) return model.canvas.getContext("2d").getImageData(0, 0, model.canvas.width, model.canvas.height);
		if (model.image) {
...
var generateMipmaps = (device, texture, mipLevelCount) => {
  ...
  ` });
	...
	for (let mipLevel = 1; mipLevel < mipLevelCount; mipLevel++) {
		...
		const commandEncoder = device.createCommandEncoder({ label: `MipmapGenerateCommandEncoder` });
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.algo(publicAPI, model, 6, 0);
...
var newInstance = macros_default.newInstance(extend, "vtkTexture");
var STATIC = { generateMipmaps };
var Texture_default = {
	newInstance,
	extend,
	...STATIC
...
//#endregion
export { STATIC, Texture_default as default, extend, newInstance };
