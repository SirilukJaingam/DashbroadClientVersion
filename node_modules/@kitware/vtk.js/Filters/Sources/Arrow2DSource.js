...
import Constants_default from "./Arrow2DSource/Constants.js";
//#region Sources/Filters/Sources/Arrow2DSource/index.js
var { ShapeType } = Constants_default;
function vtkStarSource(publicAPI, model, dataset) {
	const points = macros_default.newTypedArray(model.pointType, 30);
	const edges = new Uint32Array(11);
	edges[0] = 10;
	for (let i = 0; i < 10; i++) {
		const radius = i % 2 === 1 ? model.height : model.height * .4;
...
function vtk6PointsArrow(publicAPI, model, dataset) {
	const points = macros_default.newTypedArray(model.pointType, 18);
	const thickOp = model.height * .5 * model.thickness;
	const offsetOp = model.height * .5 - thickOp;
	const baseOffsetOp = (model.height * .9 + thickOp - offsetOp - (model.height * .5 - thickOp - offsetOp)) * (1 - model.base);
	points[0] = model.width / 2 * -1 - thickOp;
	points[1] = model.height / 4 - offsetOp - baseOffsetOp;
	points[2] = 0;
	points[3] = 0;
	points[4] = model.height * .9 + thickOp - offsetOp - baseOffsetOp;
	points[5] = 0;
	points[6] = model.width / 2 + thickOp;
	points[7] = model.height / 4 - offsetOp - baseOffsetOp;
	points[8] = 0;
	points[9] = model.width / 3;
	points[10] = model.height * .1 - thickOp - offsetOp - baseOffsetOp;
	points[11] = 0;
	points[12] = 0;
	points[13] = model.height * .5 - thickOp - offsetOp - baseOffsetOp;
	points[14] = 0;
	points[15] = model.width / 3 * -1;
	points[16] = model.height * .1 - thickOp - offsetOp - baseOffsetOp;
	points[17] = 0;
...
function vtk4PointsArrow(publicAPI, model, dataset) {
	const points = macros_default.newTypedArray(model.pointType, 12);
...
function vtkTriangleSource(publicAPI, model, dataset) {
	const points = macros_default.newTypedArray(model.pointType, 9);
...
function vtkArrow2DSource(publicAPI, model) {
	...
	publicAPI.requestData = (inData, outData) => {
		...
		MatrixBuilder_default.buildFromRadian().translate(...model.center).rotateFromDirections([
			...
			0,
			0
		], model.direction).apply(outData[0].getPoints().getData());
	};
}
/**
...
var newInstance = macros_default.newInstance(extend, "vtkArrow2DSource");
var Arrow2DSource_default = {
	newInstance,
	extend
};
//#endregion
export { Arrow2DSource_default as default, extend, newInstance };

//# sourceMappingURL=Arrow2DSource.js.map