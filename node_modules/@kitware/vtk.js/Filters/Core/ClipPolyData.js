...
import MergePoints_default from "../../Common/DataModel/MergePoints.js";
//#region Sources/Filters/Core/ClipPolyData/index.js
...
function vtkClipPolyData(publicAPI, model) {
	...
	*/
	function buildOutputs(outData) {
		...
		const clippedOutput = outData[1]?.initialize() || PolyData_default.newInstance();
		outData[0] = output;
	...
	*/
	function createClipState(input, output, clippedOutput) {
		...
		if (model.outputPointsPrecision === DesiredOutputPrecision.SINGLE) pointsDataType = VtkDataTypes.FLOAT;
		else if (model.outputPointsPrecision === DesiredOutputPrecision.DOUBLE) pointsDataType = VtkDataTypes.DOUBLE;
		const newPoints = Points_default.newInstance({ dataType: pointsDataType });
		...
		return {
			...
			newPoints,
			clipScalarValues: [],
			triangleCell: Triangle_default.newInstance(),
			triangleScalarsValues,
			triangleScalars: DataArray_default.newInstance({
	...
	*/
	function addPoint(pointRef, clipState) {
		const { inserted, id } = model.locator.insertUniquePoint(pointRef.point);
		if (inserted) {
			if (pointRef.type === "original") clipState.outputPointData.passData(clipState.inputPointData, pointRef.pointId, id);
			else clipState.outputPointData.interpolateData(clipState.inputPointData, pointRef.pointId1, pointRef.pointId2, id, pointRef.t);
			if (model.generateClipScalars) clipState.clipScalarValues[id] = pointRef.scalar;
		}
		return id;
	}
	/**
	...
	*/
	function clipPolyline(values, offset, npts, insideOut, clipState) {
		...
		for (let i = 0; i < npts - 1; i++) {
			...
			if (in0 && current.length === 0) current.push(addPoint(ref0, clipState));
			if (in0 && in1) {
				const id = addPoint(ref1, clipState);
				if (current[current.length - 1] !== id) current.push(id);
			} else if (in0 !== in1) {
				const edgeId = addPoint(makeEdgeRef(ref0, ref1), clipState);
				if (current.length === 0 || current[current.length - 1] !== edgeId) current.push(edgeId);
				if (in0) pushCurrent();
				else current.push(addPoint(ref1, clipState));
			} else pushCurrent();
		}
		pushCurrent();
		return segments;
	...
	*/
	function clipPolygon(values, offset, npts, insideOut, clipState) {
		...
		for (let i = 0; i < count; i++) {
			...
			else if (currentInside && !nextInside) result.push(makeEdgeRef(current, next));
		...
		const cleanedIds = [];
		for (let i = 0; i < polygonPointIds.length; i++) if (cleanedIds.length === 0 || cleanedIds[cleanedIds.length - 1] !== polygonPointIds[i]) cleanedIds.push(polygonPointIds[i]);
		if (cleanedIds.length > 1 && cleanedIds[0] === cleanedIds[cleanedIds.length - 1]) cleanedIds.pop();
		return cleanedIds;
	}
	/**
	...
	*/
	function processLines(cellArray, lines, insideOut, outputCellData, clipState, cellOffset) {
		iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
			clipPolyline(values, offset, npts, insideOut, clipState).forEach((segment) => {
	...
	*/
	function processPolys(cellArray, polys, insideOut, outputCellData, clipState, cellOffset) {
		...
		iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
			...
			addTriangulatedPolygon(clipPolygon(values, offset, npts, insideOut, clipState), polys, cellOffset + localCellId, outputCellData, clipState);
		});
	}
	/**
	...
	*/
	function processStrips(cellArray, polys, insideOut, outputCellData, clipState, cellOffset) {
		...
		iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
			for (let i = 0; i < npts - 2; i++) if (i % 2 === 0) clipTriangle(values[offset + i], values[offset + i + 1], values[offset + i + 2], cellOffset + localCellId);
			else clipTriangle(values[offset + i + 1], values[offset + i], values[offset + i + 2], cellOffset + localCellId);
		});
	}
	...
	publicAPI.getMTime = () => {
		...
		if (model.clipFunction) mTime = Math.max(mTime, model.clipFunction.getMTime());
		if (model.locator) mTime = Math.max(mTime, model.locator.getMTime());
		return mTime;
	};
	publicAPI.createDefaultLocator = () => {
		if (!model.locator) model.locator = MergePoints_default.newInstance();
	};
	...
	publicAPI.requestData = (inData, outData) => {
		...
		const mainVerts = CellArray_default.newInstance();
		const mainLines = CellArray_default.newInstance();
		const mainPolys = CellArray_default.newInstance();
		const clippedVerts = CellArray_default.newInstance();
		const clippedLines = CellArray_default.newInstance();
		const clippedPolys = CellArray_default.newInstance();
		let cellOffset = 0;
		processVerts(input.getVerts(), mainVerts, model.insideOut, output.getCellData(), clipState, cellOffset);
		if (model.generateClippedOutput) processVerts(input.getVerts(), clippedVerts, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
		...
		if (model.generateClippedOutput) processLines(input.getLines(), clippedLines, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
		...
		if (model.generateClippedOutput) processPolys(input.getPolys(), clippedPolys, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
		...
		if (model.generateClippedOutput) processStrips(input.getStrips(), clippedPolys, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
		output.setPoints(clipState.newPoints);
		if (mainVerts.getNumberOfCells() > 0) output.setVerts(mainVerts);
		if (mainLines.getNumberOfCells() > 0) output.setLines(mainLines);
		if (mainPolys.getNumberOfCells() > 0) output.setPolys(mainPolys);
		if (model.generateClipScalars) {
			const clipScalarValues = new Float32Array(clipState.newPoints.getNumberOfPoints());
			for (let i = 0; i < clipScalarValues.length; i++) clipScalarValues[i] = Number.isFinite(clipState.clipScalarValues[i]) ? clipState.clipScalarValues[i] : 0;
			output.getPointData().setScalars(DataArray_default.newInstance({
				name: "ClipScalars",
		...
		if (model.generateClippedOutput) {
			...
			clippedOutput.getFieldData().passData(input.getFieldData());
			if (clippedVerts.getNumberOfCells() > 0) clippedOutput.setVerts(clippedVerts);
			if (clippedLines.getNumberOfCells() > 0) clippedOutput.setLines(clippedLines);
			if (clippedPolys.getNumberOfCells() > 0) clippedOutput.setPolys(clippedPolys);
		}
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.algo(publicAPI, model, 1, 2);
...
var newInstance = macros_default.newInstance(extend, "vtkClipPolyData");
var ClipPolyData_default = {
	newInstance,
	extend
};
//#endregion
export { ClipPolyData_default as default, extend, newInstance };

//# sourceMappingURL=ClipPolyData.js.map