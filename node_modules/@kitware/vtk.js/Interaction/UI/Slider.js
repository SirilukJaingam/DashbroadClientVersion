...
import Constants_default from "./Slider/Constants.js";
import Slider_module_default from "./Slider/Slider.module.css.js";
//#region Sources/Interaction/UI/Slider/index.js
...
function vtkSlider(publicAPI, model) {
	...
	model.el.setAttribute("class", Slider_module_default.cursor);
	...
	function updateCursorPosition() {
		...
		if (Number.isNaN(position) || Number.isNaN(cursorSize)) return;
		model.el.style.width = `${cursorSize}px`;
		model.el.style.height = `${cursorSize}px`;
		if (model.orientation === Constants_default.SliderOrientation.VERTICAL) {
	...
	function onMouseMove(e) {
		e.preventDefault();
		if (isDragging) {
			const newValue = findClosestValue((ratio + ((model.orientation ? e.clientX : e.clientY) - offset) / (model.containerSizes[1] - model.containerSizes[0])) * model.range + model.values[0], model.values);
			if (newValue !== void 0) publicAPI.setValue(newValue);
		}
	}
	...
	function onMouseUp(e) {
		...
		if (!isDragging) {
			if (!((model.orientation ? e.clientX : e.clientY) - offset)) {
				const newValue = findClosestValue(model.values[0] + model.range * (offset - model.container.getBoundingClientRect()[model.orientation ? "left" : "top"] - .5 * model.containerSizes[0]) / (model.containerSizes[1] - model.containerSizes[0]), model.values);
				if (newValue !== void 0) publicAPI.setValue(newValue);
			}
		}
		isDragging = false;
	...
	publicAPI.resize = () => {
		if (model.container) {
			...
			publicAPI.setOrientation(height === max ? Constants_default.SliderOrientation.VERTICAL : Constants_default.SliderOrientation.HORIZONTAL);
	...
	publicAPI.generateValues = (min, max, nbSteps) => {
		...
		model.values = [];
		for (let i = 0; i < nbSteps; i++) model.values.push(min + i * step);
		model.range = max - min;
		updateCursorPosition();
		publicAPI.modified();
	};
	publicAPI.updateCursorStyle = (cursorStyle) => {
		...
		while (count--) model.el.style[keys[count]] = model.cursorStyle[keys[count]];
	};
	publicAPI.updateCursorStyle();
}
var DEFAULT_VALUES = {
	orientation: Constants_default.SliderOrientation.VERTICAL,
	value: .5,
	values: [
		0,
		.1,
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.set(publicAPI, model, ["orientation"]);
	macros_default.event(publicAPI, model, "ValueChange");
	vtkSlider(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkSlider");
var Slider_default = {
	newInstance,
	extend,
	...Constants_default
...
//#endregion
export { Slider_default as default, extend, newInstance };
