...
import { arrayRange } from "../../Common/Core/Math.js";
import WorkerWrapper from "./PiecewiseGaussianWidget/ComputeHistogram.worker.js";
import WebworkerPromise from "webworker-promise";
//#region Sources/Interaction/Widgets/PiecewiseGaussianWidget/index.js
var MIN_GAUSSIAN_WIDTH = .001;
...
var ACTIONS = {
	adjustPosition(x, y, { originalXY, gaussian, originalGaussian }) {
		gaussian.position = x + (originalGaussian.position - originalXY[0]);
		return true;
	},
	...
	adjustWidth(x, y, { originalXY, gaussian, originalGaussian, gaussianSide }) {
		...
		if (gaussian.width < MIN_GAUSSIAN_WIDTH) gaussian.width = MIN_GAUSSIAN_WIDTH;
		return true;
	},
	adjustZoom(x, y, { rangeZoom, publicAPI }) {
		...
		if (Math.abs(absNormX - .5 * (rangeZoom[0] + rangeZoom[1])) < Math.min(minDelta, maxDelta)) {
			const halfDelta = delta * .5;
			rangeZoom[0] = Math.min(Math.max(absNormX - halfDelta, 0), rangeZoom[1] - .1);
			rangeZoom[1] = Math.max(Math.min(absNormX + halfDelta, 1), rangeZoom[0] + .1);
		} else if (minDelta < maxDelta) rangeZoom[0] = Math.min(Math.max(absNormX, 0), rangeZoom[1] - .1);
		else rangeZoom[1] = Math.max(Math.min(absNormX, 1), rangeZoom[0] + .1);
...
function computeOpacities(gaussians, sampling = 256) {
	const opacities = [];
	while (opacities.length < sampling) opacities.push(0);
	...
	while (count--) {
		...
		for (let i = 0; i < sampling; i++) {
			...
			let x0 = 0;
			if (xBias === 0 || x === position + xBias) x0 = x;
			else if (x > position + xBias) if (correctedWidth === xBias) x0 = position;
			else x0 = position + (x - position - xBias) * (correctedWidth / (correctedWidth - xBias));
			else if (-correctedWidth === xBias) x0 = position;
			else x0 = position - (x - position - xBias) * (correctedWidth / (correctedWidth + xBias));
			...
			const h0c = 1;
			let h1;
			if (yBias < 1) h1 = yBias * h0b + (1 - yBias) * h0a;
			else h1 = (2 - yBias) * h0b + (yBias - 1) * h0c;
			const h2 = height * h1;
...
function drawChart(ctx, area, values, style = {
	...
	for (let index = 0; index < values.length; index++) ctx.lineTo(area[0] + index * horizontalScale, Math.max(area[1], offset - values[index] * verticalScale));
	if (fill) {
...
function updateColorCanvas(colorTransferFunction, width, rangeToUse, canvas) {
	...
	for (let lineIdx = 0; lineIdx < 256; lineIdx++) pixelsArea.data.set(rgba, lineIdx * 4 * width);
	const nbValues = 256 * width * 4;
	const lineSize = width * 4;
	for (let i = 3; i < nbValues; i += 4) pixelsArea.data[i] = 255 - Math.floor(i / lineSize);
...
function updateColorCanvasFromImage(img, width, canvas) {
	...
	workCanvas.setAttribute("width", width);
	workCanvas.setAttribute("height", 256);
	workCanvas.getContext("2d").drawImage(img, 0, 0, img.width, img.height, 0, 0, width, 256);
	return workCanvas;
}
...
function processTouchClicks() {
	TOUCH_CLICK.filter((t) => t.ready).forEach((touchHandle) => {
		touchHandle.callbacks.forEach((callback) => {
			if (callback.touches === touchHandle.touches && callback.clicks === touchHandle.count) callback.action(...touchHandle.singleTouche);
		});
		touchHandle.ts = 0;
		touchHandle.count = 0;
...
function createTouchListener(id, callback, nbTouches = 1, preventDefault = true) {
	return (e) => {
		...
		const singleTouche = Array.prototype.map.call(e.touches, (t) => [t.pageX - targetBounds.left, t.pageY - targetBounds.top]).reduce((a, b) => [a[0] + b[0], a[1] + b[1]], [0, 0]).map((v) => v / e.touches.length);
		...
		} else if (e.type === "touchend") if (e.timeStamp - TOUCH_CLICK[id].ts < TOUCH_CLICK[id].deltaT) {
		...
		} else TOUCH_CLICK[id].ready = false;
...
function vtkPiecewiseGaussianWidget(publicAPI, model) {
	...
	if (!model.canvas) model.canvas = document.createElement("canvas");
	publicAPI.setContainer = (el) => {
		if (model.container && model.container !== el) model.container.removeChild(model.canvas);
		if (model.container !== el) {
			model.container = el;
			if (model.container) model.container.appendChild(model.canvas);
			publicAPI.modified();
		}
	};
	publicAPI.setGaussians = (gaussians) => {
		if (model.gaussians === gaussians) return;
	...
	publicAPI.setDataArray = (array, { numberOfBinToConsiders = 1, numberOfBinsToSkip = 1, numberOfComponents = 1, component = 0 } = {}) => {
		...
		let arrayStride = Math.floor(array.length / 4) || 1;
		...
		Promise.all(workerChunks).then((subHistograms) => {
			...
			subHistograms.forEach((subHistogram) => {
				for (let i = 0, len = subHistogram.length; i < len; ++i) model.histogram[i] += subHistogram[i];
			});
			...
			for (let i = 0; i < numberOfBinsToSkip; i++) sortedArray.pop();
			while (sortedArray.length > sampleSize) sortedArray.shift();
			const mean = sortedArray.reduce((a, b) => a + b, 0) / sampleSize;
			for (let i = 0, len = model.histogram.length; i < len; ++i) model.histogram[i] /= mean;
			publicAPI.modified();
	...
	publicAPI.onClick = (x, y) => {
		...
		if (xNormalized < 0 && model.style.iconSize > 1) {
			...
			switch (buttonIdx) {
				case 0: {
					const gaussianIdx = publicAPI.addGaussian(0, 1, .1, 0, 0);
				...
				case 1:
					if (model.selectedGaussian !== -1) publicAPI.removeGaussian(model.selectedGaussian);
					break;
				default:
	...
	publicAPI.onHover = (x, y) => {
		...
		if (model.enableRangeZoom && xNormalizedAbs >= 0 && y < model.graphArea[1] - 6) {
			...
			if (xNormalizedAbs < model.rangeZoom[0] + thirdDelta || xNormalizedAbs > model.rangeZoom[1] - thirdDelta) model.canvas.style.cursor = ACTION_TO_CURSOR.adjustZoom;
			else model.canvas.style.cursor = ACTION_TO_CURSOR.adjustPosition;
			model.dragAction = {
			...
			let actionName = null;
			if (invY > gaussian.height + tolerance) actionName = "adjustPosition";
			else if (invY > gaussian.height - tolerance) if (Math.abs(xNormalized - gaussian.position) < tolerance) actionName = "adjustHeight";
			else actionName = "adjustPosition";
			else if (invY > gaussian.height * .5 + tolerance) actionName = "adjustPosition";
			else if (invY > gaussian.height * .5 - tolerance) if (Math.abs(xNormalized - gaussian.position) < tolerance) actionName = "adjustBias";
			else actionName = "adjustPosition";
			else if (invY > tolerance) actionName = "adjustPosition";
			else actionName = "adjustWidth";
	...
	publicAPI.onDown = (x, y) => {
		if (!model.mouseIsDown) publicAPI.invokeAnimation(true);
		...
		if (gaussian) model.gaussianSide = gaussian.position - xNormalized;
	...
	publicAPI.onUp = (x, y) => {
		if (model.mouseIsDown) publicAPI.invokeAnimation(false);
	...
	publicAPI.onAddGaussian = (x, y) => {
		const [xNormalized, yNormalized] = normalizeCoordinates(x, y, model.graphArea, model.enableRangeZoom ? model.rangeZoom : null);
		if (xNormalized >= 0) publicAPI.addGaussian(xNormalized, 1 - yNormalized, .1, 0, 0);
		return true;
	};
	publicAPI.onRemoveGaussian = (x, y) => {
		const xNormalized = normalizeCoordinates(x, y, model.graphArea, model.enableRangeZoom ? model.rangeZoom : null)[0];
		const newSelected = findGaussian(xNormalized, model.gaussians);
		if (xNormalized >= 0 && newSelected !== -1) publicAPI.removeGaussian(newSelected);
		return true;
	};
	publicAPI.bindMouseListeners = () => {
		if (!model.listeners) {
			...
			model.listeners = {
				...
				touchstart: createTouchListener(touchId, macros_default.chain(publicAPI.onHover, publicAPI.onDown)),
	...
	publicAPI.render = () => {
		...
		if (model.histogram) drawChart(ctx, graphArea, rescaleArray(model.histogram, model.rangeZoom), {
			lineWidth: 1,
		...
		if (model.colorTransferFunction && model.colorTransferFunction.getSize()) {
			...
			if (model.enableRangeZoom) ctx.drawImage(model.colorCanvas, model.rangeZoom[0] * graphArea[2], 0, graphArea[2], graphArea[3], graphArea[0], graphArea[1], graphArea[2] / (model.rangeZoom[1] - model.rangeZoom[0]), graphArea[3]);
			else ctx.drawImage(model.colorCanvas, graphArea[0], graphArea[1]);
			ctx.restore();
		...
		if (activeGaussian) {
			drawChart(ctx, graphArea, rescaleArray(computeOpacities([activeGaussian], graphArea[2]), model.enableRangeZoom && model.rangeZoom), {
			...
			const yMiddle = graphArea[1] + (1 - .5 * activeGaussian.height) * graphArea[3];
			const yBottom = graphArea[1] + graphArea[3];
			let widthInPixel = activeGaussian.width * graphArea[2];
			if (model.enableRangeZoom) widthInPixel /= model.rangeZoom[1] - model.rangeZoom[0];
			...
			ctx.fill();
			ctx.stroke();
			const radius = Math.min(widthInPixel * .1, activeGaussian.height * graphArea[3] * .2);
			ctx.beginPath();
			ctx.rect(xCenter - radius, yMiddle - radius, radius * 2, radius * 2);
			ctx.fill();
			ctx.stroke();
			ctx.beginPath();
			const sliderWidth = widthInPixel * .8;
	...
	publicAPI.getOpacityRange = (dataRange) => {
		...
		let maxIndex = 0;
		for (let index = 0; index < model.opacities.length; index++) {
			if (model.opacities[index] > 0) minIndex = Math.min(minIndex, index);
			if (model.opacities[index] > 0) maxIndex = Math.max(maxIndex, index);
		}
	...
	publicAPI.setRangeZoom = (...v) => {
		...
		if (change) model.colorCanvasMTime = 0;
		return change;
	};
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.setGetArray(publicAPI, model, ["rangeZoom"], 2);
	macros_default.get(publicAPI, model, [
		"size",
		"canvas",
		"gaussians"
	]);
	macros_default.event(publicAPI, model, "opacityChange");
	macros_default.event(publicAPI, model, "animation");
	macros_default.event(publicAPI, model, "zoomChange");
	vtkPiecewiseGaussianWidget(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkPiecewiseGaussianWidget");
var PiecewiseGaussianWidget_default = {
	newInstance,
	extend,
	...STATIC
};
//#endregion
export { STATIC, PiecewiseGaussianWidget_default as default, extend, newInstance };

//# sourceMappingURL=PiecewiseGaussianWidget.js.map