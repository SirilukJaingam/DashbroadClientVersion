...
//#region Sources/Rendering/OpenGL/CutterMapper/index.js
...
function vtkOpenGLCutterMapper(publicAPI, model) {
	...
	publicAPI.replaceShaderClip = (shaders, ren, actor) => {
		...
		if (model.renderable.getNumberOfClippingPlanes()) {
			...
			FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::Clip::Dec", [
				"uniform int numClipPlanes;",
				`varying float clipDistancesVSOutput[${numClipPlanes}];`,
				"//VTK::Clip::Dec"
			]).result;
		...
		switch (functionName) {
			case "vtkPlane":
				...
				cutDistanceImpl = ["cutDistanceVSOutput = dot(cutPlaneNormal, cutFunctionPoint - cutPlaneOrigin) - cutValue;"];
				break;
			case "vtkSphere":
				cutDistanceDec = [
				...
				break;
			case "vtkBox":
				cutDistanceDec = ["uniform vec3 cutBoxMinPoint;", "uniform vec3 cutBoxMaxPoint;"];
				cutDistanceImpl = [
			...
			case "vtkCone":
				...
				cutDistanceImpl = ["cutDistanceVSOutput = cutFunctionPoint.y * cutFunctionPoint.y + cutFunctionPoint.z * cutFunctionPoint.z - cutFunctionPoint.x * cutFunctionPoint.x * cutConeTanThetaSquared;"];
				break;
			default:
				vtkErrorMacro(`No cut function set.`);
				break;
		...
		VSSource = ShaderProgram_default.substitute(VSSource, "//VTK::Clip::Impl", ["vec3 cutFunctionPoint = (cutFunctionMatrix * vertexMC).xyz;", ...cutDistanceImpl]).result;
		FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::Clip::Dec", ["varying float cutDistanceVSOutput;", "uniform float cutWidth;"]).result;
		FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::Clip::Impl", ["if (abs(cutDistanceVSOutput) > max(fwidth(cutDistanceVSOutput) * cutWidth, 1e-6)) discard;"]).result;
		shaders.Vertex = VSSource;
		shaders.Fragment = FSSource;
	};
	publicAPI.replaceShaderValues = (shaders, ren, actor) => {
		superClass.replaceShaderValues(shaders, ren, actor);
		if (model.renderable.getSupportedImplicitFunctionName()) shaders.Fragment = ShaderProgram_default.substitute(shaders.Fragment, "//VTK::UniformFlow::Impl", [
	...
	publicAPI.setMapperShaderParameters = (cellBO, ren, actor) => {
		...
		const functionName = model.renderable.getSupportedImplicitFunctionName();
		if (!cutFunction || !functionName) return;
		const shiftScaleEnabled = cellBO.getCABO().getCoordShiftAndScaleEnabled();
		const inverseShiftScaleMatrix = shiftScaleEnabled ? cellBO.getCABO().getInverseShiftAndScaleMatrix() : null;
		if (shiftScaleEnabled && !inverseShiftScaleMatrix) vtkErrorMacro("Failed to retrieve inverse shift and scale matrix for cut function");
		const cutFunctionTransform = cutFunction.getTransform?.();
		const transformMatrix = cutFunctionTransform?.getMatrix ? Array.from(cutFunctionTransform.getMatrix()) : IDENTITY;
		const cutFunctionMatrix = inverseShiftScaleMatrix ? mat4.multiply(model.tmpMat4, transformMatrix, inverseShiftScaleMatrix) : transformMatrix;
		const program = cellBO.getProgram();
		if (program.isUniformUsed("cutFunctionMatrix")) program.setUniformMatrix("cutFunctionMatrix", cutFunctionMatrix);
		switch (functionName) {
			case "vtkPlane":
				if (program.isUniformUsed("cutPlaneOrigin")) program.setUniform3fArray("cutPlaneOrigin", cutFunction.getOrigin());
				if (program.isUniformUsed("cutPlaneNormal")) program.setUniform3fArray("cutPlaneNormal", cutFunction.getNormal());
				if (program.isUniformUsed("cutValue")) program.setUniformf("cutValue", model.renderable.getCutValue());
				break;
			case "vtkSphere": {
				const radius = cutFunction.getRadius();
				if (program.isUniformUsed("cutSphereCenter")) program.setUniform3fArray("cutSphereCenter", cutFunction.getCenter());
				...
				if (program.isUniformUsed("cutSphereUsesAxisRadii")) program.setUniformi("cutSphereUsesAxisRadii", Array.isArray(radius) ? 1 : 0);
				break;
			}
			case "vtkBox": {
			...
			case "vtkCylinder":
				if (program.isUniformUsed("cutCylinderCenter")) program.setUniform3fArray("cutCylinderCenter", cutFunction.getCenter());
				if (program.isUniformUsed("cutCylinderAxis")) program.setUniform3fArray("cutCylinderAxis", cutFunction.getAxis());
				if (program.isUniformUsed("cutCylinderRadius")) program.setUniformf("cutCylinderRadius", cutFunction.getRadius());
				break;
			case "vtkCone":
				if (program.isUniformUsed("cutConeTanThetaSquared")) {
					const tanTheta = Math.tan(Math_default.radiansFromDegrees(cutFunction.getAngle()));
		...
		if (program.isUniformUsed("cutWidth")) program.setUniformf("cutWidth", model.renderable.getCutWidth());
	};
}
var DEFAULT_VALUES = {
	tmpVec3A: null,
...
var newInstance = macros_default.newInstance(extend, "vtkOpenGLCutterMapper");
var CutterMapper_default = {
	newInstance,
	extend
};
registerOverride("vtkCutterMapper", newInstance);
//#endregion
export { CutterMapper_default as default, extend, newInstance };

//# sourceMappingURL=CutterMapper.js.map