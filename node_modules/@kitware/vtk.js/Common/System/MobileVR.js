import dpdb_default from "./MobileVR/dpdb.json.js";
import headsets_default from "./MobileVR/headsets.js";
//#region Sources/Common/System/MobileVR/index.js
...
function extractDeviceParameters() {
	for (let i = 0; i < dpdb_default.devices.length; i++) {
		const device = dpdb_default.devices[i];
		if (!device.rules) {
...
function getVRHeadset() {
	return new Promise((resolve, reject) => {
		...
		selector.innerHTML = headsets_default.map((headset, idx) => `<option value="${idx}">${headset.label}</option>`).join("");
		...
		selector.addEventListener("change", (e) => {
			...
			resolve(headsets_default[Number(e.target.value)]);
		});
		body.appendChild(selector);
	});
}
var MobileVR_default = {
	isMobile,
	isIOS,
	isWebViewAndroid,
	isSafari,
...
//#endregion
export { MobileVR_default as default };

//# sourceMappingURL=MobileVR.js.map