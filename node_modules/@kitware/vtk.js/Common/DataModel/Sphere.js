...
import { distance2BetweenPoints } from "../Core/Math.js";
import ImplicitFunction_default from "./ImplicitFunction.js";
//#region Sources/Common/DataModel/Sphere/index.js
function evaluate(radius, center, xyz) {
	if (!Array.isArray(radius)) return (xyz[0] - center[0]) * (xyz[0] - center[0]) + (xyz[1] - center[1]) * (xyz[1] - center[1]) + (xyz[2] - center[2]) * (xyz[2] - center[2]) - radius * radius;
	const r = [
...
function computeBoundingSphere(pts, numPts, hints) {
	...
	if (actualNumPts < 1) return sphere;
	const d1 = [
		0,
		0,
		0
...
function computeBoundingSphereFromSpheres(spheres, numSpheres, hints) {
	...
	if (actualNumSpheres < 1) return sphere;
...
function vtkSphere(publicAPI, model) {
	...
	publicAPI.evaluateGradient = (xyz) => {
		return [
			2 - (xyz[0] - model.center[0]),
			2 - (xyz[1] - model.center[1]),
			2 - (xyz[2] - model.center[2])
		];
	};
}
var DEFAULT_VALUES = {
...
var newInstance = macros_default.newInstance(extend, "vtkSphere");
var Sphere_default = {
	newInstance,
	extend,
	...STATIC
...
//#endregion
export { STATIC, Sphere_default as default, extend, newInstance };
