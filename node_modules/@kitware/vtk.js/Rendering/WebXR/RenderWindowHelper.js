...
import LineSource_default from "../../Filters/Sources/LineSource.js";
...
import Constants_default from "./RenderWindowHelper/Constants.js";
import { vec3 } from "gl-matrix";
//#region Sources/Rendering/WebXR/RenderWindowHelper/index.js
var { XrSessionTypes } = Constants_default;
var DEFAULT_RESET_FACTORS = {
	rescaleFactor: .25,
	translateZ: -1.5
};
function createRay() {
	const targetRayLineSource = LineSource_default.newInstance();
	const targetRayMapper = Mapper_default.newInstance();
	targetRayMapper.setInputConnection(targetRayLineSource.getOutputPort());
	const targetRayActor = Actor_default.newInstance();
...
function vtkWebXRRenderWindowHelper(publicAPI, model) {
	...
	publicAPI.startXR = (xrSessionType) => {
		if (navigator.xr === void 0) throw new Error("WebXR is not available");
		model.xrSessionType = xrSessionType !== void 0 ? xrSessionType : XrSessionTypes.HmdVR;
		...
		if (!navigator.xr.isSessionSupported(sessionType)) if (isXrSessionAR) throw new Error("Device does not support AR session");
		else throw new Error("VR display is not available");
		if (model.xrSession === null) navigator.xr.requestSession(sessionType).then(publicAPI.enterXR, () => {
			throw new Error("Failed to create XR session!");
		});
		else throw new Error("XR Session already exists!");
	};
	publicAPI.enterXR = async (xrSession) => {
		...
		if (model.xrSession !== null) {
			...
			const glLayer = new XRWebGLLayer(model.xrSession, gl[GET_UNDERLYING_CONTEXT]());
			...
			if ([XrSessionTypes.HmdAR, XrSessionTypes.MobileAR].includes(model.xrSessionType)) {
			...
			publicAPI.modified();
		} else throw new Error("Failed to enter XR with a null xrSession.");
	};
	publicAPI.resetXRScene = (rescaleFactor = DEFAULT_RESET_FACTORS.rescaleFactor, translateZ = DEFAULT_RESET_FACTORS.translateZ) => {
		...
		camera.setClippingRange(.1 * physicalScale, 100 * physicalScale);
	};
	publicAPI.stopXR = async () => {
		if (navigator.xr === void 0) return;
		if (model.xrSession !== null) {
	...
	model.xrRender = async (t, frame) => {
		...
		if (isXrSessionHMD && model.drawControllersRay && model.xrReferenceSpace) {
			...
			xrSession.inputSources.forEach((inputSource) => {
				if (inputSource.targetRaySpace == null || inputSource.gripSpace == null || inputSource.targetRayMode !== "tracked-pointer") return;
		...
		if (xrPose) {
			...
			xrPose.views.forEach((view, index) => {
				...
				if (isXrSessionHMD) if (view.eye === "left") ren.setViewport(0, 0, .5, 1);
				else if (view.eye === "right") ren.setViewport(.5, 0, 1, 1);
				else return;
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.get(publicAPI, model, ["xrSession"]);
	macros_default.setGet(publicAPI, model, ["renderWindow", "drawControllersRay"]);
	vtkWebXRRenderWindowHelper(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkWebXRRenderWindowHelper");
var RenderWindowHelper_default = {
	newInstance,
	extend,
	...Constants_default
};
//#endregion
export { RenderWindowHelper_default as default, extend, newInstance };

//# sourceMappingURL=RenderWindowHelper.js.map