...
//#region Sources/Proxy/Core/ProxyManager/core.js
...
function addRegistrationAPI(publicAPI, model) {
	function registerProxy(proxy) {
		...
		if (!model.proxyByGroup[group]) model.proxyByGroup[group] = [];
		if (model.proxyByGroup[group].indexOf(proxy) === -1) model.proxyByGroup[group].push(proxy);
	...
	function unRegisterProxy(proxyOrId) {
		...
		Object.keys(model.proxyByGroup).forEach((groupName) => {
			...
			if (index !== -1) proxyList.splice(index, 1);
		});
	...
	publicAPI.setActiveSource = (source) => {
		if (model.activeSource !== source) {
			...
			if (source) model.activeSourceSubscription = source.onModified(publicAPI.modified);
			publicAPI.modified();
	...
	publicAPI.setActiveView = (view) => {
		if (model.activeView !== view) {
			...
			if (view) model.activeViewSubscription = view.onModified(publicAPI.modified);
			publicAPI.modified();
	...
	publicAPI.createProxy = (group, name, options) => {
		...
		if (!definitions[group] || !definitions[group][name]) return null;
		...
		if (definition.props) proxy.set(definition.props);
		registerProxy(proxy);
		if (definitionOptions.activateOnCreate) proxy.activate();
		return proxy;
	};
	publicAPI.getRepresentation = (source, view) => {
		...
		if (!sourceToUse || !viewToUse || !sourceToUse.getType()) return null;
	...
	publicAPI.deleteProxy = (proxy) => {
		...
		if (group === "views") {
			...
			if (publicAPI.getActiveView() === proxy) publicAPI.setActiveView(publicAPI.getViews()[0]);
		} else if (group === "representations") {
			const { sourceId, viewId } = model.r2svMapping[proxy.getProxyId()];
			publicAPI.getProxyById(viewId).removeRepresentation(proxy);
			...
			unRegisterProxy(proxy);
			if (publicAPI.getActiveSource() === proxy) publicAPI.setActiveSource(publicAPI.getSources()[0]);
		} else unRegisterProxy(proxy);
		proxy.delete();
	};
}
//#endregion
