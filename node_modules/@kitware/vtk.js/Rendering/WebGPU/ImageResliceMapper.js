...
import { BufferUsage, PrimitiveTypes } from "./BufferManager/Constants.js";
import { registerOverride } from "./ViewNodeFactory.js";
import { fillLabelOutlineTextureTable, getInputProperty, getLabelOutlineTextureParameters } from "../Core/ImageResliceMapper/LabelOutlineHelper.js";
import { computeColorShiftScale, computeOpacityShiftScale, getTransferFunctionHash } from "../Core/ImageResliceMapper/TransferFunctionHelper.js";
...
import { getTextureChannelMode } from "./Helpers/ImageSampling.js";
import ImageMapper_default from "./ImageMapper.js";
import { mat4 } from "gl-matrix";
//#region Sources/Rendering/WebGPU/ImageResliceMapper/index.js
var { vtkErrorMacro } = macros_default;
var tmpMat4 = new Float64Array(16);
var tmpColorScale = new Float32Array(4);
var tmpColorShift = new Float32Array(4);
var tmpOpacityScale = new Float32Array(4);
var tmpOpacityShift = new Float32Array(4);
var tmpComponentWeight = new Float32Array(4);
var tmpPlaneNormal = [
	0,
	0,
	1
];
var tmpOutlineTexelSize = [
	0,
	0,
	0
];
var tmpOutlineVec4 = new Float32Array(4);
var tmpVec4 = new Float32Array(4);
var ZERO_OUTLINE_VEC4 = new Float32Array(4);
var COMPONENT_NAMES = [
	"r",
	"g",
	"b",
...
function collectValidInputs(actor, renderable) {
	...
	for (let inputIndex = 0; inputIndex < numberOfInputs; ++inputIndex) {
		...
		if (imageData && !imageData.isDeleted()) {
			...
			currentValidInputs.push({
				imageData,
				inputIndex
			});
			const property = getInputProperty(actor, inputIndex);
			if (property?.getUseLabelOutline()) labelOutlineProperties.push({
				property,
				arrayIndex
			});
		}
...
function getColorTextureSlot(model) {
	return model.multiTexturePerVolumeEnabled ? model.currentValidInputs.length : TextureSlot.COLOR_LUT;
}
...
function getOutlineFlagString(actor, currentValidInputs) {
	return currentValidInputs.map(({ inputIndex }) => getInputProperty(actor, inputIndex)?.getUseLabelOutline() ? "1" : "0").join("");
}
...
function getCachedInputTexture(model, imageData, index) {
	const currentMTime = imageData.getPointData().getScalars().getMTime();
	const cachedInfo = model.scalarTextures[index];
	if (cachedInfo && cachedInfo.mtime === currentMTime) return cachedInfo.texture;
	const texture = model.device.getTextureManager().getTextureForImageData(imageData);
...
function getInterpolationFilter(property) {
	return property?.getInterpolationType() === InterpolationType.NEAREST ? "nearest" : "linear";
}
function updateInputTextureView(publicAPI, model, actor, imageData, inputIndex, index) {
	...
	if (!model.textureViews[textureSlot] || model.textureViews[textureSlot].getTexture() !== texture) model.textureViews[textureSlot] = texture.createView(textureName);
	const iType = getInterpolationFilter(getInputProperty(actor, inputIndex));
...
function buildShaderSamplingContext(dimensions) {
	if (dimensions === 2) return {
		...
		coordBounds: "all(sampleCoord >= vec2<f32>(0.0)) && all(sampleCoord <= vec2<f32>(1.0))",
		negBounds: "all(coordNeg >= vec2<f32>(0.0)) && all(coordNeg <= vec2<f32>(1.0))",
		posBounds: "all(coordPos >= vec2<f32>(0.0)) && all(coordPos <= vec2<f32>(1.0))"
	};
	return {
		...
		coordBounds: "all(sampleCoord >= vec3<f32>(0.0)) && all(sampleCoord <= vec3<f32>(1.0))",
		negBounds: "all(coordNeg >= vec3<f32>(0.0)) && all(coordNeg <= vec3<f32>(1.0))",
		posBounds: "all(coordPos >= vec3<f32>(0.0)) && all(coordPos <= vec3<f32>(1.0))"
	};
}
function buildMultiInputSampleExpr(model, numInputs, worldPosExpr) {
	const comps = [];
	const coordSuffix = getSampleCoordSuffix(model.dimensions);
	for (let i = 0; i < MAX_TEXTURE_INPUTS; i++) if (i < numInputs) {
		const texName = getTextureLabelForInput(i);
		const coordExpr = `(mapperUBO.${getWCTCMatrixName(i)} * vec4<f32>(${worldPosExpr}, 1.0)).${coordSuffix}`;
		comps.push(`textureSampleLevel(${texName}, ${texName}Sampler, ${coordExpr}, 0.0).r`);
	} else comps.push("0.0");
...
function buildInitialImageSampleLines(model, samplingCtx, multiInputSampleExpr) {
	const useSlab = model.renderable.getSlabThickness() > 0;
	const rawSampleLine = model.multiTexturePerVolumeEnabled ? `      var rawValue: vec4<f32> = ${multiInputSampleExpr("input.worldPosVS")};` : "      var rawValue: vec4<f32> = textureSampleLevel(imgTexture, imgTextureSampler, sampleCoord, 0.0);";
	const lines = [
	...
	if (useSlab) {
		let negSlabSample = "textureSampleLevel(imgTexture, imgTextureSampler, coordNeg, 0.0)";
		let posSlabSample = "textureSampleLevel(imgTexture, imgTextureSampler, coordPos, 0.0)";
		...
		lines.push("      if (mapperUBO.SlabThickness > 0.0) {", "        var numSlices: i32 = 1;", "        var distTraveled: f32 = mapperUBO.SlabSampleStep;", "        var trapezoid: i32 = 0;", "        let slabNormal: vec3<f32> = normalize(input.normalWC);", "        while (distTraveled < mapperUBO.SlabThickness * 0.5) {", "          distTraveled = distTraveled + mapperUBO.SlabSampleStep;", "          let fnumSlices: f32 = f32(numSlices);", "          var localStep: f32 = fnumSlices * mapperUBO.SlabSampleStep;", "          if (distTraveled > mapperUBO.SlabThickness * 0.5) {", "            localStep = mapperUBO.SlabThickness * 0.5;", "            trapezoid = i32(mapperUBO.SlabTrapezoid);", "          }", "          let worldNeg: vec3<f32> = input.worldPosVS - localStep * slabNormal;", `          let coordNeg: ${samplingCtx.sampleCoordType} = ${samplingCtx.coordFromWorldNeg};`, `          if (${samplingCtx.negBounds}) {`, `            rawValue = vtkCompositeSlab(rawValue, ${negSlabSample}, i32(mapperUBO.SlabType), trapezoid);`, "            numSlices += 1;", "          }", "          let worldPos: vec3<f32> = input.worldPosVS + localStep * slabNormal;", `          let coordPos: ${samplingCtx.sampleCoordType} = ${samplingCtx.coordFromWorldPos};`, `          if (${samplingCtx.posBounds}) {`, `            rawValue = vtkCompositeSlab(rawValue, ${posSlabSample}, i32(mapperUBO.SlabType), trapezoid);`, "            numSlices += 1;", "          }", "        }", "        if (i32(mapperUBO.SlabType) == 2) {", "          rawValue = rawValue / f32(numSlices);", "        }", "      }");
	}
	lines.push("      computedColor = rawValue;", "      //VTK::Image::Sample", "    }");
	return lines;
}
function buildOrderedInputIndices(numberOfComponents, labelRows) {
	const orderedInputs = [];
	for (let i = 0; i < numberOfComponents; i++) if (!labelRows.has(i)) orderedInputs.push(i);
	for (let i = 0; i < numberOfComponents; i++) if (labelRows.has(i)) orderedInputs.push(i);
	return orderedInputs;
}
function createLabelRowsMap(labelOutlineProperties) {
	return new Map(labelOutlineProperties.map(({ arrayIndex }, row) => [arrayIndex, row]));
}
function buildSingleComponentSampleLines(model, samplingCtx, useLabelOutline) {
	const coordSuffix = getSampleCoordSuffix(model.dimensions);
	if (useLabelOutline) return [
		...
		`            if (!(${samplingCtx.coordBounds.replace(/sampleCoord/g, "neighborCoord")})) {`,
		"              pixelOnBorder = true;",
		"              break;",
		"            }",
...
function buildIndependentComponentSampleLines(model, samplingCtx) {
	...
	const orderedInputs = buildOrderedInputIndices(model.numberOfComponents, labelRows);
	const coordSuffix = getSampleCoordSuffix(model.dimensions);
	const lines = ["      var convergentColor: vec4<f32> = vec4<f32>(0.0);"];
	for (let i = 0; i < model.numberOfComponents; i++) {
		const rowExpr = `${2 * i}.0 / f32(textureDimensions(tfunTexture, 0).y) + 0.5 / f32(textureDimensions(tfunTexture, 0).y)`;
		lines.push(`      let scalar${i}: f32 = computedColor.${COMPONENT_NAMES[i]};`, `      let colorCoord${i}: vec2<f32> = vec2<f32>(scalar${i} * mapperUBO.cScale[${i}] + mapperUBO.cShift[${i}], ${rowExpr});`, `      let tColor${i}: vec4<f32> = textureSampleLevel(tfunTexture, tfunTextureSampler, colorCoord${i}, 0.0);`, `      let opacityCoord${i}: vec2<f32> = vec2<f32>(scalar${i} * mapperUBO.oScale[${i}] + mapperUBO.oShift[${i}], ${rowExpr});`, `      let alpha${i}: f32 = textureSampleLevel(ofunTexture, ofunTextureSampler, opacityCoord${i}, 0.0).r * mapperUBO.componentWeight[${i}] * mapperUBO.Opacity;`);
	}
	for (let orderedIdx = 0; orderedIdx < orderedInputs.length; orderedIdx++) {
		...
		if (labelRows.has(i)) {
			...
			lines.push(`      if (scalar${i} > 0.0) {`, `        let sampleCoord${i}: ${samplingCtx.sampleCoordType} = (mapperUBO.${getWCTCMatrixName(i)} * vec4<f32>(input.worldPosVS, 1.0)).${coordSuffix};`, `        let segmentIndex${i}: u32 = u32(scalar${i} * 255.0);`, `        let textureCoordinate${i}: f32 = f32(segmentIndex${i} - 1u) / 255.0;`, `        let labelmapRow${i}: f32 = (${row}.0 + 0.5) / f32(textureDimensions(labelOutlineThickness, 0).y);`, `        let thickness${i}: i32 = i32(textureSampleLevel(labelOutlineThickness, labelOutlineThicknessSampler, vec2<f32>(textureCoordinate${i}, labelmapRow${i}), 0.0).r * 255.0);`, `        let outlineOpacity${i}: f32 = textureSampleLevel(labelOutlineOpacity, labelOutlineOpacitySampler, vec2<f32>(textureCoordinate${i}, labelmapRow${i}), 0.0).r;`, `        var pixelOnBorder${i}: bool = false;`, `        for (var ii${i}: i32 = -thickness${i}; ii${i} <= thickness${i}; ii${i}++) {`, `          for (var jj${i}: i32 = -thickness${i}; jj${i} <= thickness${i}; jj${i}++) {`, `            if (ii${i} == 0 && jj${i} == 0) { continue; }`, `            let neighborCoord${i}: ${samplingCtx.sampleCoordType} = sampleCoord${i} + f32(ii${i}) * mapperUBO.${getOutlineTangent1Name(i)}.${coordSuffix} * mapperUBO.${getOutlineTexelSizeName(i)}.${coordSuffix} + f32(jj${i}) * mapperUBO.${getOutlineTangent2Name(i)}.${coordSuffix} * mapperUBO.${getOutlineTexelSizeName(i)}.${coordSuffix};`, `            if (!(${samplingCtx.coordBounds.replace(/sampleCoord/g, `neighborCoord${i}`)})) { pixelOnBorder${i} = true; break; }`, `            let neighborValue${i}: f32 = textureSampleLevel(${texName}, ${texName}Sampler, neighborCoord${i}, 0.0).r;`, `            if (neighborValue${i} != scalar${i}) { pixelOnBorder${i} = true; break; }`, "          }", `          if (pixelOnBorder${i}) { break; }`, "        }", `        let finalAlpha${i}: f32 = select(alpha${i}, outlineOpacity${i}, pixelOnBorder${i});`, `        convergentColor = vec4<f32>(mix(convergentColor.rgb, tColor${i}.rgb, finalAlpha${i}), max(convergentColor.a, finalAlpha${i}));`, "      }");
		} else lines.push(`      convergentColor = vec4<f32>(mix(convergentColor.rgb, tColor${i}.rgb, alpha${i}), max(convergentColor.a, alpha${i}));`);
	}
...
function buildDependentComponentSampleLines(textureChannelMode) {
	switch (textureChannelMode) {
		case TextureChannelMode.DEPENDENT_LA: return [
		...
		case TextureChannelMode.DEPENDENT_RGB: return [
		...
		case TextureChannelMode.DEPENDENT_RGBA: return [
			"      let rawColor: vec4<f32> =",
			"        computedColor * vec4<f32>(mapperUBO.cScale.x) + vec4<f32>(mapperUBO.cShift.x);",
			"      computedColor = vec4<f32>(",
			"        textureSampleLevel(tfunTexture, tfunTextureSampler, vec2<f32>(rawColor.r, 0.5), 0.0).r,",
...
function updateResliceMatrixUBO(model) {
	...
	mat4.translate(tmpMat4, tmpMat4, [
		-.5,
		-.5,
		-.5
	]);
	...
	for (let i = 1; i < MAX_TEXTURE_INPUTS; i++) {
		...
		} else model.UBO.setArray(getWCTCMatrixName(i), mat4.identity(tmpMat4));
	}
}
function updateOutlineBasisUBO(model, slicePlane) {
	const { planeNormal, tangent1, tangent2 } = computeSliceTangents(slicePlane, tmpPlaneNormal);
	model.UBO.setArray("PlaneNormalWC", setTmpVec4(tmpVec4, planeNormal[0], planeNormal[1], planeNormal[2]));
	for (let i = 0; i < MAX_TEXTURE_INPUTS; i++) {
		const imageData = model.currentValidInputs[i]?.imageData;
		if (imageData) {
			const { tangent1: inputTangent1, tangent2: inputTangent2, texelSize } = computeInputOutlineBasis(imageData, tangent1, tangent2, tmpOutlineTexelSize);
			model.UBO.setArray(getOutlineTangent1Name(i), setTmpVec4(tmpOutlineVec4, inputTangent1[0], inputTangent1[1], inputTangent1[2]));
			model.UBO.setArray(getOutlineTangent2Name(i), setTmpVec4(tmpOutlineVec4, inputTangent2[0], inputTangent2[1], inputTangent2[2]));
			model.UBO.setArray(getOutlineTexelSizeName(i), setTmpVec4(tmpOutlineVec4, texelSize[0], texelSize[1], texelSize[2]));
		} else {
...
function updateTransferFunctionUBO(model, actor, ppty, imageState) {
	...
	const { numberOfComponents: numComp, independentComponents: iComps } = imageState;
	for (let i = 0; i < numComp; i++) {
		const textureSlot = model.multiTexturePerVolumeEnabled ? getTextureSlotForInput(i) : TextureSlot.IMAGE;
		const tScale = model.textureViews[textureSlot].getTexture().getScale();
		let inputProperty = ppty;
		if (model.multiTexturePerVolumeEnabled && model.currentValidInputs[i]) inputProperty = getInputProperty(actor, model.currentValidInputs[i].inputIndex);
		const cw = inputProperty.getColorWindow();
		const cl = inputProperty.getColorLevel();
		let target = 0;
		if (!model.multiTexturePerVolumeEnabled && iComps) target = i;
		...
		const opacityParams = computeOpacityShiftScale({
			pwfRange: inputProperty.getPiecewiseFunction(target)?.getRange?.(),
			volumeScale: tScale,
			volumeOffset: 0
		});
...
function vtkWebGPUImageResliceMapper(publicAPI, model) {
	...
	publicAPI.render = () => {
		...
		const { currentValidInputs, labelOutlineProperties } = collectValidInputs(model.WebGPUImageSlice.getRenderable(), model.renderable);
		...
		if (!model.numberOfVertices) return;
	...
	publicAPI.computePipelineHash = () => {
		...
		model.pipelineHash += `slab${model.renderable.getSlabThickness() > 0 ? 1 : 0}`;
		model.pipelineHash += `norm${model.vertexInput.hasAttribute("vertexNormal") ? 1 : 0}`;
		if (model.multiTexturePerVolumeEnabled) model.pipelineHash += `multi${model.numberOfComponents}`;
		model.pipelineHash += `in${model.currentValidInputs.length}`;
		model.pipelineHash += `lbl${getOutlineFlagString(actor, model.currentValidInputs)}`;
	...
	publicAPI.updateResliceGeometry = () => {
		...
		if (model.resliceGeom && model.resliceGeomUpdateString === resGeomString) return;
		if (slicePD) {
			...
			if (!orthoSlicing) {
				...
				model.resliceGeom.getPoints().setData(points, 3);
				model.resliceGeom.getPolys().setData(polys, 1);
				model.resliceGeom.getPointData().setNormals(DataArray_default.newInstance({
					numberOfComponents: 3,
					values: normalsData,
					name: "Normals"
				}));
	...
	publicAPI.updateGeometryBuffers = () => {
		...
		if (!indexBuffer || !indexBuffer.getFlatSize() || !indexBuffer.getIndexCount()) {
			clearGeometryBuffers(publicAPI, model);
			return;
		}
		model.vertexInput.setIndexBuffer(indexBuffer);
		model.vertexInput.addBuffer(device.getBufferManager().getBuffer({
		...
		if (useSlab && normals) model.vertexInput.addBuffer(device.getBufferManager().getBuffer({
		...
		else model.vertexInput.removeBufferIfPresent("vertexNormal");
		publicAPI.setTopology("triangle-list");
		publicAPI.setNumberOfVertices(indexBuffer.getIndexCount());
	};
	publicAPI.updateUBO = () => {
		const actor = model.WebGPUImageSlice.getRenderable();
		const ppty = getInputProperty(actor, model.currentValidInputs[0].inputIndex);
		const mapper = actor.getMapper();
		const slicePlane = mapper.getSlicePlane();
		const center = model.WebGPURenderer.getStabilizedCenterByReference();
		const imageState = publicAPI.getImageState();
		updateResliceMatrixUBO(model);
		model.UBO.setArray("StabilizedCenter", setTmpVec4(tmpVec4, center[0], center[1], center[2]));
		model.UBO.setArray("BackgroundColor", model.renderable.getBackgroundColorByReference());
		const spacing = model.currentInput.getSpacing();
		const sampleStep = .5 * Math.min(spacing[0], spacing[1], spacing[2]);
	...
	publicAPI.updateBuffers = () => {
		publicAPI.updateGeometryBuffers();
		if (!model.currentInput || !model.numberOfVertices) return;
		const actor = model.WebGPUImageSlice.getRenderable();
		const textureCount = model.currentValidInputs.length;
		if (model.multiTexturePerVolumeEnabled) for (let i = 0; i < textureCount; i++) {
			const { imageData, inputIndex } = model.currentValidInputs[i];
			updateInputTextureView(publicAPI, model, actor, imageData, inputIndex, i);
		}
		else {
			const { imageData, inputIndex } = model.currentValidInputs[0];
			updateInputTextureView(publicAPI, model, actor, imageData, inputIndex, 0);
		}
		for (let i = textureCount; i < model.scalarTextures.length; i++) model.scalarTextures[i] = null;
		...
		const maxSlot = model.imageState.useLabelOutline ? getLabelOutlineOpacitySlot(model) : getOpacityTextureSlot(model);
		model.textureViews.length = Math.min(model.textureViews.length, maxSlot + 1);
		publicAPI.updateUBO();
	};
	publicAPI.replaceShaderPosition = (hash, pipeline, vertexInput) => {
		...
		if (model.dimensions === 2) lines.push("output.tcoordVS = (mapperUBO.SCTCMatrix * vertexWC).xy;");
		else lines.push("output.tcoordVS = (mapperUBO.SCTCMatrix * vertexWC).xyz;");
		lines.push("output.worldPosVS = vertexWC.xyz;");
		if (useSlab) lines.push(vertexInput.hasAttribute("vertexNormal") ? "output.normalWC = normalize(vertexNormal);" : "output.normalWC = mapperUBO.PlaneNormalWC.xyz;");
		lines.push("output.vertexSC = vertexSC;", "var pos: vec4<f32> = rendererUBO.SCPCMatrix * vertexSC;", "pos.z = clamp(pos.z - 0.000016 * mapperUBO.CoincidentOffset * pos.w, 0.0, pos.w);", "output.Position = pos;");
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", lines).result;
		vDesc.setCode(code);
	};
	publicAPI.getShaderReplacements().set("replaceShaderPosition", publicAPI.replaceShaderPosition);
	publicAPI.replaceShaderTCoord = (hash, pipeline, vertexInput) => {
		...
		if (model.renderable.getSlabThickness() > 0) vDesc.addOutput("vec3<f32>", "normalWC");
	};
	publicAPI.getShaderReplacements().set("replaceShaderTCoord", publicAPI.replaceShaderTCoord);
	publicAPI.replaceShaderImage = (hash, pipeline, vertexInput) => {
		...
		const multiInputSampleExpr = (worldPosExpr) => buildMultiInputSampleExpr(model, numInputs, worldPosExpr);
		code = ShaderCache_default.substitute(code, "//VTK::Image::Dec", getSlabCompositeDecLines()).result;
		code = ShaderCache_default.substitute(code, "//VTK::Image::Sample", buildInitialImageSampleLines(model, samplingCtx, multiInputSampleExpr)).result;
		switch (imageState.textureChannelMode) {
			case TextureChannelMode.SINGLE:
			case TextureChannelMode.INDEPENDENT_1:
				code = ShaderCache_default.substitute(code, "//VTK::Image::Sample", buildSingleComponentSampleLines(model, samplingCtx, imageState.useLabelOutline)).result;
				break;
			case TextureChannelMode.INDEPENDENT_2:
			case TextureChannelMode.INDEPENDENT_3:
			case TextureChannelMode.INDEPENDENT_4:
				code = ShaderCache_default.substitute(code, "//VTK::Image::Sample", buildIndependentComponentSampleLines(model, samplingCtx)).result;
				break;
			default:
				code = ShaderCache_default.substitute(code, "//VTK::Image::Sample", buildDependentComponentSampleLines(imageState.textureChannelMode)).result;
				break;
		}
		fDesc.setCode(code);
	};
	publicAPI.getShaderReplacements().set("replaceShaderImage", publicAPI.replaceShaderImage);
	publicAPI.updateLabelOutlineThicknessTexture = () => {
		if (!model.labelOutlineProperties.length) return;
		const { dataArrays, hash, width, height } = getLabelOutlineTextureParameters(model.labelOutlineProperties, (property) => property.getLabelOutlineThicknessByReference());
		if (hash === model._labelOutlineThicknessHash) return;
		...
		const tview = model.device.getTextureManager().getTexture({
		...
		}).createView("labelOutlineThickness");
	...
	publicAPI.updateLabelOutlineOpacityTexture = () => {
		if (!model.labelOutlineProperties.length) return;
		const { dataArrays, hash, width, height } = getLabelOutlineTextureParameters(model.labelOutlineProperties, (property) => {
			const dataArray = property.getLabelOutlineOpacity();
			return typeof dataArray === "number" ? [dataArray] : dataArray;
		});
		if (hash === model._labelOutlineOpacityHash) return;
		...
		const tview = model.device.getTextureManager().getTexture({
			...
			format: "r16float",
			usage: getTextureBindingUsage()
		}).createView("labelOutlineOpacity");
		publicAPI.ensureTextureSampler(tview, {
			minFilter: "nearest",
			magFilter: "nearest"
		});
...
function extend(publicAPI, model, initialValues = {}) {
	...
	ImageMapper_default.extend(publicAPI, model, initialValues);
	macros_default.get(publicAPI, model, ["scalarTextures"]);
	const superComputeImageState = publicAPI.computeImageState;
	publicAPI.computeImageState = () => {
		if (model.multiTexturePerVolumeEnabled && model.currentValidInputs?.length) {
			const firstProperty = getInputProperty(model.WebGPUImageSlice.getRenderable(), model.currentValidInputs[0].inputIndex);
			...
			return {
				...
				textureChannelMode: getTextureChannelMode(independentComponents, model.numberOfComponents)
			};
		}
	...
	publicAPI.updateLUTImage = () => {
		...
		if (model.colorTextureString === colorHash) return;
		...
		for (let c = 0; c < numIComps; c++) {
			const cfun = getInputProperty(actor, model.currentValidInputs[c].inputIndex)?.getRGBTransferFunction(0);
			if (cfun) {
	...
	publicAPI.updateOpacityLUTImage = () => {
		...
		if (model.opacityTextureString === opacityHash) return;
		model.numRows = numIComps;
		...
		opacityArray.fill(1);
		for (let c = 0; c < numIComps; c++) {
			const pwfun = getInputProperty(actor, model.currentValidInputs[c].inputIndex)?.getPiecewiseFunction(0);
			if (pwfun) {
	...
	for (let i = 0; i < 4; i++) {
		if (i > 0) model.UBO.addEntry(getWCTCMatrixName(i), "mat4x4<f32>");
...
var newInstance = macros_default.newInstance(extend, "vtkWebGPUImageResliceMapper");
