...
//#region Sources/Common/Transform/Transform/index.js
...
function vtkTransform(publicAPI, model) {
	...
	*/
	...
	publicAPI.transformMatrix = (matrix, out) => {
		if (model.preMultiplyFlag) mat4.multiply(out, model.matrix, matrix);
		else mat4.multiply(out, matrix, model.matrix);
		return out;
	};
	publicAPI.transformMatrices = (matrices, out) => {
		...
		for (let i = 0; i < matrices.length; i += 16) {
			for (let j = 0; j < 16; ++j) inMat[j] = matrices[i + j];
			transform();
			for (let j = 0; j < 16; ++j) out[i + j] = outMat[j];
		}
		return out;
	};
	publicAPI.getInverse = () => newInstance({
		matrix: Math_default.invertMatrix(Array.from(model.matrix), [], 4),
	...
	*/
	publicAPI.translate = (x, y, z) => {
		...
		if (model.preMultiplyFlag) mat4.multiply(model.matrix, model.matrix, tMat);
		else mat4.multiply(model.matrix, tMat, model.matrix);
		publicAPI.modified();
	};
	/**
	...
	*/
	publicAPI.rotateWXYZ = (degrees, x, y, z) => {
		...
		if (degrees === 0) return;
		const angle = Math_default.radiansFromDegrees(degrees);
		const q = quat.create();
		quat.setAxisAngle(q, [
			x,
			y,
		...
		if (model.preMultiplyFlag) mat4.multiply(model.matrix, model.matrix, quatMat);
		else mat4.multiply(model.matrix, quatMat, model.matrix);
		publicAPI.modified();
	};
	/**
	...
	*/
	publicAPI.scale = (x, y, z) => {
		...
		mat4.fromScaling(sMat, [
			x,
			y,
			z
		]);
		if (model.preMultiplyFlag) mat4.multiply(model.matrix, model.matrix, sMat);
		else mat4.multiply(model.matrix, sMat, model.matrix);
		publicAPI.modified();
	};
	/**
	...
	*/
	publicAPI.transformNormal = (inNormal, outNormal = []) => {
		...
		Math_default.normalize(outNormal);
		return outNormal;
	};
	/**
	...
	*/
	publicAPI.transformPointsNormalsVectors = (inPoints, outPoints, inNormals, outNormals, inVectors, outVectors, inVectorsArr = null, outVectorsArr = null) => {
		...
		for (let ptId = 0; ptId < n; ptId++) {
			...
			if (!Math_default.areEquals(oldPoint, point)) modifiedPoint = true;
			if (inVectors) {
				...
				if (!Math_default.areEquals(oldVector, point)) modifiedVector = true;
			}
			if (inNormals) {
				...
				outData[ptId * 3 + 1] = point[1];
				outData[ptId * 3 + 2] = point[2];
				if (!Math_default.areEquals(oldNormal, point)) modifiedNormal = true;
			}
			if (inVectorsArr) for (let iArr = 0; iArr < nOptionalVectors; iArr++) {
				...
				outData[ptId * 3 + 1] = point[1];
				outData[ptId * 3 + 2] = point[2];
				if (!Math_default.arrayEqual(oldVector, point) && !modifiedVectorsArr.includes(iArr)) modifiedVectorsArr.push(iArr);
			}
		}
		if (modifiedPoint) outPoints.modified();
		if (modifiedVector) outVectors.modified();
		if (modifiedNormal) outNormals.modified();
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.setGet(publicAPI, model, ["preMultiplyFlag"]);
	macros_default.setGetArray(publicAPI, model, ["matrix"], 16);
	vtkTransform(publicAPI, model);
}
newInstance = macros_default.newInstance(extend, "vtkTransform");
var Transform_default = {
	newInstance,
	extend
};
//#endregion
export { Transform_default as default, extend, newInstance };
