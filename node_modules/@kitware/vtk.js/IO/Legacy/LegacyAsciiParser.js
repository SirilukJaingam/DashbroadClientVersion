...
//#region Sources/IO/Legacy/LegacyAsciiParser/index.js
var METHOD_MAPPING = {
	POINTS: "getPoints",
	VERTICES: "getVerts",
	LINES: "getLines",
...
function createArrayHandler(array, setData, nbComponents) {
	...
	function fillWith(line) {
		line.split(" ").forEach((token) => {
			if (token.length) array[offset++] = Number(token);
		});
		if (offset < array.length) return true;
...
var TYPE_PARSER = {
	DATASET: {
		init(line, datamodel) {
			...
			switch (type) {
				case "POLYDATA":
					datamodel.dataset = PolyData_default.newInstance();
					break;
				default: console.error(`Dataset of type ${type} not supported`);
			}
			return false;
		},
		parse(line, datamodel) {
	...
	SCALARS: {
		...
		parse(line, dataModel) {
			if (line.split(" ")[0] === "LOOKUP_TABLE") return true;
			return dataModel.arrayHandler(line);
		}
	},
	COLOR_SCALARS: {
...
function getParser(line, dataModel) {
	return TYPE_PARSER[line.split(" ")[0]];
}
function parseLegacyASCII(content, dataModel = {}) {
	let parser = null;
	const separatorRes = /\r?\n/.exec(content);
	...
	content.split(separator).forEach((line, index) => {
		...
		if (parser && !parser.parse(line, dataModel)) parser = null;
	});
	return dataModel;
}
var LegacyAsciiParser_default = { parseLegacyASCII };
//#endregion
export { LegacyAsciiParser_default as default };

//# sourceMappingURL=LegacyAsciiParser.js.map