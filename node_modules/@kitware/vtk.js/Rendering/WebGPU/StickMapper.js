...
//#region Sources/Rendering/WebGPU/StickMapper/index.js
...
function vtkWebGPUStickMapper(publicAPI, model) {
  ...
  `;
		...
		code = ShaderCache_default.substitute(code, "//VTK::Normal::Impl", [stickFrag]).result;
		fDesc.setCode(code);
	};
	publicAPI.replaceShaderPosition = (hash, pipeline, vertexInput) => {
		...
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", ["  output.Position = rendererUBO.VCPCMatrix*vertexVC;"]).result;
		vDesc.setCode(code);
	};
	publicAPI.computePipelineHash = () => {
		model.pipelineHash = "stm";
		if (model.vertexInput.hasAttribute(`colorVI`)) model.pipelineHash += `c`;
		...
		if (scales || defaultRadius !== model._lastRadius) {
			...
			if (!device.getBufferManager().hasBuffer(hash)) {
				...
				for (let id = 0; id < numPoints; ++id) {
					...
					if (scales) radius = scales[id * 2 + 1];
		...
		if (model.renderable.getScalarVisibility()) {
			...
			if (c) {
				...
				if (!device.getBufferManager().hasBuffer(hash)) {
					...
					if (colorComponents !== 4) vtkErrorMacro("this should be 4");
