...
//#region Sources/Proxy/Representations/SliceRepresentationProxy/index.js
function sum(a, b) {
	return a + b;
}
function mean(...array) {
...
function updateDomains(dataset, dataArray, model, updateProp) {
	...
	switch (sliceMode) {
		case ImageMapper_default.SlicingMode.I:
		case ImageMapper_default.SlicingMode.J:
		case ImageMapper_default.SlicingMode.K:
...
function vtkSliceRepresentationProxy(publicAPI, model) {
	...
	model.mapper = ImageMapper_default.newInstance();
	...
	function updateSlicingMode(mode) {
		model.mapper.setSlicingMode(ImageMapper_default.SlicingMode[mode]);
		const modelKey = `${mode.toLowerCase()}Slice`;
		if (modelKey in model && model[modelKey] !== void 0) model.mapper.setSlice(model[modelKey]);
		if (model.input) {
	...
	publicAPI.getSliceIndex = () => {
		if ("XYZ".indexOf(model.slicingMode) !== -1) return model.mapper.getSliceAtPosition(model.mapper.getSlice());
	...
	publicAPI.getAnnotations = () => {
		...
		const { ijkMode } = model.mapper.getClosestIJKAxis();
		const sliceOrigin = [
			(sliceBounds[0] + sliceBounds[1]) * .5,
			(sliceBounds[2] + sliceBounds[3]) * .5,
			(sliceBounds[4] + sliceBounds[5]) * .5
		];
		let slicePosition = 0;
		if (sliceBounds[1] - sliceBounds[0] < Number.EPSILON) slicePosition = sliceBounds[0];
		if (sliceBounds[3] - sliceBounds[2] < Number.EPSILON) slicePosition = sliceBounds[2];
		if (sliceBounds[5] - sliceBounds[4] < Number.EPSILON) slicePosition = sliceBounds[4];
		...
		if (imageData) {
			...
			for (let i = 0; i < 3; i++) {
				...
				if (tmpBounds[1] - tmpBounds[0] > Number.EPSILON) ijkOrientation[0] = "IJK"[i];
				if (tmpBounds[3] - tmpBounds[2] > Number.EPSILON) ijkOrientation[1] = "IJK"[i];
				if (tmpBounds[5] - tmpBounds[4] > Number.EPSILON) ijkOrientation[2] = "IJK"[i];
			}
...
var newInstance = macros_default.newInstance(extend, "vtkSliceRepresentationProxy");
var SliceRepresentationProxy_default = {
	newInstance,
	extend
};
//#endregion
export { SliceRepresentationProxy_default as default, extend, newInstance };

//# sourceMappingURL=SliceRepresentationProxy.js.map