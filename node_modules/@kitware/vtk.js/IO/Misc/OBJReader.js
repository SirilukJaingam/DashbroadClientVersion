...
//#region Sources/IO/Misc/OBJReader/index.js
var data = {};
function copyVector(src, srcOffset, dst, dstOffset, vectorSize) {
	for (let i = 0; i < vectorSize; i++) dst[dstOffset + i] = src[srcOffset + i];
}
...
function parseLine(line) {
	...
	if (tokens[0] === data.splitOn) {
		...
		cells.push(size);
		for (let i = 0; i < size; i++) cells.push(faceMap(faces[i]));
	}
}
function end(model) {
	...
	if (model.splitMode) {
		...
		for (let idx = 0; idx < data.size; idx++) {
				...
				for (let offset = 0; offset < nbElems;) {
					...
					for (let pIdx = 0; pIdx < cellSize; pIdx++) {
						...
						if (keyPointId[key] === void 0) {
							if (pointKeys[vIdx] === void 0) pointKeys[vIdx] = [key];
							else {
			...
			const ctMapping = {};
			const polydata = PolyData_default.newInstance({ name: data.pieces[idx] });
			const pts = [];
			const tc = [];
			const normals = [];
			const polys = [];
			...
			while (offset < nbElems) {
				...
				for (let pIdx = 0; pIdx < cellSize; pIdx++) {
					...
					if (ctMapping[key] === void 0) {
						...
						if (hasTcoords) copyVector(data.vt, tcIdx * 2, tc, dstOffset * 2, 2);
						if (hasNormals) copyVector(data.vn, nIdx * 3, normals, dstOffset * 3, 3);
					}
			...
			if (model.trackDuplicates) {
				const duplicatesArray = DataArray_default.newInstance({
					name: "Duplicates",
			...
			if (hasNormals) {
				const normalsArray = DataArray_default.newInstance({
...
function getPointDuplicateIds(polyData, pointId) {
	...
	if (duplicateId !== originalPointId) while (duplicateId < duplicatesData.length && duplicatesData[duplicateId] === originalPointId) res.push(duplicateId++);
	return res;
}
var STATIC = { getPointDuplicateIds };
...
var newInstance = macros_default.newInstance(extend, "vtkOBJReader");
var OBJReader_default = {
	newInstance,
	extend,
	...STATIC
};
//#endregion
export { STATIC, OBJReader_default as default, extend, newInstance };
