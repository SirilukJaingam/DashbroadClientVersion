...
import { norm } from "../../Common/Core/Math.js";
import BoundingBox_default from "../../Common/DataModel/BoundingBox.js";
import Mapper_default from "./Mapper.js";
import Constants_default from "./Glyph3DMapper/Constants.js";
import { mat3, mat4, vec3 } from "gl-matrix";
//#region Sources/Rendering/Core/Glyph3DMapper/index.js
var { OrientationModes, ScaleModes } = Constants_default;
...
function vtkGlyph3DMapper(publicAPI, model) {
	...
	* freedom left.
	*/
	publicAPI.getOrientationModeAsString = () => macros_default.enumToString(OrientationModes, model.orientationMode);
	...
	publicAPI.getOrientationArrayData = () => {
		...
		if (!idata || !idata.getPointData()) return null;
		if (!model.orientationArray) return idata.getPointData().getVectors();
		return idata.getPointData().getArray(model.orientationArray);
	};
	publicAPI.getScaleModeAsString = () => macros_default.enumToString(ScaleModes, model.scaleMode);
	...
	publicAPI.getScaleArrayData = () => {
		const idata = publicAPI.getInputData(0);
		if (!idata || !idata.getPointData()) return null;
		if (!model.scaleArray) return idata.getPointData().getScalars();
	...
	publicAPI.buildArrays = () => {
		...
		if (model.buildTime.getMTime() < gdata.getMTime() || model.buildTime.getMTime() < idata.getMTime() || model.buildTime.getMTime() < publicAPI.getMTime()) {
			...
			const corners = [];
			BoundingBox_default.getCorners(gbounds, corners);
			BoundingBox_default.reset(model.bounds);
			...
			for (let i = 0; i < numPts; ++i) {
				...
				if (oArray) {
					...
					switch (model.orientationMode) {
						...
						case OrientationModes.DIRECTION:
							if (orientation[1] === 0 && orientation[2] === 0) {
								if (orientation[0] < 0) mat4.rotateY(z, z, 3.1415926);
							} else {
								const vMag = norm(orientation);
								const vNew = [];
								vNew[0] = (orientation[0] + vMag) / 2;
								vNew[1] = orientation[1] / 2;
								vNew[2] = orientation[2] / 2;
				...
				if (model.scaling) {
					...
					if (sArray) switch (model.scaleMode) {
						case ScaleModes.SCALE_BY_MAGNITUDE:
							for (let t = 0; t < numSComp; ++t) tuple[t] = sData[i * numSComp + t];
					...
					if (scale[0] === 0) scale[0] = 1e-10;
					if (scale[1] === 0) scale[1] = 1e-10;
					if (scale[2] === 0) scale[2] = 1e-10;
					mat4.scale(z, z, scale);
				}
				for (let p = 0; p < 8; ++p) {
					vec3.transformMat4(tcorner, corners[p], z);
					BoundingBox_default.addPoint(model.bounds, tcorner);
				}
...
function extend(publicAPI, model, initialValues = {}) {
	...
	model.buildTime = {};
	macros_default.obj(model.buildTime, { mtime: 0 });
...
var newInstance = macros_default.newInstance(extend, "vtkGlyph3DMapper");
