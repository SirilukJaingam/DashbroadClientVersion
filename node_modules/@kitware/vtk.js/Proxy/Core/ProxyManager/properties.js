//#region Sources/Proxy/Core/ProxyManager/properties.js
function addVPropertyHandlingAPI(publicAPI, model) {
	publicAPI.getSections = () => {
		...
		if (source) {
			...
			if (section.ui.length) sections.push(Object.assign(section, { collapsed: model.collapseState[section.name] }));
		}
		if (source && view) {
	...
	publicAPI.applyChanges = (changeSet) => {
		...
		while (count--) {
			...
			const obj = publicAPI.getProxyById(id);
			if (obj) obj.set(groupBy[id]);
		}
		publicAPI.modified();
		publicAPI.renderAllViews();
	};
	publicAPI.getLookupTable = (arrayName, options) => {
		if (!model.lookupTables[arrayName]) model.lookupTables[arrayName] = publicAPI.createProxy("Proxy", "LookupTable", {
			arrayName,
			...options
		});
	...
	publicAPI.getPiecewiseFunction = (arrayName, options) => {
		if (!model.piecewiseFunctions[arrayName]) model.piecewiseFunctions[arrayName] = publicAPI.createProxy("Proxy", "PiecewiseFunction", {
			arrayName,
			...options
		});
