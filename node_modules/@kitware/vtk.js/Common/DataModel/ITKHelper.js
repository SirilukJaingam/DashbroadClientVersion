...
import ImageData_default from "./ImageData.js";
import PolyData_default from "./PolyData.js";
//#region Sources/Common/DataModel/ITKHelper/index.js
...
var vtkArrayTypeToItkComponentType = new Map([
	["Uint8Array", "uint8"],
	["Int8Array", "int8"],
	["Uint16Array", "uint16"],
	["Int16Array", "int16"],
...
var itkComponentTypeToVtkArrayType = new Map([
	["uint8", "Uint8Array"],
	["int8", "Int8Array"],
	["uint16", "Uint16Array"],
	["int16", "Int16Array"],
...
function convertItkToVtkImage(itkImage, options = {}) {
	...
	for (let idx = 0; idx < itkImage.imageType.dimension; ++idx) {
		...
		for (let col = 0; col < itkImage.imageType.dimension; ++col) if (isITKWasm) direction[col + idx * 3] = itkImage.direction[idx + col * itkImage.imageType.dimension];
		else direction[col + idx * 3] = itkImage.direction.data[idx + col * itkImage.imageType.dimension];
	}
	const imageData = ImageData_default.newInstance(vtkImage);
	const pointData = DataArray_default.newInstance({
	...
	switch (isITKWasm ? ITKPixelTypes[itkImage.imageType.pixelType] : itkImage.imageType.pixelType) {
		case ITKPixelTypes.Scalar: break;
		case ITKPixelTypes.RGB: break;
		case ITKPixelTypes.RGBA: break;
		case ITKPixelTypes.Offset: break;
		case ITKPixelTypes.Vector:
			if (itkImage.imageType.dimension === 3 && itkImage.imageType.components === 3) imageData.getPointData().setVectors(pointData);
			break;
		case ITKPixelTypes.Point: break;
		case ITKPixelTypes.CovariantVector:
			if (itkImage.imageType.dimension === 3 && itkImage.imageType.components === 3) imageData.getPointData().setVectors(pointData);
			break;
		case ITKPixelTypes.SymmetricSecondRankTensor:
			if (itkImage.imageType.dimension === 3 && itkImage.imageType.components === 6) imageData.getPointData().setTensors(pointData);
			break;
		case ITKPixelTypes.DiffusionTensor3D:
			if (itkImage.imageType.dimension === 3 && itkImage.imageType.components === 6) imageData.getPointData().setTensors(pointData);
			break;
		case ITKPixelTypes.Complex: break;
		case ITKPixelTypes.FixedArray: break;
		case ITKPixelTypes.Array: break;
		case ITKPixelTypes.Matrix: break;
		case ITKPixelTypes.VariableLengthVector: break;
		case ITKPixelTypes.VariableSizeMatrix: break;
		default:
...
function convertVtkToItkImage(vtkImage, copyData = false) {
	...
	for (let idx = 0; idx < dimension; ++idx) for (let idy = 0; idy < dimension; ++idy) itkImage.direction[idx + idy * dimension] = direction[idy + idx * dimension];
	...
	} else vtkArray = pointData.getScalars();
	itkImage.imageType.componentType = vtkArrayTypeToItkComponentType.get(vtkArray.getDataType());
	if (copyData) itkImage.data = vtkArray.getData().slice(0);
	else itkImage.data = vtkArray.getData();
	return itkImage;
}
/**
...
function convertItkToVtkPolyData(itkPolyData, options = {}) {
	...
	if (itkPolyData.pointData.length) pointDataArrays.push({ data: {
	...
	if (itkPolyData.cellData.length) cellDataArrays.push({ data: {
		vtkClass: "vtkDataArray",
	...
	const polyData = PolyData_default.newInstance(vtkPolyDataModel);
	const pd = polyData.getPointData();
	const cd = polyData.getCellData();
	if (itkPolyData.pointData.length) switch (ITKWASMPixelTypes[itkPolyData.polyDataType.pointPixelType]) {
		case ITKWASMPixelTypes.Scalar:
			pd.setScalars(pd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.RGB: break;
		case ITKWASMPixelTypes.RGBA: break;
		case ITKWASMPixelTypes.Offset: break;
		case ITKWASMPixelTypes.Vector:
			if (itkPolyData.polyDataType.pointPixelComponents === 3) pd.setVectors(pd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.Point: break;
		case ITKWASMPixelTypes.CovariantVector:
			if (itkPolyData.polyDataType.pointPixelComponents === 3) pd.setVectors(pd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.SymmetricSecondRankTensor:
			if (itkPolyData.polyDataType.pointPixelComponents === 6) pd.setTensors(pd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.DiffusionTensor3D:
			if (itkPolyData.polyDataType.pointPixelComponents === 6) pd.setTensors(pd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.Complex: break;
		case ITKWASMPixelTypes.FixedArray: break;
		case ITKWASMPixelTypes.Array: break;
		case ITKWASMPixelTypes.Matrix: break;
		case ITKWASMPixelTypes.VariableLengthVector: break;
		case ITKWASMPixelTypes.VariableSizeMatrix: break;
		default:
			vtkErrorMacro(`Cannot handle unexpected itk-wasm pixel type ${itkPolyData.polyDataType.pointPixelType}`);
			return null;
	}
	if (itkPolyData.cellData.length) switch (ITKWASMPixelTypes[itkPolyData.polyDataType.cellPixelType]) {
		case ITKWASMPixelTypes.Scalar:
		...
		case ITKWASMPixelTypes.Offset: break;
		case ITKWASMPixelTypes.Vector:
			if (itkPolyData.polyDataType.pointPixelComponents === 3) cd.setVectors(cd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.Point: break;
		case ITKWASMPixelTypes.CovariantVector:
			if (itkPolyData.polyDataType.pointPixelComponents === 3) cd.setVectors(cd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.SymmetricSecondRankTensor:
			if (itkPolyData.polyDataType.pointPixelComponents === 6) cd.setTensors(cd.getArrayByIndex(0));
			break;
		case ITKWASMPixelTypes.DiffusionTensor3D:
			if (itkPolyData.polyDataType.pointPixelComponents === 6) cd.setTensors(cd.getArrayByIndex(0));
			break;
...
function convertVtkToItkPolyData(polyData, options = {}) {
	...
	if (pd.getNumberOfArrays()) {
		...
		if (pd.getTensors() === pdArray) itkPolyData.polyDataType.pointPixelType = ITKWASMPixelTypes.SymmetricSecondRankTensor;
		else if (pd.getVectors() === pdArray) itkPolyData.polyDataType.pointPixelType = ITKWASMPixelTypes.Vector;
	...
	if (cd.getNumberOfArrays()) {
		...
		if (cd.getTensors() === cdArray) itkPolyData.polyDataType.cellPixelType = ITKWASMPixelTypes.SymmetricSecondRankTensor;
		else if (cd.getVectors() === cdArray) itkPolyData.polyDataType.cellPixelType = ITKWASMPixelTypes.Vector;
		else itkPolyData.polyDataType.cellPixelType = ITKWASMPixelTypes.Scalar;
	}
	return itkPolyData;
}
var ITKHelper_default = {
...
//#endregion
export { convertItkToVtkImage, convertItkToVtkPolyData, convertVtkToItkImage, convertVtkToItkPolyData, ITKHelper_default as default };

//# sourceMappingURL=ITKHelper.js.map