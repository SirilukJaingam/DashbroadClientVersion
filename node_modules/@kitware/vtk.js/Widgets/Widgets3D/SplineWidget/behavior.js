...
//#region Sources/Widgets/Widgets3D/SplineWidget/behavior.js
function widgetBehavior(publicAPI, model) {
	...
	const addPoint = () => {
		if (!model.lastHandle || model.keysDown.Control || !model.freeHand || vec3.squaredDistance(model.moveHandle.getOrigin(), model.lastHandle.getOrigin()) > publicAPI.getFreehandMinDistance() * publicAPI.getFreehandMinDistance()) {
			...
			if (!model.firstHandle) model.firstHandle = model.lastHandle;
	...
	const getHoveredHandle = () => {
		...
		return handles.reduce(({ closestHandle, closestDistance }, handle) => {
	...
	publicAPI.handleRightButtonPress = (e) => {
		...
		if (model.activeState !== model.moveHandle) {
			...
			if (handle) model.widgetState.removeHandle(handle);
	...
	publicAPI.handleLeftButtonPress = (e) => {
		...
		if (!manipulator || !model.activeState || !model.activeState.getActive() || !model.pickable) return macros_default.VOID;
		manipulator.handleEvent(e, model._apiSpecificRenderWindow);
		if (model.activeState === model.moveHandle) {
			if (model.widgetState.getHandleList().length === 0) addPoint();
			else {
	...
	publicAPI.handleLeftButtonRelease = (e) => {
		if (model._isDragging) if (!model.hasFocus) {
			...
			if (!model.draggedPoint) {
				if (vec3.squaredDistance(model.moveHandle.getOrigin(), model.lastHandle.getOrigin()) < model.moveHandle.getScale1() * model.moveHandle.getScale1() || vec3.squaredDistance(model.moveHandle.getOrigin(), model.firstHandle.getOrigin()) < model.moveHandle.getScale1() * model.moveHandle.getScale1()) {
					model.lastHandle.setVisible(true);
					publicAPI.invokeEndInteractionEvent();
					if (publicAPI.getResetAfterPointPlacement()) publicAPI.reset();
					else publicAPI.loseFocus();
				}
			}
			model._interactor.render();
		}
		else if (model.activeState !== model.moveHandle) model.widgetState.deactivate();
		model.freeHand = false;
		model.draggedPoint = false;
		model._isDragging = false;
		return model.hasFocus ? macros_default.EVENT_ABORT : macros_default.VOID;
	};
	publicAPI.handleMouseMove = (callData) => {
		...
		if (hoveredHandle) {
			model.moveHandle.setVisible(false);
			if (hoveredHandle !== model.firstHandle) model._apiSpecificRenderWindow.setCursor("grabbing");
		} else if (!model._isDragging && model.hasFocus) {
			model.moveHandle.setVisible(true);
			model._apiSpecificRenderWindow.setCursor(model.defaultCursor);
		}
		if (model.lastHandle) model.lastHandle.setVisible(true);
		...
		if (worldCoords.length && isHandleMoving) {
			...
			if (curOrigin) model.activeState.setOrigin(add(curOrigin, worldDelta, []));
			else model.activeState.setOrigin(worldCoords);
			if (model._isDragging) model.draggedPoint = true;
			if (model.freeHand && model.activeState === model.moveHandle) addPoint();
			return macros_default.EVENT_ABORT;
		}
		return model.hasFocus ? macros_default.EVENT_ABORT : macros_default.VOID;
	};
	...
	publicAPI.loseFocus = () => {
		if (model.hasFocus) model._interactor.cancelAnimation(publicAPI);
		model.lastHandle?.setVisible(true);
		model.widgetState.deactivate();
		model.moveHandle.deactivate();
		model.moveHandle.setVisible(false);
