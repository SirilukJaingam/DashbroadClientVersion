...
//#region Sources/Rendering/OpenGL/CellArrayBufferObject/index.js
var { vtkErrorMacro } = macros_default;
function shouldApplyCoordShiftAndScale(coordShift, coordScale) {
	if (coordShift === null || coordScale === null) return false;
	return !(vec3.exactEquals(coordShift, [
		0,
		0,
		0
	]) && vec3.exactEquals(coordScale, [
		1,
		1,
		1
	]));
...
function vtkOpenGLCellArrayBufferObject(publicAPI, model) {
	...
	publicAPI.createVBO = (cellArray, inRep, outRep, options, selectionMaps = null) => {
		...
		if (options.customAttributes) options.customAttributes.forEach((a) => {
			if (a) {
		...
		if (options.colors) {
			...
			if (!model.colorBO) model.colorBO = BufferObject_default.newInstance();
			model.colorBO.setOpenGLRenderWindow(model._openGLRenderWindow);
		} else model.colorBO = null;
		...
		const cellBuilders = {
			anythingToPoints(numPoints, cellPts, offset, cellId) {
				for (let i = 0; i < numPoints; ++i) addAPoint(cellPts[offset + i], cellId);
			},
			...
			polysToWireframe(numPoints, cellPts, offset, cellIdx) {
				if (numPoints > 2) for (let i = 0; i < numPoints; ++i) {
					addAPoint(cellPts[offset + i], cellIdx);
		...
		const cellCounters = {
			...
			linesToWireframe(numPoints, cellPts) {
				if (numPoints > 1) return (numPoints - 1) * 2;
				return 0;
			},
			polysToWireframe(numPoints, cellPts) {
				if (numPoints > 2) return numPoints * 2;
				return 0;
			},
			stripsToWireframe(numPoints, cellPts) {
				if (numPoints > 2) return numPoints * 4 - 6;
				return 0;
			},
			polysToSurface(npts, cellPts) {
				if (npts > 2) return (npts - 2) * 3;
				return 0;
			},
		...
		if (colorData) packedUCVBO = new Uint8Array(caboCount * 4);
		let vboidx = 0;
		let ucidx = 0;
		const { useShiftAndScale, coordShift, coordScale } = computeCoordShiftAndScale(options.points);
		if (useShiftAndScale) publicAPI.setCoordShiftAndScale(coordShift, coordScale);
		else if (model.coordShiftAndScaleEnabled === true) publicAPI.setCoordShiftAndScale(null, null);
		if (selectionMaps) if (!selectionMaps.points && !selectionMaps.cells) {
		...
		addAPoint = function addAPointFunc(pointId, cellId) {
			...
			if (normalData !== null) {
				if (options.haveCellNormals) normalIdx = (cellCount + options.cellOffset) * 3;
				else normalIdx = pointId * 3;
			...
			model.customData.forEach((attr) => {
				...
				for (let j = 0; j < attr.components; ++j) packedVBO[vboidx++] = attr.data[custIdx++];
			});
			if (tcoordData !== null) {
				if (options.useTCoordsPerCell) tcoordIdx = cellId * textureComponents;
				else tcoordIdx = pointId * textureComponents;
				for (let j = 0; j < textureComponents; ++j) packedVBO[vboidx++] = tcoordData[tcoordIdx++];
			}
			if (colorData !== null) {
				if (options.haveCellScalars) colorIdx = (cellCount + options.cellOffset) * colorComponents;
				else colorIdx = pointId * colorComponents;
		...
		for (let index = 0; index < size; index += array[index] + 1, cellCount++) func(array[index], array, index + 1, cellCount + options.cellOffset);
	...
	publicAPI.setCoordShiftAndScale = (coordShift, coordScale) => {
		...
		if (model.coordShiftAndScaleEnabled) model.inverseShiftAndScaleMatrix = computeInverseShiftAndScaleMatrix(model.coordShift, model.coordScale);
		else model.inverseShiftAndScaleMatrix = null;
	};
}
var DEFAULT_VALUES = {
	elementCount: 0,
...
function extend(publicAPI, model, initialValues = {}) {
	...
	BufferObject_default.extend(publicAPI, model, initialValues);
...
var CellArrayBufferObject_default = {
	newInstance,
	extend
};
//#endregion
export { CellArrayBufferObject_default as default, extend, newInstance };

//# sourceMappingURL=CellArrayBufferObject.js.map