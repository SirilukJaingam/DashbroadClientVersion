...
//#region Sources/Common/DataModel/TriangleStrip/index.js
...
function vtkTriangleStrip(publicAPI, model) {
	...
	*/
	publicAPI.evaluatePosition = (x, closestPoint, pcoords, dist2, weights) => {
		...
		const numPoints = model.triangle.getPointsIds().length;
		for (let i = 0; i < numPoints; i++) weights[i] = 0;
		for (let i = 0; i < numPoints - 2; i++) {
			...
			points.setData(Float32Array.from([
				...pt0,
				...pt1,
				...pt2
			]), 3);
	...
	*/
	publicAPI.evaluateLocation = (subId, pcoords, x, weights) => {
		...
		for (let i = 0; i < numPoints; i++) weights[i] = 0;
		weights[subId] = 1 - pcoords[0] - pcoords[1];
		...
		for (let i = 0; i < 3; i++) x[i] = pt1[i] * weights[subId] + pt2[i] * weights[subId + 1] + pt3[i] * weights[subId + 2];
	};
	/**
	...
	*/
	publicAPI.triangulate = () => {
		...
		for (let subId = 0; subId < numTris; subId++) {
			const order = subId % 2;
			for (let i = 0; i < 3; i++) model.tris[subId * 3 + i] = subId + idx[order][i];
		}
		return true;
	};
	/**
...
function extend(publicAPI, model, initialValues = {}) {
	...
	if (!model.triangle) model.triangle = Triangle_default.newInstance();
	vtkTriangleStrip(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkTriangleStrip");
var TriangleStrip_default = {
	newInstance,
	extend,
	...STATIC
};
//#endregion
export { STATIC, TriangleStrip_default as default, extend, newInstance };
