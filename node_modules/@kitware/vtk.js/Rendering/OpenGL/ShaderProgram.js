...
//#region Sources/Rendering/OpenGL/ShaderProgram/index.js
...
function vtkShaderProgram(publicAPI, model) {
	...
	publicAPI.cleanup = () => {
		if (model.shaderType === "Unknown" || model.handle === 0) return;
		publicAPI.release();
	...
	publicAPI.bind = () => {
		if (!model.linked && !publicAPI.link()) return false;
	...
	publicAPI.link = () => {
		...
		if (!model.context.getProgramParameter(model.handle, model.context.LINK_STATUS)) {
			vtkErrorMacro(`Error linking shader ${model.context.getProgramInfoLog(model.handle)}`);
			model.handle = 0;
			return false;
		}
	...
	publicAPI.setUniform2f = (name, v1, v2) => {
		...
		if (v2 === void 0) throw new RangeError("Invalid number of values for array");
	...
	publicAPI.setUniform3f = (name, a1, a2, a3) => {
		...
		if (a3 === void 0) throw new RangeError("Invalid number of values for array");
	...
	publicAPI.setUniform3fArray = (name, a) => {
		...
		if (!Array.isArray(a) || a.length !== 3) throw new RangeError("Invalid number of values for array");
	...
	publicAPI.setUniform3i = (name, ...args) => {
		...
		if (array.length !== 3) throw new RangeError("Invalid number of values for array");
	...
	publicAPI.setUniform4f = (name, ...args) => {
		...
		let array = args;
		if (array.length === 1 && Array.isArray(array[0])) array = array[0];
		if (array.length !== 4) throw new RangeError("Invalid number of values for array");
	...
	publicAPI.findUniform = (name) => {
		if (!name || !model.linked) return -1;
		let loc = model.uniformLocs[name];
		if (loc !== void 0) return loc;
	...
	publicAPI.attachShader = (shader) => {
		...
		if (shader.getShaderType() === "Vertex") {
			if (model.vertexShaderHandle !== 0) model.context.detachShader(model.handle, model.vertexShaderHandle);
			model.vertexShaderHandle = shader.getHandle();
		}
		if (shader.getShaderType() === "Fragment") {
			if (model.fragmentShaderHandle !== 0) model.context.detachShader(model.handle, model.fragmentShaderHandle);
	...
	publicAPI.detachShader = (shader) => {
		...
		if (model.handle === 0) model.error = "This shader program has not been initialized yet.";
...
function extend(publicAPI, model, initialValues = {}) {
	...
	model.uniformLocs = {};
	model.vertexShader = Shader_default.newInstance();
	model.vertexShader.setShaderType("Vertex");
	model.fragmentShader = Shader_default.newInstance();
	model.fragmentShader.setShaderType("Fragment");
	model.geometryShader = Shader_default.newInstance();
	model.geometryShader.setShaderType("Geometry");
	macros_default.obj(publicAPI, model);
	macros_default.get(publicAPI, model, ["lastCameraMTime"]);
...
var ShaderProgram_default = {
	newInstance: macros_default.newInstance(extend, "vtkShaderProgram"),
	extend,
	substitute
};
//#endregion
export { ShaderProgram_default as default, substitute };
