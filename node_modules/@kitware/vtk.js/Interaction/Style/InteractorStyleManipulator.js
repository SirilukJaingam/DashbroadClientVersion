...
import InteractorStyle_default from "../../Rendering/Core/InteractorStyle.js";
//#region Sources/Interaction/Style/InteractorStyleManipulator/index.js
var { vtkDebugMacro } = macros_default;
var { States } = InteractorStyle_default;
var START_INTERACTION_EVENT = { type: "StartInteractionEvent" };
var INTERACTION_EVENT = { type: "InteractionEvent" };
var END_INTERACTION_EVENT = { type: "EndInteractionEvent" };
...
function vtkInteractorStyleManipulator(publicAPI, model) {
	...
	const addManipulator = (manipulator, list) => {
		if (list.indexOf(manipulator) !== -1) return false;
	...
	publicAPI.handleButton3D = (ed) => {
		...
		if (manipulator) {
			...
			if (ed.pressed) publicAPI.startCameraPose();
			else {
				model.currentVRManipulators.delete(ed.device);
				if (model.currentVRManipulators.size === 0) publicAPI.endCameraPose();
			}
		} else vtkDebugMacro("No manipulator found");
	};
	publicAPI.handleMove3D = (ed) => {
		const manipulator = model.currentVRManipulators.get(ed.device);
		if (manipulator && model.state === States.IS_CAMERA_POSE) manipulator.onMove3D(publicAPI, model.getRenderer(ed), model.state, ed);
	};
	publicAPI.onButtonDown = (button, callData) => {
		if (model.currentManipulator) return;
		...
		if (model.currentManipulator) {
			if (model.currentManipulator.setCenter) model.currentManipulator.setCenter(model.centerOfRotation);
			if (model.currentManipulator.setRotationFactor) model.currentManipulator.setRotationFactor(model.rotationFactor);
	...
	publicAPI.findMouseManipulator = (button, shift, control, alt) => {
		...
		while (count--) {
			...
			if (manip && manip.getButton() === button && manip.getShift() === shift && manip.getControl() === control && manip.getAlt() === alt && manip.isDragEnabled()) manipulator = manip;
		}
		return manipulator;
	};
	publicAPI.findVRManipulator = (device, input) => {
		...
		while (count--) {
			...
			if (manip && manip.getDevice() === device && manip.getInput() === input) manipulator = manip;
		}
		return manipulator;
	};
	...
	publicAPI.onButtonUp = (button) => {
		...
		if (model.currentManipulator.getButton && model.currentManipulator.getButton() === button) {
			...
			if (!model._interactor.isPointerLocked()) model.currentManipulator = null;
	...
	publicAPI.handleStartMouseWheel = (callData) => {
		if (model.currentWheelManipulator) return;
		let manipulator = null;
		let count = model.mouseManipulators.length;
		while (count--) {
			const manip = model.mouseManipulators[count];
			if (manip && manip.isScrollEnabled() && manip.getShift() === callData.shiftKey && manip.getControl() === callData.controlKey && manip.getAlt() === callData.altKey) manipulator = manip;
		}
		if (manipulator) {
	...
	publicAPI.handlePinch = (callData) => {
		...
		if (actionCount) publicAPI.invokeInteractionEvent(INTERACTION_EVENT);
	};
...
function extend(publicAPI, model, initialValues = {}) {
	...
	InteractorStyle_default.extend(publicAPI, model, initialValues);
	macros_default.setGet(publicAPI, model, ["rotationFactor"]);
	...
	macros_default.setGetArray(publicAPI, model, ["centerOfRotation"], 3);
	vtkInteractorStyleManipulator(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkInteractorStyleManipulator");
var InteractorStyleManipulator_default = {
	newInstance,
	extend,
	...STATIC
};
//#endregion
export { STATIC, InteractorStyleManipulator_default as default, extend, newInstance };

//# sourceMappingURL=InteractorStyleManipulator.js.map