...
//#region Sources/Filters/General/ShrinkPolyData/index.js
	...
	*/
	function shrinkCell(cellPointIds, inPoints, shrinkFactor, newPointsData, outCount) {
		...
		if (numPoints === 1) {
			...
			Polygon_default.computeCentroid(cellPointIds, inPoints, center);
			for (let i = 0; i < 2; i++) {
	...
	function shrinkData(input, output) {
		...
		if (inVerts) {
			...
			for (let i = 0; i < cellSizes.length; i++) numNewPts += cellSizes[i];
		}
		if (inLines) {
			const cellSizes = inLines.getCellSizes();
			for (let i = 0; i < cellSizes.length; i++) numNewPts += (cellSizes[i] - 1) * 2;
		}
		if (inPolys) {
		...
		if (inStrips) {
			...
			for (let i = 0; i < cellSizes.length; i++) numNewPts += (cellSizes[i] - 2) * 3;
		}
		...
		const newVerts = CellArray_default.newInstance();
		...
		if (inVerts) {
			...
			for (let i = 0; i < vertData.length;) {
				...
				for (let j = 1; j <= npts; j++) cellPointIds.push(vertData[i + j]);
		...
		if (inLines) {
			...
			for (let i = 0; i < lineData.length;) {
				...
				for (let j = 0; j < npts - 1; j++) {
					const result = shrinkCell([lineData[i + j + 1], lineData[i + j + 2]], inPoints, shrinkFactor, newPointsData, outCount);
					outCount = result.outCount;
			...
			for (let i = 0; i < stripData.length;) {
				...
				for (let j = 0; j < npts - 2; j++) {
					const result = shrinkCell([
						...
						stripData[i + j + 2],
						stripData[i + j + 3]
					], inPoints, shrinkFactor, newPointsData, outCount);
					outCount = result.outCount;
...
var DEFAULT_VALUES = { shrinkFactor: .5 };
function extend(publicAPI, model, initialValues = {}) {
	Object.assign(model, DEFAULT_VALUES, initialValues);
	macros_default.obj(publicAPI, model);
	macros_default.algo(publicAPI, model, 1, 1);
	macros_default.setGet(publicAPI, model, ["shrinkFactor"]);
	vtkShrinkPolyData(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkShrinkPolyData");
var ShrinkPolyData_default = {
	newInstance,
	extend
};
//#endregion
export { ShrinkPolyData_default as default, extend, newInstance };

//# sourceMappingURL=ShrinkPolyData.js.map