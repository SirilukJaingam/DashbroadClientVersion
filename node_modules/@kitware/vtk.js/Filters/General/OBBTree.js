...
import { add, cross, dot, jacobi, norm, normalize } from "../../Common/Core/Math.js";
...
import OBBNode_default from "./OBBTree/OBBNode.js";
import { getCellTriangles, pushArray } from "./OBBTree/helper.js";
import { mat4, vec4 } from "gl-matrix";
//#region Sources/Filters/General/OBBTree/index.js
...
function vtkOBBTree(publicAPI, model) {
	...
	*/
	function computeOBB(cells, corner, max, mid, min, size) {
		...
		let triMass = 0;
		if (!model.dataset.getCells()) model.dataset.buildCells();
		for (let i = 0; i < numCells; i++) {
			...
			for (let j = 0; j < numPts - 2; j++) {
				...
				triMass = .5 * norm(cross(dp0, dp1, []));
				totMass += triMass;
				for (let k = 0; k < 3; k++) mean[k] += triMass * c[k];
			...
			for (let j = 0; j < numPts; j++) if (model.insertedPoints[ptIds[j]] !== model.OBBCount) {
		...
		for (let i = 0; i < 3; i++) mean[i] /= totMass;
		a1[0] = a0[1];
		a2[0] = a0[2];
		a2[1] = a1[2];
		a = [
		...
		for (let i = 0; i < 3; i++) for (let j = 0; j < 3; j++) a[i * 3 + j] = a[i * 3 + j] / totMass - mean[i] * mean[j];
		const v = [
			0,
			0,
			0,
		...
		for (let ptId = 0; ptId < numPts; ptId++) {
			...
			for (let i = 0; i < 3; i++) {
				const out = Line_default.distanceToLine(p, mean, a.slice(3 * i, 3 * (i + 1)), []);
				if (out.t < tMin[i]) tMin[i] = out.t;
				if (out.t > tMax[i]) tMax[i] = out.t;
			}
		}
		for (let i = 0; i < 3; i++) {
	...
	*/
	function buildTree(cells, obbNode, level) {
		const numCells = cells.length;
		if (level > model.level) model.level = level;
		const axes = obbNode.getAxes();
		const corner = obbNode.getCorner();
		computeOBB(cells, corner, axes[0], axes[1], axes[2], [
			0,
			0,
			0
		]);
		...
		if (level < model.maxLevel && numCells > model.numberOfCellsPerNode) {
			...
			for (let i = 0; i < 3; i++) p[i] = corner[i] + axes[0][i] / 2 + axes[1][i] / 2 + axes[2][i] / 2;
			let splitPlane = 0;
			...
			for (; !splitAcceptable && splitPlane < 3;) {
				for (let i = 0; i < 3; i++) n[i] = axes[splitPlane][i];
				normalize(n);
				for (let i = 0; i < numCells; i++) {
					const cellId = cells[i];
					...
					if (negative && positive) {
						...
						c[1] /= numPts;
						c[2] /= numPts;
						if (n[0] * (c[0] - p[0]) + n[1] * (c[1] - p[1]) + n[2] * (c[2] - p[2]) < 0) LHlist.push(cellId);
						else RHlist.push(cellId);
					} else if (negative) LHlist.push(cellId);
					else RHlist.push(cellId);
				}
				const numInLHnode = LHlist.length;
				const numInRHnode = RHlist.length;
				const ratio = Math.abs((numInRHnode - numInLHnode) / numCells);
				if (ratio < .6 || foundBestSplit) splitAcceptable = 1;
				else {
					LHlist = [];
					RHlist = [];
					if (ratio < bestRatio) {
			...
			if (splitAcceptable) {
				const LHnode = OBBNode_default.newInstance();
				const RHnode = OBBNode_default.newInstance();
		...
		if (cells && model.retainCellLists) obbNode.setCells(cells);
	...
	function generatePolygons(obbNode, level, repLevel, points, cells) {
		if (level === repLevel || repLevel < 0 && obbNode.getKids()) {
			...
			cubeIds[3] = nbPoints++;
			newPoints.push(...add(obbNode.getCorner(), obbNode.getAxis(2), []));
			cubeIds[4] = nbPoints++;
			newPoints.push(...add(x, obbNode.getAxis(2), []));
			cubeIds[5] = nbPoints++;
			newPoints.push(...add(y, obbNode.getAxis(2), []));
			cubeIds[6] = nbPoints++;
			newPoints.push(...add(xy, obbNode.getAxis(2), []));
			cubeIds[7] = nbPoints++;
	...
	*/
	publicAPI.transform = (transform) => {
		...
		matrix[14] = 0;
		matrix[15] = 1;
		const transformVector = MatrixBuilder_default.buildFromRadian().setMatrix(matrix);
	...
	*/
	publicAPI.deepCopy = (tree) => {
		...
		if (root) {
			model.tree = OBBNode_default.newInstance();
	...
	*/
	publicAPI.computeOBBFromDataset = (input, corner, max, mid, min, size) => {
		...
		model.pointsList = [];
		computeOBB(Array.from({ length: numCells }, (_, i) => i), corner, max, mid, min, size);
	};
	/**
	...
	*/
	publicAPI.disjointOBBNodes = (nodeA, nodeB, XformBtoA) => {
		...
		const pA = nodeA;
		let pB = OBBNode_default.newInstance();
		const dotAB = [
			0,
			0,
			0,
		...
		for (let ii = 0; ii < 3; ii++) {
			centerA[ii] = pA.getCorner()[ii] + .5 * (pA.getAxis(0)[ii] + pA.getAxis(1)[ii] + pA.getAxis(2)[ii]);
			centerB[ii] = pB.getCorner()[ii] + .5 * (pB.getAxis(0)[ii] + pB.getAxis(1)[ii] + pB.getAxis(2)[ii]);
		...
		for (let ii = 0; ii < 3; ii++) {
			...
			if (dotA > 0) rangeAmax += dotA;
			else rangeAmin += dotA;
			dotB = dot(pB.getAxis(ii), AtoB);
			if (dotB > 0) rangeBmax += dotB;
			else rangeBmin += dotB;
		}
		if (rangeAmax + eps < rangeBmin || rangeBmax + eps < rangeAmin) return 1;
		for (let ii = 0; ii < 3; ii++) {
			...
			if (rangeAmax + eps < rangeBmin || rangeBmax + eps < rangeAmin) return 2;
		}
		for (let ii = 0; ii < 3; ii++) {
			...
			if (rangeAmax + eps < rangeBmin || rangeBmax + eps < rangeAmin) return 3;
		}
		for (let ii = 0; ii < 3; ii++) for (let jj = 0; jj < 3; jj++) {
			...
			if (rangeAmax + eps < rangeBmin || rangeBmax + eps < rangeAmin) return 4;
		}
		return 0;
	};
	/**
	...
	*/
	publicAPI.intersectWithOBBTree = (obbTreeB, XformBtoA, onIntersect = () => -1) => {
		...
		while (depth > 0 && returnValue > -1) {
			...
			if (!publicAPI.disjointOBBNodes(nodeA, nodeB, XformBtoA)) if (!nodeA.getKids()) if (!nodeB.getKids()) {
	...
	publicAPI.triangleIntersectsNode = (nodeA, p0, p1, p2, XformBtoA) => {
		...
		for (let ii = 0; ii < 3; ii++) {
			...
			if (dotB > rangeBmax) rangeBmax = dotB;
			else rangeBmin = dotB;
			dotB = dot(pB[2], pA.getAxis(ii));
			if (dotB > rangeBmax) rangeBmax = dotB;
			else if (dotB < rangeBmin) rangeBmin = dotB;
			if (rangeAmax + eps < rangeBmin || rangeBmax + eps < rangeAmin) return 0;
		}
		const AtoB = [];
		let dotB;
	...
	*/
	publicAPI.findTriangleIntersections = (info, node0, node1, transform) => {
		...
		for (let id0 = 0; id0 < numCells0; id0++) {
			...
			if (mesh0.getCellType(cellId0) === CellType.VTK_TRIANGLE) {
				...
				for (let id = 0; id < triPtIds0.length; id++) mesh0.getPoints().getPoint(triPtIds0[id], triPts0[id]);
				if (info.obbTree1.triangleIntersectsNode(node1, triPts0[0], triPts0[1], triPts0[2], transform)) {
					...
					for (let id1 = 0; id1 < numCells1; id1++) {
						...
						if (mesh1.getCellType(cellId1) === CellType.VTK_TRIANGLE) {
							...
							for (let id = 0; id < triPtIds1.length; id++) mesh1.getPoints().getPoint(triPtIds1[id], triPts1[id]);
							const { intersect, coplanar, pt1: outpt0, pt2: outpt1 } = Triangle_default.intersectWithTriangle(...triPts0, ...triPts1, model.tolerance);
	...
	*/
	publicAPI.generateRepresentation = (level) => {
		...
		const output = PolyData_default.newInstance();
...
var newInstance = macros_default.newInstance(extend, "vtkOBBTree");
var OBBTree_default = {
	newInstance,
	extend
};
//#endregion
export { OBBTree_default as default, extend, newInstance };

//# sourceMappingURL=OBBTree.js.map