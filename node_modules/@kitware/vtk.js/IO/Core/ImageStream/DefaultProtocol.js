//#region Sources/IO/Core/ImageStream/DefaultProtocol.js
function createMethods(session) {
	return {
		...
		setQuality: (viewId, quality, ratio = 1) => session.call("viewport.image.push.quality", [
			viewId,
			quality,
			ratio
		]),
		setSize: (viewId, width = 400, height = 400) => session.call("viewport.image.push.original.size", [
			viewId,
			width,
			height
		]),
		...
		updateCamera: (viewId, focalPoint, viewUp, position, forceUpdate = true) => session.call("viewport.camera.update", [
			viewId ?? -1,
			focalPoint,
			viewUp,
			position,
		...
		updateCameraParameters: (viewId = -1, parameters = {}, forceUpdate = true) => session.call("viewport.camera.update.params", [
			viewId,
			parameters,
			forceUpdate
		])
...
//# sourceMappingURL=DefaultProtocol.js.map