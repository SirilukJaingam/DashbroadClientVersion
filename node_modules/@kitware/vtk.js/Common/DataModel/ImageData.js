...
import { clampVector, roundVector } from "../Core/Math.js";
...
import StructuredData_default from "./StructuredData.js";
...
function vtkImageData(publicAPI, model) {
	...
	publicAPI.setExtent = (...inExtent) => {
		...
		if (changeDetected) {
			...
			model.dataDescription = StructuredData_default.getDataDescriptionFromExtent(model.extent);
			publicAPI.modified();
		}
	...
	publicAPI.getPoint = (index) => {
		...
		switch (model.dataDescription) {
			case StructuredType.EMPTY: return null;
	...
	publicAPI.extentToBounds = (ex) => BoundingBox_default.transformBounds(ex, model.indexToWorld);
	publicAPI.getSpatialExtent = () => BoundingBox_default.inflate([...model.extent], .5);
	...
	publicAPI.indexToWorldBounds = (bin, bout = []) => BoundingBox_default.transformBounds(bin, model.indexToWorld, bout);
	publicAPI.worldToIndexBounds = (bin, bout = []) => BoundingBox_default.transformBounds(bin, model.worldToIndex, bout);
	...
	publicAPI.computeTransforms();
	publicAPI.getCenter = () => BoundingBox_default.getCenter(publicAPI.getBounds());
	publicAPI.computeHistogram = (worldBounds, voxelFunction = null) => {
		...
		BoundingBox_default.computeCornerPoints(bounds, point1, point2);
...
function extend(publicAPI, model, initialValues = {}) {
	...
	if (!model.direction) model.direction = mat3.identity(new Float64Array(9));
	else if (Array.isArray(model.direction)) model.direction = new Float64Array(model.direction.slice(0, 9));
	model.indexToWorld = new Float64Array(16);
	model.worldToIndex = new Float64Array(16);
	macros_default.get(publicAPI, model, ["indexToWorld", "worldToIndex"]);
	macros_default.setGetArray(publicAPI, model, ["origin", "spacing"], 3);
	macros_default.setGetArray(publicAPI, model, ["direction"], 9);
	macros_default.getArray(publicAPI, model, ["extent"], 6);
	vtkImageData(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkImageData");
var ImageData_default = {
	newInstance,
	extend
};
//#endregion
export { ImageData_default as default, extend, newInstance };
