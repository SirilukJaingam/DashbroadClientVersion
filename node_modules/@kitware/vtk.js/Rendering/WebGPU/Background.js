...
//#region Sources/Rendering/WebGPU/Background/index.js
var solidFragTemplate = `
//VTK::Renderer::Dec
...
`;
var _fsqMat4 = new Float64Array(16);
...
function vtkWebGPUBackground(publicAPI, model) {
	...
	publicAPI.getMode = (renderer) => {
		if (renderer.getTexturedBackground?.() && renderer.getBackgroundTexture?.().getImageLoaded?.()) return {
			mode: "texture",
			texture: renderer.getBackgroundTexture(),
			textureName: "BackgroundTexture",
			pipelineHash: "backgroundTexture"
		};
		if (renderer.getUseEnvironmentTextureAsBackground?.() && renderer.getEnvironmentTexture?.().getImageLoaded?.()) return {
			mode: "environment",
		...
		const background = renderer.getBackgroundByReference?.() ?? [
			0,
			0,
			0
		];
		const background2 = renderer.getBackground2ByReference?.() ?? [
			0,
			0,
			0
		];
		if (isGradientBackground && !areEquals(background, background2)) return {
			mode: "gradient",
			texture: null,
			textureName: null,
	...
	publicAPI.getFragmentTemplate = (mode) => {
		switch (mode) {
			case "gradient": return gradientFragTemplate;
			case "texture": return textureFragTemplate;
			case "environment": return environmentFragTemplate;
			default: return solidFragTemplate;
		}
	};
	publicAPI.updateTexture = (device, texture, textureName) => {
		...
		const webgpuTexture = device.getTextureManager().getTextureForVTKTexture(texture, textureName);
	...
	publicAPI.render = (renderEncoder, rendererNode) => {
		...
		publicAPI.ensureQuad(device);
		model.webgpuCamera = rendererNode.getViewNodeFor(renderer.getActiveCamera(), model.webgpuCamera);
		const { mode, texture, textureName, pipelineHash } = publicAPI.getMode(renderer);
...
var newInstance = macros_default.newInstance(extend, "vtkWebGPUBackground");
var Background_default = {
	newInstance,
	extend
};
//#endregion
export { Background_default as default, extend, newInstance };
