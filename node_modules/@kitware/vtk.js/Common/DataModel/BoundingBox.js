...
function transformBounds(bounds, transform, out = []) {
	...
	for (let i = 0; i < corners.length; ++i) vec3.transformMat4(corners[i], corners[i], transform);
	reset(out);
...
function intersectPlane(bounds, origin, normal) {
	...
	for (let z = 4; z <= 5; ++z) {
		...
		for (let y = 2; y <= 3; ++y) {
			...
			for (let x = 0; x <= 1; ++x) {
				p[0] = bounds[x];
				d = Plane_default.evaluate(normal, origin, p);
				if (firstOne) {
					sign = d >= 0 ? 1 : -1;
					firstOne = 0;
				}
...
function cutWithPlane(bounds, origin, normal) {
	...
	for (let ix = 0; ix < 2; ix++) for (let iy = 2; iy < 4; iy++) for (let iz = 4; iz < 6; iz++) {
		...
		d[idx++] = Plane_default.evaluate(normal, origin, x);
	}
	let dir = 2;
