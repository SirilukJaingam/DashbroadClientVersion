...
//#region Sources/Rendering/WebGPU/Skybox/index.js
var { VtkDataTypes } = DataArray_default;
var { vtkErrorMacro } = macros_default;
var backgroundFragTemplate = `
//VTK::Renderer::Dec
...
`;
var _imcpc = new Float64Array(16);
function getTextureFormat(dataArray) {
	const numComp = dataArray.getNumberOfComponents();
	let format = "rgba";
	if (numComp === 1) format = "r";
	else if (numComp === 2) format = "rg";
	switch (dataArray.getDataType()) {
		case VtkDataTypes.UNSIGNED_CHAR: return `${format}8unorm`;
		...
		default: return `${format}16float`;
	}
}
function flipTextureRowsY(dataArray, width, height, numComponents) {
	const flipped = macros_default.newTypedArray(dataArray.constructor.name, dataArray.length);
...
function vtkWebGPUSkybox(publicAPI, model) {
	...
	function getCachedTextureView(device, texture, webgpuTexture, format) {
		...
		if (format === "background") {
			if (model.backgroundTextureView && model.backgroundWebGPUTexture === webgpuTexture && model.backgroundInterpolate === interpolate) return model.backgroundTextureView;
		...
		if (model.boxTextureView && model.boxWebGPUTexture === webgpuTexture && model.boxInterpolate === interpolate) return model.boxTextureView;
		const tview = webgpuTexture.createView("sbtexture", { dimension: "cube" });
		tview.addSampler(device, {
	...
	publicAPI.buildPass = (prepass) => {
		if (prepass) {
			...
			if (model.WebGPURenderer) {
				...
				model.webgpuCamera = model.WebGPURenderer.getViewNodeFor(renderer.getActiveCamera(), model.webgpuCamera);
			}
		}
	};
	...
	publicAPI.ensureQuad = (device) => {
		if (model.quad) return;
		model.quad = FullScreenQuad_default.newInstance();
		...
		model.quad.getShaderReplacements().set("replaceShaderPosition", replaceSkyboxShaderPosition);
	};
	...
	publicAPI.getCubeTextureHash = (texture) => {
		...
		for (let i = 0; i < 6; i++) {
			...
			if (scalars) hash += `-${scalars.getMTime()}`;
		}
		return hash;
	};
	publicAPI.getCubeTexture = (device, texture) => {
		...
		return device.getCachedObject(hash, () => {
			...
			const webgpuTexture = Texture_default.newInstance({ label: "sbtexture" });
			...
			for (let i = 0; i < 6; i++) {
				...
				const scalars = imageData.getPointData().getScalars();
				const faceData = flipTextureRowsY(scalars.getData(), faceDims[0], faceDims[1], scalars.getNumberOfComponents());
	...
	publicAPI.updateTexture = (device) => {
		...
		if (model.renderable.getFormat() === "background") {
			const webgpuTexture = device.getTextureManager().getTextureForVTKTexture(texture, "sbtexture");
			if (!webgpuTexture.getReady()) {
				model.quad.setTextureViews([]);
				return false;
			}
			const tview = getCachedTextureView(device, texture, webgpuTexture, "background");
		...
		vtkErrorMacro(`Unsupported vtkSkybox format ${model.renderable.getFormat()}`);
		model.quad.setTextureViews([]);
		return false;
	};
	publicAPI.updateUBO = (device) => {
		...
		const stabilizedCenter = model.WebGPURenderer.getStabilizedCenterByReference();
	...
	publicAPI.opaquePass = (prepass) => {
		if (!prepass || !model.renderable || !model.renderable.getVisibility() || model.WebGPURenderer?.getSelector()) return;
		...
		if (model.lastFormat !== format) {
			...
			model.quad.setFragmentShaderTemplate(format === "background" ? backgroundFragTemplate : boxFragTemplate);
			model.lastFormat = format;
		}
		if (!publicAPI.updateTexture(device)) return;
		publicAPI.updateUBO(device);
...
var newInstance = macros_default.newInstance(extend, "vtkWebGPUSkybox");
var Skybox_default = {
	newInstance,
	extend
};
