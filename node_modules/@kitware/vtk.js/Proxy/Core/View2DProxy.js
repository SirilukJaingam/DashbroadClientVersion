...
import { cross, getMajorAxisIndex, radiansFromDegrees } from "../../Common/Core/Math.js";
import BoundingBox_default from "../../Common/DataModel/BoundingBox.js";
import MouseRangeManipulator_default from "../../Interaction/Manipulators/MouseRangeManipulator.js";
import ViewProxy_default from "./ViewProxy.js";
import { mat4, vec3 } from "gl-matrix";
//#region Sources/Proxy/Core/View2DProxy/index.js
var DEFAULT_STEP_WIDTH = 512;
function formatAnnotationValue(value) {
	if (Array.isArray(value)) return value.map(formatAnnotationValue).join(", ");
	if (Number.isInteger(value)) return value;
	if (Number.isFinite(value)) {
		if (Math.abs(value) < .01) return "0";
...
function getPropCoarseHull(prop) {
	...
	if (!BoundingBox_default.isValid(finestBounds)) return [];
	const corners = [];
	BoundingBox_default.getCorners(finestBounds, corners);
	if (finestMatrix) corners.forEach((pt) => vec3.transformMat4(pt, pt, finestMatrix));
	return corners;
}
function vtkView2DProxy(publicAPI, model) {
	...
	publicAPI.updateWidthHeightAnnotation = () => {
		...
		if (ijkOrientation && dimensions) {
			...
			if (dimensions.length > 3) realDimensions = dimensions.split(",").map(Number);
	...
	publicAPI.updateOrientation = (axisIndex, orientation, viewUp) => {
		...
		while (count--) {
			...
			if (rep.setSlicingMode) rep.setSlicingMode(slicingMode);
		}
	...
	publicAPI.addRepresentation = (rep) => {
		...
		if (rep.setSlicingMode) rep.setSlicingMode("XYZ"[model.axis]);
	...
	publicAPI.removeRepresentation = (rep) => {
		...
		if (rep === model.sliceRepresentation) {
			...
			let count = model.representations.length;
			while (count--) if (publicAPI.bindRepresentationToManipulator(model.representations[count])) count = 0;
		}
	};
	...
	*/
	model._resetCamera = (bounds = null) => {
		...
		if (!model.fitProps || !model.useParallelRendering || !initialReset) return initialReset;
		const visiblePoints = [];
		if (bounds) BoundingBox_default.getCorners(bounds, visiblePoints);
		else publicAPI.getRepresentations().forEach((representationProxy) => [representationProxy.getActors(), representationProxy.getVolumes()].flat().forEach((prop) => visiblePoints.push(...getPropCoarseHull(prop))));
		if (!visiblePoints) return initialReset;
		const viewBounds = BoundingBox_default.reset([]);
		const viewMatrix = model.camera.getViewMatrix();
		mat4.transpose(viewMatrix, viewMatrix);
		for (let i = 0; i < visiblePoints.length; ++i) {
			const point = visiblePoints[i];
			vec3.transformMat4(point, point, viewMatrix);
			BoundingBox_default.addPoint(viewBounds, ...point);
		}
		const dims = model.renderer.getRenderWindow().getViews()[0].getViewportSize(model.renderer);
		const aspect = dims[1] && dims[0] ? dims[0] / dims[1] : 1;
		const xLength = BoundingBox_default.getLength(viewBounds, 0);
		const yLength = BoundingBox_default.getLength(viewBounds, 1);
		const parallelScale = .5 * Math.max(yLength, xLength / aspect);
		const viewFocalPoint = BoundingBox_default.getCenter(viewBounds);
		const perspectiveAngle = radiansFromDegrees(model.camera.getViewAngle());
		const distance = parallelScale / Math.tan(perspectiveAngle * .5);
		const viewPosition = [
			viewFocalPoint[0],
			viewFocalPoint[1],
		...
		const worldBounds = BoundingBox_default.transformBounds(viewBounds, inverseViewMatrix);
	...
	model.rangeManipulator = MouseRangeManipulator_default.newInstance({
		button: 1,
		scrollEnabled: true
	});
	...
	function setWindowWidth(windowWidth) {
		...
		if (model.sliceRepresentation && model.sliceRepresentation.setWindowWidth) model.sliceRepresentation.setWindowWidth(windowWidth);
	}
	function setWindowLevel(windowLevel) {
		publicAPI.updateCornerAnnotation({ windowLevel });
		if (model.sliceRepresentation && model.sliceRepresentation.setWindowLevel) model.sliceRepresentation.setWindowLevel(windowLevel);
	}
	function setSlice(sliceRaw) {
		const numberSliceRaw = Number(sliceRaw);
		const annotation = { slice: Number.isInteger(numberSliceRaw) ? sliceRaw : numberSliceRaw.toFixed(2) };
		if (model.sliceRepresentation && model.sliceRepresentation.setSlice) model.sliceRepresentation.setSlice(numberSliceRaw);
	...
	publicAPI.bindRepresentationToManipulator = (representation) => {
		let nbListeners = 0;
		if (!representation.getProxyId) return nbListeners;
		model.rangeManipulator.removeAllListeners();
		model.sliceRepresentation = representation;
		while (model.sliceRepresentationSubscriptions.length) model.sliceRepresentationSubscriptions.pop().unsubscribe();
		if (representation) {
			...
			if (representation.getWindowWidth) {
				...
				if (!step || step === "any") step = 1 / DEFAULT_STEP_WIDTH;
...
function extend(publicAPI, model, initialValues = {}) {
	...
	ViewProxy_default.extend(publicAPI, model, initialValues);
	macros_default.get(publicAPI, model, ["axis"]);
	macros_default.setGet(publicAPI, model, ["fitProps"]);
	vtkView2DProxy(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkView2DProxy");
var View2DProxy_default = {
	newInstance,
	extend
};
//#endregion
export { View2DProxy_default as default, extend, newInstance };

//# sourceMappingURL=View2DProxy.js.map