...
import Constants_default from "./WidgetManager/Constants.js";
import { WIDGET_PRIORITY } from "./AbstractWidget/Constants.js";
//#region Sources/Widgets/Core/WidgetManager/index.js
var { ViewTypes, RenderingTypes, CaptureOn } = Constants_default;
...
function getPixelWorldHeightAtCoord(worldCoord, displayScaleParams) {
	const { dispHeightFactor, cameraPosition, cameraDir, isParallel, rendererPixelDims } = displayScaleParams;
	let scale = 1;
	if (isParallel) scale = dispHeightFactor;
	else {
		const worldCoordToCamera = [...worldCoord];
		Math_default.subtract(worldCoordToCamera, cameraPosition, worldCoordToCamera);
		scale = Math_default.dot(worldCoordToCamera, cameraDir) * dispHeightFactor;
	}
	const rHeight = rendererPixelDims[1];
	return scale / rHeight;
}
function vtkWidgetManager(publicAPI, model) {
	if (!model.viewId) model.viewId = `view-${viewIdCount++}`;
	...
	function getViewWidget(widget) {
		return widget && (widget.isA("vtkAbstractWidget") ? widget : widget.getWidgetForView({ viewId: model.viewId }));
	}
	function updateDisplayScaleParams() {
		...
		if (_renderer && _apiSpecificRenderWindow && _camera) {
			...
			const dispHeightFactor = isParallel ? 2 * _camera.getParallelScale() : 2 * Math.tan(Math_default.radiansFromDegrees(_camera.getViewAngle()) / 2);
			model.widgets.forEach((w) => {
				w.getNestedProps().forEach((r) => {
					if (r.getScaleInPixels()) r.setDisplayScaleParams({
						dispHeightFactor,
						cameraPosition,
						cameraDir,
						isParallel,
	...
	async function updateSelection(callData, fromTouchEvent, callID) {
		...
		const { requestCount, selectedState, representation, widget } = await publicAPI.getSelectedDataForXY(position.x, position.y);
		if (requestCount || callID !== model._currentUpdateSelectionCallID) return;
		function activateHandle(w) {
			if (fromTouchEvent) model._interactor.invokeLeftButtonRelease(callData);
			w.activateHandle({
				selectedState,
				representation
			});
			if (fromTouchEvent) model._interactor.invokeLeftButtonPress(callData);
		}
		const cursor = publicAPI.getCursorStyles()[widget ? "hover" : "default"];
		if (cursor) model._apiSpecificRenderWindow.setCursor(cursor);
		...
		if (model.widgetInFocus === widget && widget.hasFocus()) {
			...
			wantRender = true;
		} else for (let i = 0; i < model.widgets.length; i++) {
	...
	publicAPI.setRenderer = (renderer) => {
		...
		macros_default.moveToProtected({}, model, Object.keys(renderingComponents));
		...
		subscriptions.push(model._interactor.onLeftButtonPress((eventData) => {
			...
			if (touchEvent) handleEvent(eventData, touchEvent);
			return macros_default.VOID;
		}, WIDGET_PRIORITY / 2));
		publicAPI.modified();
		if (model.pickingEnabled) publicAPI.enablePicking();
	};
	...
	publicAPI.removeWidget = (widget) => {
		...
		if (index !== -1) {
			...
			if (model.widgetInFocus === viewWidget) publicAPI.releaseFocus();
	...
	publicAPI.getSelectedDataForXY = async (x, y) => {
		...
		if (model.pickingEnabled) {
			if (!model._capturedBuffers || model.captureOn === CaptureOn.MOUSE_MOVE) await captureBuffers(x, y, x, y);
			else {
				const capturedRegion = model._capturedBuffers.area;
				if (x < capturedRegion[0] || x > capturedRegion[2] || y < capturedRegion[1] || y > capturedRegion[3]) await captureBuffers(x, y, x, y);
			}
	...
	publicAPI.grabFocus = (widget) => {
		const viewWidget = getViewWidget(widget);
		if (model.widgetInFocus && model.widgetInFocus !== viewWidget) model.widgetInFocus.loseFocus();
		model.widgetInFocus = viewWidget;
		if (model.widgetInFocus) model.widgetInFocus.grabFocus();
	};
...
var defaultValues = (initialValues = {}) => ({
	...
	...initialValues,
	cursorStyles: initialValues.cursorStyles ? { ...initialValues.cursorStyles } : {
		default: "default",
		hover: "pointer"
	}
});
...
var newInstance = macros_default.newInstance(extend, "vtkWidgetManager");
var WidgetManager_default = {
	newInstance,
	extend,
...
//#endregion
export { WidgetManager_default as default, extend, extractRenderingComponents, getPixelWorldHeightAtCoord, newInstance };
