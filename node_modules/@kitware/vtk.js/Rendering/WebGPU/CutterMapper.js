...
import { boundsToMaxPoint, boundsToMinPoint, shiftVec3ToVec4, vec3ToVec4 } from "./CutterMapper/helpers.js";
//#region Sources/Rendering/WebGPU/CutterMapper/index.js
...
function vtkWebGPUCutterCellArrayMapper(publicAPI, model) {
	...
	publicAPI.replaceShaderPosition = (hash, pipeline, vertexInput) => {
		...
		let positionImpl = [];
		if (model.useRendererMatrix) positionImpl = ["    var pCoord: vec4<f32> = rendererUBO.SCPCMatrix*mapperUBO.BCSCMatrix*vertexBC;", "    output.vertexVC = rendererUBO.SCVCMatrix * mapperUBO.BCSCMatrix * vec4<f32>(vertexBC.xyz, 1.0);"];
		else positionImpl = [
		...
		switch (functionName) {
			case "vtkPlane":
				cutDistanceImpl = ["    output.cutDistanceVS = dot(mapperUBO.CutPlaneNormal.xyz, vertexBC.xyz - mapperUBO.CutPlaneOrigin.xyz) - mapperUBO.CutValue;"];
				break;
			case "vtkSphere":
				cutDistanceImpl = [
		...
		if (model.forceZValue) code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", [
			model.useRendererMatrix ? "pCoord = vec4<f32>(pCoord.xyz/pCoord.w, 1.0);" : "    pCoord = vec4<f32>(pCoord.xyz/pCoord.w, 1.0);",
		...
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", ["    output.Position = pCoord;"]).result;
		vDesc.setCode(code);
		const fDesc = pipeline.getShaderDescription("fragment");
		code = fDesc.getCode();
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", ["  if (abs(input.cutDistanceVS) > max(fwidth(input.cutDistanceVS) * mapperUBO.CutWidth, 1.0e-6)) { discard; }", "//VTK::Position::Impl"]).result;
	...
	publicAPI.computePipelineHash = () => {
		...
		const functionName = model.renderable.getSupportedImplicitFunctionName() || "none";
	...
	publicAPI.updateUBO = () => {
		...
		if (!functionName || !cutFunction) return;
		const bufferShift = model.WebGPUActor.getBufferShift(model.WebGPURenderer);
		model.UBO.setValue("CutWidth", Math.max(renderable.getCutWidth(), 0));
		switch (functionName) {
			case "vtkPlane":
				model.UBO.setArray("CutPlaneOrigin", shiftVec3ToVec4(model.cutPlaneOrigin, cutFunction.getOrigin(), bufferShift));
				model.UBO.setArray("CutPlaneNormal", vec3ToVec4(model.cutPlaneNormal, cutFunction.getNormal()));
			...
			case "vtkSphere": {
				...
				model.UBO.setArray("CutSphereCenter", shiftVec3ToVec4(model.cutSphereCenter, cutFunction.getCenter(), bufferShift));
				if (usesAxisRadii) {
			...
			case "vtkBox": {
				...
				model.UBO.setArray("CutBoxMinPoint", boundsToMinPoint(model.cutBoxMinPoint, bounds, bufferShift));
				model.UBO.setArray("CutBoxMaxPoint", boundsToMaxPoint(model.cutBoxMaxPoint, bounds, bufferShift));
				break;
			}
			case "vtkCylinder":
				model.UBO.setArray("CutCylinderCenter", shiftVec3ToVec4(model.cutCylinderCenter, cutFunction.getCenter(), bufferShift));
				model.UBO.setArray("CutCylinderAxis", vec3ToVec4(model.cutCylinderAxis, cutFunction.getAxis()));
			...
			case "vtkCone": {
				...
				if (inverseTransform?.transformPoint) {
					...
					Math_default.subtract(axisPoint, apex, axis);
					if (Math_default.norm(axis) > 0) Math_default.normalize(axis);
				} else if (cutFunction.getTransform?.()) vtkErrorMacro("Failed to retrieve inverse transform for cone cut function");
				model.UBO.setArray("CutConeApex", shiftVec3ToVec4(model.cutConeApex4, apex, bufferShift));
...
function extendCellArray(publicAPI, model, initialValues = {}) {
	CellArrayMapper_default.extend(publicAPI, model, initialValues);
	...
	model.cutSphereRadius = [
		0,
		0,
		0,
		0
	];
	model.cutBoxMinPoint = [
		0,
		0,
		0,
		0
	];
	model.cutBoxMaxPoint = [
		0,
		0,
		0,
		0
...
var newCellArrayInstance = macros_default.newInstance(extendCellArray, "vtkWebGPUCutterCellArrayMapper");
...
var newInstance = macros_default.newInstance(extend, "vtkWebGPUCutterMapper");
