...
//#region Sources/Filters/Sources/Cursor3D/index.js
function vtkCursor3D(publicAPI, model) {
	...
	publicAPI.setModelBounds = (bounds) => {
		...
		if (model.modelBounds[0] === bounds[0] && model.modelBounds[1] === bounds[1] && model.modelBounds[2] === bounds[2] && model.modelBounds[3] === bounds[3] && model.modelBounds[4] === bounds[4] && model.modelBounds[5] === bounds[5]) return;
		publicAPI.modified();
		model.modelBounds = bounds.map((v) => Number(v));
		for (let i = 0; i < 3; ++i) model.modelBounds[2 * i] = Math.min(model.modelBounds[2 * i], model.modelBounds[2 * i + 1]);
	};
	publicAPI.setFocalPoint = (points) => {
		if (!Array.isArray(points) || points.length < 3) return;
		if (points[0] === model.focalPoint[0] && points[1] === model.focalPoint[1] && points[2] === model.focalPoint[2]) return;
		publicAPI.modified();
		const v = [];
		for (let i = 0; i < 3; i++) {
			...
			} else if (model.wrap) model.focalPoint[i] = model.modelBounds[2 * i] + (model.focalPoint[i] - model.modelBounds[2 * i]) * 1 % ((model.modelBounds[2 * i + 1] - model.modelBounds[2 * i]) * 1);
			else {
				if (points[i] < model.modelBounds[2 * i]) model.focalPoint[i] = model.modelBounds[2 * i];
				if (points[i] > model.modelBounds[2 * i + 1]) model.focalPoint[i] = model.modelBounds[2 * i + 1];
			}
		}
	};
	...
	publicAPI.requestData = (inData, outData) => {
		let numPts = 0;
		let numLines = 0;
		if (model.wrap) for (let i = 0; i < model.focalPoint.length; ++i) model.focalPoint[i] = model.modelBounds[2 * i] + (model.focalPoint[i] - model.modelBounds[2 * i]) * 1 % (model.modelBounds[2 * i + 1] - model.modelBounds[2 * i]);
		else for (let i = 0; i < model.focalPoint.length; ++i) {
		...
		const newPts = Points_default.newInstance({ size: numPts * 3 });
		const newLines = CellArray_default.newInstance({ size: numLines * 3 });
		let pid = 0;
		let cid = 0;
		if (model.axes) {
		...
		if (model.xShadows) for (let i = 0; i < 2; ++i) {
		...
		if (model.yShadows) for (let i = 0; i < 2; ++i) {
			newPts.getData()[pid * 3 + 0] = model.modelBounds[0];
		...
		if (model.zShadows) for (let i = 0; i < 2; ++i) {
			newPts.getData()[pid * 3 + 0] = model.modelBounds[0];
			newPts.getData()[pid * 3 + 1] = model.focalPoint[1];
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.get(publicAPI, model, ["focus"]);
	macros_default.getArray(publicAPI, model, ["modelBounds"], 6);
	macros_default.getArray(publicAPI, model, ["focalPoint"], 3);
	macros_default.setGet(publicAPI, model, ["outline"]);
	macros_default.setGet(publicAPI, model, ["axes"]);
	macros_default.setGet(publicAPI, model, ["xShadows"]);
	macros_default.setGet(publicAPI, model, ["yShadows"]);
	macros_default.setGet(publicAPI, model, ["zShadows"]);
	macros_default.setGet(publicAPI, model, ["wrap"]);
	macros_default.setGet(publicAPI, model, ["translationMode"]);
	macros_default.algo(publicAPI, model, 0, 1);
	vtkCursor3D(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkCursor3D");
var Cursor3D_default = {
	newInstance,
	extend
};
//#endregion
export { Cursor3D_default as default, extend, newInstance };
