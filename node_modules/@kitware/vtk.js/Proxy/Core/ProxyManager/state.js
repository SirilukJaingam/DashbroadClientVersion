...
import PiecewiseFunctionProxy_default from "../PiecewiseFunctionProxy.js";
//#region Sources/Proxy/Core/ProxyManager/state.js
...
function addStateAPI(publicAPI, model) {
	publicAPI.loadState = (state, options = {}) => new Promise((resolve, reject) => {
		...
		Promise.all(sourcePromises).then(() => {
			...
			state.views.forEach(({ id, group, name, props, camera }) => {
				let proxy = null;
				if (state.options.recycleViews) proxy = views.find((v) => v.getProxyGroup() === group && v.getProxyName() === name && v.getName() === props.name);
				if (!proxy) proxy = publicAPI.createProxy(group, name, { disableAnimation: true });
				else proxy.setDisableAnimation(true);
				proxy.set(props, true);
				proxyMapping[id] = proxy;
				cameras[id] = camera;
			});
			function updateView(view) {
				if (!proxyMapping[view] || !cameras[view]) return;
			...
			state.representations.forEach(({ source, view, props }) => {
				publicAPI.getRepresentation(proxyMapping[source], proxyMapping[view]).set(props, true);
				updateView(view);
			});
			Object.keys(state.fields).forEach((fieldName) => {
				...
				switch (piecewiseFunction.mode) {
					case PiecewiseFunctionProxy_default.Mode.Gaussians:
						pwfProxy.setGaussians(piecewiseFunction.gaussians);
						break;
					case PiecewiseFunctionProxy_default.Mode.Points:
						pwfProxy.setPoints(piecewiseFunction.points);
						break;
					case PiecewiseFunctionProxy_default.Mode.Nodes:
			...
			Object.keys(proxyMapping).forEach((originalId) => {
				$oldToNewIdMapping[originalId] = proxyMapping[originalId].getProxyId();
			});
	...
	publicAPI.saveState = (options = {}, userData = {}) => new Promise((resolve, reject) => {
		...
		Promise.all(datasets).then(() => {
			for (let i = 0; i < state.sources.length; i++) state.sources[i].props.dataset.then((value) => {
