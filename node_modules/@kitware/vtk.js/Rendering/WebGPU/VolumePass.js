...
import VolumePassFSQ_default from "./VolumePassFSQ.js";
//#region Sources/Rendering/WebGPU/VolumePass/index.js
var { Representation } = Property_default;
var { BufferUsage, PrimitiveTypes } = BufferManager_default;
var cubeFaceTriangles = [
	[
		0,
		4,
		6
...
function vtkWebGPUVolumePass(publicAPI, model) {
	...
	publicAPI.initialize = (viewNode) => {
		if (!model._clearEncoder) publicAPI.createClearEncoder(viewNode);
		...
		if (!model.fullScreenQuad) {
			model.fullScreenQuad = VolumePassFSQ_default.newInstance();
		...
		if (!model._volumeCopyQuad) {
			model._volumeCopyQuad = FullScreenQuad_default.newInstance();
			...
			model._copyUBO = UniformBuffer_default.newInstance({ label: "mapperUBO" });
	...
	publicAPI.traverse = (renNode, viewNode) => {
		...
		if (model.volumes.length > maxVolumes) {
			...
			for (let v = 0; v < model.volumes.length; v++) {
				...
				distances[v] = distance2BetweenPoints([
					.5 * (bounds[1] + bounds[0]),
					.5 * (bounds[3] + bounds[2]),
					.5 * (bounds[5] + bounds[4])
				], cameraPos);
		...
		} else publicAPI.rayCastPass(viewNode, renNode, model.volumes);
		...
		} else model._copyUBO.setArray("tscale", [1, 1]);
	...
	publicAPI.computeTiming = (viewNode) => {
		...
		if (model._lastScale == null) model._lastScale = model.volumes[0].getRenderable().getMapper().getInitialInteractionScale() || 1;
		...
		if (!model._animationRateSubscription) model._animationRateSubscription = rwi.onAnimationFrameRateUpdate(() => {
			...
			if (firstMapper.getAutoAdjustSampleDistances()) {
				...
				model._lastScale = model._lastScale * rwi.getDesiredUpdateRate() / frate;
				if (model._lastScale > 400) model._lastScale = 400;
			} else model._lastScale = firstMapper.getImageSampleDistance() * firstMapper.getImageSampleDistance();
	...
	publicAPI.createDepthRangeEncoder = (viewNode) => {
		const device = viewNode.getDevice();
		model._depthRangeEncoder = RenderEncoder_default.newInstance({ label: "VolumePass DepthRange" });
		...
		model._depthRangeEncoder.setReplaceShaderCodeFunction((pipeline) => {
			...
			code = ShaderCache_default.substitute(code, "//VTK::RenderEncoder::Impl", ["output.outColor1 = vec4<f32>(input.fragPos.z, 0.0, 0.0, 0.0);", "output.outColor2 = vec4<f32>(stopval, 0.0, 0.0, 0.0);"]).result;
			fDesc.setCode(code);
		});
		model._depthRangeEncoder.setDescription({ colorAttachments: [{
			view: null,
			clearValue: [
				0,
				0,
		...
		model._depthRangeTexture = Texture_default.newInstance({ label: "volumePassMaxDepth" });
		...
		model._depthRangeTexture2 = Texture_default.newInstance({ label: "volumePassDepthMin" });
	...
	publicAPI.createClearEncoder = (viewNode) => {
		model._colorTexture = Texture_default.newInstance({ label: "volumePassColor" });
		...
		model._clearEncoder = RenderEncoder_default.newInstance({ label: "VolumePass Clear" });
		model._clearEncoder.setColorTextureView(0, model._colorTextureView);
		model._clearEncoder.setDescription({ colorAttachments: [{
			view: null,
			clearValue: [
				0,
				0,
	...
	publicAPI.createCopyEncoder = (viewNode) => {
		model._copyEncoder = RenderEncoder_default.newInstance({ label: "volumePassCopy" });
		model._copyEncoder.setDescription({ colorAttachments: [{
			view: null,
			loadOp: "load",
			storeOp: "store"
		}] });
	...
	publicAPI.createMergeEncoder = (viewNode) => {
		model._mergeEncoder = RenderEncoder_default.newInstance({ label: "volumePassMerge" });
		model._mergeEncoder.setColorTextureView(0, model._colorTextureView);
		model._mergeEncoder.setDescription({ colorAttachments: [{
			view: null,
			loadOp: "load",
			storeOp: "store"
		}] });
...
function extend(publicAPI, model, initialValues = {}) {
	...
	model._mapper = SimpleMapper_default.newInstance();
	...
	model._boundsPoly = PolyData_default.newInstance();
	model._lastMTimes = [];
	macros_default.setGet(publicAPI, model, ["colorTextureView", "depthTextureView"]);
	vtkWebGPUVolumePass(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkWebGPUVolumePass");
var VolumePass_default = {
	newInstance,
	extend
};
//#endregion
export { VolumePass_default as default, extend, newInstance };

//# sourceMappingURL=VolumePass.js.map