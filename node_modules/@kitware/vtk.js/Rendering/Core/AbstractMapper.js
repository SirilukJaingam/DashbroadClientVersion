...
//#region Sources/Rendering/Core/AbstractMapper/index.js
var { vtkErrorMacro } = macros_default;
var tmpClipMatrix = new Float64Array(16);
var tmpClipWorldPlane = new Float64Array(4);
function getClipPlaneEquation(plane, out) {
	...
	out[1] = normal[1];
	out[2] = normal[2];
	out[3] = -(normal[0] * origin[0] + normal[1] * origin[1] + normal[2] * origin[2]);
	return out;
}
function vtkAbstractMapper(publicAPI, model) {
	...
	publicAPI.addClippingPlane = (plane) => {
		if (!plane.isA("vtkPlane")) return false;
	...
	publicAPI.removeAllClippingPlanes = () => {
		if (model.clippingPlanes.length === 0) return false;
	...
	publicAPI.getClippingPlanesMTime = () => {
		let mtime = 0;
		for (let i = 0; i < model.clippingPlanes.length; i++) mtime = Math.max(mtime, model.clippingPlanes[i].getMTime());
		return mtime;
	};
	publicAPI.setClippingPlanes = (planes) => {
		if (!planes) return;
		if (!Array.isArray(planes)) publicAPI.addClippingPlane(planes);
		else {
			const nbPlanes = planes.length;
			for (let i = 0; i < nbPlanes && i < 6; i++) publicAPI.addClippingPlane(planes[i]);
		}
	};
...
var DEFAULT_VALUES = { clippingPlanes: [] };
function extend(publicAPI, model, initialValues = {}) {
	Object.assign(model, DEFAULT_VALUES, initialValues);
	macros_default.obj(publicAPI, model);
	macros_default.algo(publicAPI, model, 1, 0);
	if (!model.clippingPlanes) model.clippingPlanes = [];
	vtkAbstractMapper(publicAPI, model);
}
var AbstractMapper_default = { extend };
//#endregion
export { AbstractMapper_default as default, extend };

//# sourceMappingURL=AbstractMapper.js.map