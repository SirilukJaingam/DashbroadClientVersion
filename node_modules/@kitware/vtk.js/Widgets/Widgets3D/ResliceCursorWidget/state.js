...
import { ScrollingMethods, planeNameToViewType, planeNames } from "./Constants.js";
//#region Sources/Widgets/Widgets3D/ResliceCursorWidget/state.js
var defaultPlanes = {
	X: {
		normal: [
			1,
...
function generateState(planes = planeNames) {
	const state = StateBuilder_default.createBuilder().addField({
		name: "center",
		initialValue: [
			0,
			0,
	...
	planes.reduce((viewState, view) => planes.filter((v) => v !== view).reduce((axisState, axis) => {
		...
		for (let rotationHandle = 0; rotationHandle < 2; ++rotationHandle) axisState.addStateFromMixin({
			labels: [
				"handles",
				"sphere",
				"rotation",
