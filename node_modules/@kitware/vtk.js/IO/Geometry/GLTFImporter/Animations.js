...
//#region Sources/IO/Geometry/GLTFImporter/Animations.js
var { vtkDebugMacro, vtkWarningMacro } = macros_default;
/**
	...
	function applyAnimation(value) {
		...
		switch (path) {
			...
			case "scale":
				...
				break;
			default: vtkWarningMacro(`Unsupported animation path: ${path}`);
		}
	}
	function animate(currentTime) {
		applyAnimation(glTFSamplers[glTFChannel.sampler].evaluate(currentTime, path));
	}
	return {
		...glTFChannel,
		animate
...
function createAnimationSampler(glTFSampler) {
	...
	function findKeyframes(time) {
		...
		while (i1 < glTFSampler.input.length - 1 && glTFSampler.input[i1] <= time) i1++;
		const i0 = Math.max(0, i1 - 1);
		lastKeyframeIndex = i0;
		return [
			glTFSampler.input[i0],
			glTFSampler.input[i1],
			i0,
			i1
		];
	}
	function stepInterpolate(path, i0) {
		...
		for (let i = 0; i < 3; ++i) v0[i] = glTFSampler.output[startIndex + i];
		return v0;
	}
	function linearInterpolate(path, t0, t1, i0, i1, t) {
		...
		switch (path) {
			case "translation":
			case "scale": return vec3.lerp(vec3.create(), v0, v1, ratio);
			case "rotation": return quat.slerp(quat.create(), v0, v1, ratio);
			default:
	...
	function cubicSplineInterpolate(path, t0, t1, i0, i1, time) {
		...
		if (Array.isArray(p0)) return p0.map((v, j) => {
			const a = 2 * t3 - 3 * t2 + 1;
			const b = t3 - 2 * t2 + t;
			const c = -2 * t3 + 3 * t2;
			const d = t3 - t2;
	...
	function evaluate(time, path) {
		...
		switch (glTFSampler.interpolation) {
			...
			case "CUBICSPLINE":
				...
				break;
			default: vtkWarningMacro(`Unknown interpolation method: ${glTFSampler.interpolation}`);
		}
		return result;
	}
	return {
...
function createAnimationMixer(nodes, accessors) {
	...
	function stop(name) {
		if (activeAnimations.delete(name)) vtkWarningMacro(`Stopped animation "${name}"`);
		else vtkWarningMacro(`Animation "${name}" was not playing`);
	}
	function stopAll() {
	...
	function update(deltaTime) {
		const totalWeight = Array.from(activeAnimations.values()).reduce((sum, { weight }) => sum + weight, 0);
