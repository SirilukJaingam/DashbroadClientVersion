...
//#region Sources/Filters/General/Calculator/index.js
...
function vtkCalculator(publicAPI, model) {
	...
	publicAPI.setFormula = (formula) => {
		if (formula === model.formula) return false;
	...
	publicAPI.augmentInputArrays = (locn, arraysIn) => {
		...
		if (locn === FieldDataTypes.POINT || locn === FieldDataTypes.VERTEX) arraysOut.push({ location: FieldDataTypes.COORDINATE });
		return arraysOut;
	};
	publicAPI.createSimpleFormulaObject = (locn, arrNames, resultName, singleValueFormula, options = {}) => ({
		...
		evaluate: (arraysIn, arraysOut) => {
			...
			if (nc === 1) arrayOutRaw.forEach((xxx, ii) => {
	...
	publicAPI.prepareArrays = (arraySpec, inData, outData) => {
		...
		arraySpec.input.forEach((spec) => {
			if (spec.location === FieldDataTypes.COORDINATE) arraysIn.push(inData.getPoints());
			else {
				...
				if (dsa) if (spec.name) arraysIn.push(dsa.getArrayByName(spec.name));
				else if ("index" in spec) arraysIn.push(dsa.getArrayByIndex(spec.index));
				else if ("attribute" in spec && spec.location !== FieldDataTypes.UNIFORM) arraysIn.push(dsa.getActiveAttribute(spec.attribute));
				else {
		...
		arraySpec.output.forEach((spec) => {
			...
			if (spec.location === FieldDataTypes.UNIFORM && "tuples" in fullSpec) fullSpec.size = ncomp * fullSpec.tuples;
			if (spec.location === FieldDataTypes.COORDINATE) {
				const inPts = inData.getPoints();
				const pts = Points_default.newInstance({ dataType: inPts.getDataType() });
				...
				if (tuples <= 0) {
					...
					const arrOut = DataArray_default.newInstance(fullSpec);
					const arrIdx = dsa.addArray(arrOut);
					if ("attribute" in fullSpec && spec.location !== FieldDataTypes.UNIFORM) dsa.setActiveAttributeByIndex(arrIdx, fullSpec.attribute);
	...
	publicAPI.requestData = (inData, outData) => {
		...
		const newDataSet = outData[0]?.initialize() || vtk({ vtkClass: inData[0].getClassName() });
...
var newInstance = macros_default.newInstance(extend, "vtkCalculator");
var Calculator_default = {
	newInstance,
	extend
};
//#endregion
export { Calculator_default as default, extend, newInstance };
