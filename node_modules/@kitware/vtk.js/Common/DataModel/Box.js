...
function vtkBox(publicAPI, model) {
	...
	publicAPI.setBounds = (...bounds) => {
		...
		BoundingBox_default.setBounds(model.bbox, boundsArray);
	};
	...
	publicAPI.evaluateFunction = (x, y, z) => {
		...
		let distance = 0;
		const minPoint = BoundingBox_default.getMinPoint(model.bbox);
		const maxPoint = BoundingBox_default.getMaxPoint(model.bbox);
		let inside = 1;
		for (let i = 0; i < 3; i++) {
			diff = BoundingBox_default.getLength(model.bbox, i);
			if (diff !== 0) {
	...
	publicAPI.addBounds = (...bounds) => {
		...
		BoundingBox_default.addBounds(model.bbox, ...boundsArray);
		publicAPI.modified();
	};
...
var DEFAULT_VALUES = { bbox: [...BoundingBox_default.INIT_BOUNDS] };
function extend(publicAPI, model, initialValues = {}) {
	Object.assign(model, DEFAULT_VALUES, initialValues);
	ImplicitFunction_default.extend(publicAPI, model, initialValues);
	vtkBox(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkBox");
var Box_default = {
	newInstance,
	extend,
	intersectWithLine,
...
//#endregion
export { STATIC, Box_default as default, extend, intersectWithLine, newInstance };
