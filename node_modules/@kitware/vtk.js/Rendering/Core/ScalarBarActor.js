...
import Actor_default from "./Actor.js";
import Mapper_default from "./Mapper.js";
import Texture_default from "./Texture.js";
import Constants_default from "./ScalarBarActor/Constants.js";
import * as d3 from "d3-scale";
//#region Sources/Rendering/Core/ScalarBarActor/index.js
var { VectorMode } = ScalarsToColors_default;
var { Orientation } = Constants_default;
...
function defaultAutoLayout(publicAPI, model) {
	return (helper) => {
		...
		const xAxisAdjust = (lastSize[0] / 700) ** .8;
		const yAxisAdjust = (lastSize[1] / 700) ** .8;
		...
		if (axisTextStyle.fontSize === void 0) axisTextStyle.fontSize = Math.max(24 * minAdjust, 12);
		if (tickTextStyle.fontSize === void 0) if (helper.getLastAspectRatio() > 1) tickTextStyle.fontSize = Math.max(20 * minAdjust, 10);
		else tickTextStyle.fontSize = Math.max(16 * minAdjust, 10);
		const textSizes = helper.updateTextureAtlas();
		helper.setTopTitle(false);
		const boxSize = helper.getBoxSizeByReference();
		let isVertical = false;
		if (model.orientation === Orientation.VERTICAL) isVertical = true;
		else if (model.orientation === Orientation.HORIZONTAL) isVertical = false;
		else isVertical = helper.getLastAspectRatio() > 1;
		if (isVertical) {
			helper.setTickLabelPixelOffset(.3 * tickTextStyle.fontSize);
			if (textSizes.titleWidth <= textSizes.tickWidth + helper.getTickLabelPixelOffset() + .8 * tickTextStyle.fontSize) {
				helper.setTopTitle(true);
				helper.setAxisTitlePixelOffset(.2 * tickTextStyle.fontSize);
				boxSize[0] = 2 * (textSizes.tickWidth + helper.getTickLabelPixelOffset() + .8 * tickTextStyle.fontSize) / lastSize[0];
				helper.setBoxPosition([.98 - boxSize[0], -.92]);
			} else {
				helper.setAxisTitlePixelOffset(.2 * tickTextStyle.fontSize);
				boxSize[0] = 2 * (textSizes.titleHeight + helper.getAxisTitlePixelOffset() + textSizes.tickWidth + helper.getTickLabelPixelOffset() + .8 * tickTextStyle.fontSize) / lastSize[0];
				helper.setBoxPosition([.99 - boxSize[0], -.92]);
			}
			boxSize[1] = Math.max(1.2, Math.min(1.84 / yAxisAdjust, 1.84));
		} else {
			helper.setAxisTitlePixelOffset(1.2 * tickTextStyle.fontSize);
			helper.setTickLabelPixelOffset(.1 * tickTextStyle.fontSize);
			const titleHeight = 2 * (.8 * tickTextStyle.fontSize + textSizes.titleHeight + helper.getAxisTitlePixelOffset()) / lastSize[1];
			...
			helper.setBoxPosition([-.5 * boxSize[0], -.97]);
		}
...
function vtkScalarBarActorHelper(publicAPI, model) {
	...
	publicAPI.setRenderable = (renderable) => {
		if (model.renderable === renderable) return;
	...
	publicAPI.updateAPISpecificData = (size, camera, renderWindow) => {
		...
		if (!scalarsToColors || !model.renderable.getVisibility()) return;
		...
		if (model.forceUpdate || Math.max(scalarsToColors.getMTime(), publicAPI.getMTime(), model.renderable.getMTime()) > model.lastRebuildTime.getMTime()) {
			model.lastTickBounds = [...scalarsToColors.getMappingRange()];
			model.renderable.getGenerateTicks()(publicAPI);
			if (model.renderable.getAutomated()) model.renderable.getAutoLayout()(publicAPI);
			else {
	...
	publicAPI.updateTextureAtlas = () => {
		...
		const strings = [
			...publicAPI.getTickStrings(),
			"NaN",
			"Below",
			"Above"
		];
		for (let t = 0; t < strings.length; t++) if (!newTmAtlas.has(strings[t])) {
			...
			totalHeight += entry.height;
			if (maxWidth < entry.width) maxWidth = entry.width;
			if (results.tickWidth < entry.width) results.tickWidth = entry.width;
			if (results.tickHeight < entry.height) results.tickHeight = entry.height;
		}
	...
	publicAPI.recomputeBarSegments = (textSizes) => {
		...
		const segSpace = model.vertical ? .01 : .02;
		...
		if (model.renderable.getDrawNanAnnotation() && model.renderable.getScalarsToColors().getNanColor()) pushSeg("NaN", [
			NaN,
			NaN,
			NaN,
			NaN
		]);
		if (model.renderable.getDrawBelowRangeSwatch() && model.renderable.getScalarsToColors().getUseBelowRangeColor?.()) pushSeg("Below", [
			-.1,
			-.1,
			-.1,
			-.1
	...
	publicAPI.createPolyDataForOneLabel = (text, pos, alignment, orientation, offset, results) => {
		...
		tmp2v3[0] = (.5 * pos[0] + .5) * model.lastSize[0];
		tmp2v3[1] = (.5 * pos[1] + .5) * model.lastSize[1];
		tmp2v3[2] = pos[2];
		tmp2v3[0] += offset[0];
		tmp2v3[1] += offset[1];
		const textSize = [];
		...
		if (orientation === "vertical") {
			...
			if (alignment[0] === "middle") tmp2v3[1] -= value.width / 2;
			else if (alignment[0] === "right") tmp2v3[1] -= value.width;
			if (alignment[1] === "middle") tmp2v3[0] += value.height / 2;
			else if (alignment[1] === "top") tmp2v3[0] += value.height;
		} else {
			textSize[0] = value.width;
			textSize[1] = value.height;
			if (alignment[0] === "middle") tmp2v3[0] -= value.width / 2;
			else if (alignment[0] === "right") tmp2v3[0] -= value.width;
			if (alignment[1] === "middle") tmp2v3[1] -= value.height / 2;
			else if (alignment[1] === "top") tmp2v3[1] -= value.height;
		}
	...
	publicAPI.updatePolyDataForLabels = () => {
		...
		if (model.vertical) {
			...
			if (model.topTitle) {
				tmpv3[0] = model.boxPosition[0] + .5 * model.boxSize[0];
				...
				tmpv3[1] = model.barPosition[1] + .5 * model.barSize[1];
			...
			tmpv3[0] = model.barPosition[0] + .5 * model.barSize[0];
			tmpv3[1] = model.barPosition[1] + model.barSize[1];
			publicAPI.createPolyDataForOneLabel(model.renderable.getAxisLabel(), tmpv3, ["middle", "bottom"], "horizontal", [0, model.axisTitlePixelOffset], results);
		}
		tmpv3[offsetAxis] = model.barPosition[offsetAxis] + (.5 * dir[offsetAxis] + .5) * model.barSize[offsetAxis];
		tmpv3[spacedAxis] = model.barPosition[spacedAxis] + model.barSize[spacedAxis] * .5;
		let tickSeg = null;
		for (let i = 0; i < model.barSegments.length; i++) {
			const seg = model.barSegments[i];
			if (seg.title === "ticks") tickSeg = seg;
			else {
				tmpv3[spacedAxis] = model.barPosition[spacedAxis] + .5 * model.barSize[spacedAxis] * (seg.corners[2][spacedAxis] + seg.corners[0][spacedAxis]);
		...
		for (let t = 0; t < ticks.length; t++) {
			tmpv3[spacedAxis] = tickSegmentStart + tickSegmentSize * (tickPositions ? tickPositions[t] : (ticks[t] - model.lastTickBounds[0]) / (model.lastTickBounds[1] - model.lastTickBounds[0]));
			publicAPI.createPolyDataForOneLabel(tickStrings[t], tmpv3, alignment, "horizontal", tickOffsets, results);
		}
		const tcoordDA = DataArray_default.newInstance({
	...
	publicAPI.updatePolyDataForBarSegments = () => {
		...
		if (model.renderable.getDrawNanAnnotation() && scalarsToColors.getNanColor()) numberOfExtraColors += 1;
		if (model.renderable.getDrawBelowRangeSwatch() && scalarsToColors.getUseBelowRangeColor?.()) numberOfExtraColors += 1;
		if (model.renderable.getDrawAboveRangeSwatch() && scalarsToColors.getUseAboveRangeColor?.()) numberOfExtraColors += 1;
		const numPts = 4 * (1 + numberOfExtraColors);
		const numQuads = numPts;
		let numComps = 1;
		if (scalarsToColors.getVectorMode() === VectorMode.COMPONENT) numComps = scalarsToColors.getVectorComponent() + 1;
		const points = new Float64Array(numPts * 3);
		...
		for (let i = 0; i < model.barSegments.length; i++) {
			...
			for (let e = 0; e < 4; e++) {
				...
				points[ptIdx * 3] = (.5 * tmpv3[0] + .5) * model.lastSize[0];
				points[ptIdx * 3 + 1] = (.5 * tmpv3[1] + .5) * model.lastSize[1];
				points[ptIdx * 3 + 2] = tmpv3[2];
				for (let nc = 0; nc < numComps; nc++) scalars[ptIdx * numComps + nc] = model.lastTickBounds[0] + seg.scalars[e] * (model.lastTickBounds[1] - model.lastTickBounds[0]);
				ptIdx++;
			}
			cells[cellIdx * 5] = 4;
		...
		const scalarsDA = DataArray_default.newInstance({
...
var newScalarBarActorHelper = macros_default.newInstance((publicAPI, model, initialValues = { renderable: null }) => {
	...
	macros_default.getArray(publicAPI, model, ["boxPosition", "boxSize"]);
	macros_default.setArray(publicAPI, model, ["boxPosition", "boxSize"], 2);
	model.forceUpdate = false;
	model.lastRebuildTime = {};
	macros_default.obj(model.lastRebuildTime, { mtime: 0 });
	model.lastSize = [-1, -1];
	model.tmCanvas = document.createElement("canvas");
	model.tmContext = model.tmCanvas.getContext("2d");
	model._tmAtlas = /* @__PURE__ */ new Map();
	model.barMapper = Mapper_default.newInstance();
	model.barMapper.setInterpolateScalarsBeforeMapping(true);
	model.barMapper.setUseLookupTableScalarRange(true);
	model.polyData = PolyData_default.newInstance();
	model.barMapper.setInputData(model.polyData);
	model.barActor = Actor_default.newInstance();
	model.barActor.setMapper(model.barMapper);
	model.tmPolyData = PolyData_default.newInstance();
	model.tmMapper = Mapper_default.newInstance();
	model.tmMapper.setInputData(model.tmPolyData);
	model.tmTexture = Texture_default.newInstance({ resizable: true });
	model.tmTexture.setInterpolate(false);
	model.tmActor = Actor_default.newInstance({ parentProp: publicAPI });
	model.tmActor.setMapper(model.tmMapper);
	model.tmActor.addTexture(model.tmTexture);
	model.barPosition = [0, 0];
	model.barSize = [0, 0];
	model.boxPosition = [.88, -.92];
	model.boxSize = [.1, 1.1];
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.get(publicAPI, model, ["axisTextStyle", "tickTextStyle"]);
...
var newInstance = macros_default.newInstance(extend, "vtkScalarBarActor");
var ScalarBarActor_default = {
	newInstance,
	extend,
...
//#endregion
export { ScalarBarActor_default as default, extend, newInstance };

//# sourceMappingURL=ScalarBarActor.js.map