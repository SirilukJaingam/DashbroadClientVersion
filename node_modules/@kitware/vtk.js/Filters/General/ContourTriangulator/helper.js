...
import { add, cross, distance2BetweenPoints, dot, norm, normalize, subtract } from "../../../Common/Core/Math.js";
import Points_default from "../../../Common/Core/Points.js";
import IncrementalOctreePointLocator_default from "../../../Common/DataModel/IncrementalOctreePointLocator.js";
...
//#region Sources/Filters/General/ContourTriangulator/helper.js
...
*/
function reverseElements(arr, firstIdx = void 0, lastIdx = void 0) {
	const first = firstIdx ?? 0;
	const last = lastIdx ?? arr.length - 1;
	const mid = first + Math.floor((last - first) / 2);
	for (let i = first; i <= mid; ++i) [arr[i], arr[last - (i - first)]] = [arr[last - (i - first)], arr[i]];
}
/**
...
function vtkCCSInsertTriangle(polys, poly, trids, polyEdges, originalEdges) {
	...
	for (let vert = 0; vert < 3; vert++) {
		...
		if (edgeLoc >= 0) {
			let nextId = currId + 1;
			if (nextId === poly.length) nextId = 0;
	...
	if (edgeCount === 0) polys.insertNextCell([
		poly[trids[0]],
		poly[trids[1]],
		poly[trids[2]]
	]);
		...
		let maxPoints = 0;
		let currSide = 0;
		for (let i = 0; i < 3; i++) if (edgeLocs[i] >= 0) {
			const edgeLoc = edgeLocs[i];
			const npts = originalEdges[edgeLoc];
			const pts = originalEdges.slice(edgeLoc + 1, edgeLoc + 1 + npts);
			if (!(edgePts[i][0] === pts[0] || edgePts[i][1] === pts[npts - 1])) vtkErrorMacro("assertion error in vtkCCSInsertTriangle");
			if (npts > maxPoints) {
				maxPoints = npts;
				currSide = i;
			}
		...
		for (let side = 0; side < 3; side++) if ((side !== prevSide || prevNeeded) && (side !== nextSide || nextNeeded)) {
			let m = 0;
			...
			for (let k = m; k < n; k++) polys.insertNextCell([
				edgePts[side][k],
				edgePts[side][k + 1],
				tailPtIds[side]
			]);
			...
			if (concave) {
				...
				for (; foundEar && jj !== k; jj = jj + 1 !== n ? jj + 1 : 0) {
					...
					if (side ? !sside : sside) {
						side = !side;
						foundNegative = true;
						foundEar = Line_default.intersection(ppoint, npoint, x, y, s, t) === Line_default.IntersectionState.NO_INTERSECTION;
					}
				}
				foundEar &&= foundNegative;
			}
			if (!foundEar) verts[i][1] = Number.MIN_VALUE;
			else {
				vtkCCSInsertTriangle(polys, poly, [
					verts[i][0],
					verts[j][0],
...
function vtkCCSMakePolysFromLines(polyData, firstLine, endLine, oriented, newPolys, incompletePolys) {
	...
	while (remainingLines > 0) {
		...
		for (lineId = firstLine; lineId < endLine; lineId++) if (!usedLines[lineId - firstLine]) {
			...
			poly.length = n;
			for (let i = 0; i < n; i++) poly[i] = pts[i];
			break;
		}
				...
				if (matches.length > 0) {
					if (matches.length > 1) {
						...
						do {
							...
							if (!r && (endIdx === 0 && poly[npoly - 2] === pts[1] || endIdx === 1 && poly[1] === pts[npts - 2]) || r && (endIdx === 0 && poly[npoly - 2] === pts[npts - 2] || endIdx === 1 && poly[1] === pts[1])) matches.splice(k, 1);
					...
					lineEndPts[0] = pts[0];
					lineEndPts[1] = pts[npts - 1];
					if (endPts[endIdx] === lineEndPts[endIdx]) completePoly = endPts[1 - endIdx] === lineEndPts[1 - endIdx];
					else completePoly = endPts[1 - endIdx] === lineEndPts[endIdx];
					if (endIdx === 0) for (let i = 1; i < npts - (completePoly ? 1 : 0); i++) poly.push(pts[i]);
					else for (let i = completePoly ? 1 : 0; i < npts - 1; i++) poly.unshift(pts[i]);
					if (endPts[endIdx] !== lineEndPts[endIdx]) {
						...
						while (ptsIt !== ptsEnd) poly[--pit] = poly[ptsIt++];
					}
					usedLines[lineId - firstLine] = 1;
					remainingLines--;
					noLinesMatch = false;
...
function vtkCCSJoinLooseEnds(polys, incompletePolys, points, normal) {
	...
	while (n !== 0) {
		...
		if (dMin < Number.MAX_VALUE) if (iMin === n - 1) incompletePolys.pop();
		else {
	...
	while (i > 0) polys.splice(removePolys[--i], 1);
...
function vtkCCSVectorProgression(p, p1, p2, p3, normal) {
	...
	if (s1 !== 0 && s2 !== 0) {
		...
		const c1 = dot(v2, v1);
		const l1 = norm(v1);
		const ck = (dot(v2, v3) * norm(v3) - c1 * l1) * (1 - sb1 * 2);
...
function vtkCCSSplitAtPinchPoints(polys, points, polyGroups, polyEdges, normal, oriented) {
	const tryPoints = Points_default.newInstance({
		dataType: VtkDataTypes.DOUBLE,
		empty: true
	});
	const locator = IncrementalOctreePointLocator_default.newInstance();
	let splitCount = 0;
	let poly;
	let n;
	let bounds;
	...
	for (let i = 0; i < polys.length; i++) {
		...
		n = poly.length;
		bounds = [];
		tol = CCS_POLYGON_TOLERANCE * Math.sqrt(Polygon_default.getBounds(poly, points, bounds));
		if (tol === 0) continue;
		...
		for (idx2 = 0; idx2 < n; idx2++) {
			...
			if (!success) {
				...
				if (idx2 > idx1 + 2 - unique && n + idx1 > idx2 + 2 - unique) if (oriented) {
		...
		if (foundMatch) {
			...
			if (polyGroups[i].length > 0) polyGroups[polys.length - 1].push(polys.length - 1);
		}
	}
	return splitCount;
}
...
function vtkCCSFindTrueEdges(polys, points, polyEdges, originalEdges) {
	...
	for (let polyId = 0; polyId < polys.length; polyId++) {
		...
		let m = n;
		const tol2 = Polygon_default.getBounds(oldPoly, points, []) * atol2;
		const newPoly = [];
		let cornerPointId = 0;
		let oldOriginalId = -1;
		const partialEdge = [];
		...
		for (let j = 0; j < n; j++) {
			...
			if (m <= 3 || l1 > tol2 && (c < 0 || l1 < tol2 || l2 < tol2 || s2 > l1 * l2 * atol2)) {
				...
				} else if (cellCount === 0) partialEdge.push(pointId);
				else newEdges.push(-1);
...
function vtkCCSReversePoly(poly, edges, originalEdges) {
	...
	edges.reverse();
	for (let i = 0; i < edges.length; i++) if (edges[i] >= 0) {
...
function vtkCCSPolyInPoly(outerPoly, innerPoly, points, normal, pp, bounds, tol2) {
	...
	for (let jj = 0; jj < m; jj++) {
		...
		const intersectionState = Polygon_default.pointInPolygon(p, pp, bounds, normal);
		if (intersectionState === PolygonWithPointIntersectionState.FAILURE) vtkErrorMacro("Error finding point in polygon in vtkCCSPolyInPoly");
		if (intersectionState !== PolygonWithPointIntersectionState.OUTSIDE) {
			...
			for (let ii = 0; ii < n; ii++) {
				...
				const { distance } = Line_default.distanceToLine(p, q1, q2);
				if (distance < tol2) {
					pointOnEdge = 1;
					break;
				}
...
function vtkCCSPrepareForPolyInPoly(outerPoly, points, pp, bounds) {
	...
	return Polygon_default.getBounds(outerPoly, points, bounds) * (CCS_POLYGON_TOLERANCE * CCS_POLYGON_TOLERANCE);
}
/**
...
function vtkCCSMakeHoleyPolys(newPolys, points, polyGroups, polyEdges, originalEdges, normal, oriented) {
	...
	if (numNewPolys <= 1) return;
	const polyReversed = [];
	const innerPolys = [];
	let groupCount;
	if (!oriented) groupCount = new Int32Array(numNewPolys);
	let nmax = 1;
	for (let kk = 0; kk < numNewPolys; kk++) nmax = Math.max(nmax, newPolys[kk].length);
	...
	for (let i = 0; i < numNewPolys; i++) {
		...
		if (n < 3) continue;
		const { isNormalNotZero, sense } = vtkCCSCheckPolygonSense(newPolys[i], points, normal);
		if (isNormalNotZero) polyReversed[i] = !sense;
		tol2 = vtkCCSPrepareForPolyInPoly(newPolys[i], points, pp, bounds);
		for (let j = 0; j < numNewPolys; j++) if (j !== i && newPolys[j].length >= 3) {
			if (!polyGroups[j].includes(i)) {
	...
	if (!oriented) {
		...
		for (let ll = 0; ll < numNewPolys; ll++) if (groupCount[ll] === 0) outerPolyStack.push(ll);
		let j;
		while (outerPolyStack.length > 0) {
			...
			if (polyGroups[j].length > 1) {
				...
				for (let k = 1; k < polyGroups[j].length; k++) {
					...
					if (groupCount[jj] > 1) {
						groupCount[jj] -= 2;
						if (groupCount[jj] === 0) outerPolyStack.push(jj);
					} else {
						innerPolys[jj] = 1;
				...
				polyGroups[j].push(j);
				for (let jj = 0; jj < numNewPolys; jj++) if (innerPolys[jj]) polyGroups[j].push(jj);
			}
		}
	} else for (let j = 0; j < numNewPolys; j++) if (polyReversed[j]) polyGroups[j].length = 0;
	else if (polyGroups[j].length > 1) {
		innerPolys.length = 0;
		for (let k = 1; k < polyGroups[j].length; k++) innerPolys[polyGroups[j][k]] = true;
		for (let kk = 1; kk < polyGroups[j].length; kk++) {
			const jj = polyGroups[j][kk];
			if (!polyReversed[jj]) for (let ii = 0; ii < polyGroups[jj].length; ii++) innerPolys[polyGroups[jj][ii]] = false;
		}
		polyGroups[j].length = 0;
		polyGroups[j].push(j);
		for (let jj = 0; jj < numNewPolys; jj++) if (innerPolys[jj]) polyGroups[j].push(jj);
				...
				if ((c1 ? !c2 : c2) || v1 * v1 < tol2 || v2 * v2 < tol2) {
					...
					if (dot(w, w) > 0) {
						...
						if (d1 ? !d2 : d2) {
							...
							if (v2 * v2 < v1 * v1) p = p2;
							if (u2 * u2 < u1 * u1) q = q2;
							if (distance2BetweenPoints(p, q) > tol2) return false;
						}
					}
				}
			}
...
function vtkCCSFindCuts(polys, polyGroup, outerPolyId, innerPolyId, points, normal, cuts, exhaustive) {
	...
	for (cutId = 0; cutId < 2; cutId++) {
		...
		for (let i = 0; i < count && !foundCut; i++) {
			...
			for (let kk = 0; kk < outerPoly.length; kk++) cutlist[kk] = [vtkCCSCutQuality(outerPoly, innerPoly, kk, j, points), kk];
			cutlist.sort((a, b) => a[0] - b[0]);
			for (let lid = 0; lid < cutlist.length; lid++) {
				const k = cutlist[lid][1];
				if (cutId > 0) {
					if (j === cuts[0][1] || k === cuts[0][0]) continue;
					const p1 = [];
					const p2 = [];
					...
					let u;
					let v;
					if (Line_default.intersection(p1, p2, q1, q2, u, v) === Line_default.IntersectionState.YES_INTERSECTION) continue;
				}
...
function vtkCCSCutHoleyPolys(polys, points, polyGroups, polyEdges, normal) {
	...
	while (groupId < polyGroups.length) {
		...
		if (polyGroup.length > 1) {
			...
			let inner = 0;
			const cuts = [[0, 0], [0, 0]];
			for (let exhaustive = 0; exhaustive < 2 && !madeCut; exhaustive++) for (let j = 1; j < polyGroup.length; j++) {
			...
			if (madeCut) {
				...
				if (polyGroups[innerPolyId].length === 0) polyGroups[innerPolyId].push(innerPolyId);
			} else {
			...
			if (polyGroup.length > 1) {
				...
				let ii = 1;
				while (ii < polyGroup.length) if (vtkCCSPolyInPoly(poly1, polys[polyGroup[ii]], points, normal, pp, bounds, tol2)) ii++;
				else {
