...
import { addClipPlaneEntries, getClipPlaneShaderChecks } from "./Helpers/ClippingPlanes.js";
import { registerOverride } from "./ViewNodeFactory.js";
import { computeFnToString } from "./Helpers/ImageSampling.js";
import { ProjectionMode } from "../Core/ImageCPRMapper/Constants.js";
import { mat4 } from "gl-matrix";
//#region Sources/Rendering/WebGPU/ImageCPRMapper/index.js
var { BufferUsage } = BufferManager_default;
var cprVertTemplate = `
//VTK::Renderer::Dec
...
function vtkWebGPUImageCPRMapper(publicAPI, model) {
	...
	publicAPI.computePipelineHash = () => {
		const numClipPlanes = Math.min(model.renderable.getNumberOfClippingPlanes(), 6);
		const coincidentParameters = publicAPI.getCoincidentParameters();
		model.pipelineHash = `cprcp${numClipPlanes}co${coincidentParameters?.factor || coincidentParameters?.offset ? 1 : 0}${model.renderEncoder.getPipelineHash()}`;
	};
	publicAPI.updateGeometry = () => {
		...
		for (let lineIdx = 0; lineIdx < nLines; ++lineIdx) {
			...
			for (let localIdx = 0; localIdx < 6; ++localIdx) {
				...
				positions[posOffset] = quadIdx === 1 || quadIdx === 3 ? widthMC : 0;
		...
		const mtime = Math.max(model.renderable.getMTime(), centerline.getMTime(), centerline.getPoints().getMTime());
		...
		].forEach(([hash, nativeArray, format, names]) => {
	...
	publicAPI.updateVolumeTexture = () => {
		const newTex = model.device.getTextureManager().getTextureForImageData(model.currentImageDataInput);
		if (!model.textureViews[0] || model.textureViews[0].getTexture() !== newTex) model.textureViews[0] = newTex.createView("volumeTexture");
	};
	publicAPI.updateColorTexture = () => {
		const property = model.WebGPUImageSlice.getRenderable().getProperty();
		const scalars = model.currentImageDataInput?.getPointData()?.getScalars();
		if (!scalars) return;
		const numComp = scalars.getNumberOfComponents();
		const numRows = property.getIndependentComponents() ? numComp : 1;
		const colorTextureString = computeFnToString(property, property.getRGBTransferFunction, numRows);
		if (model.colorTextureString === colorTextureString) return;
		const colorArray = new Uint8ClampedArray(model.rowLength * numRows * 4);
		const tmpTable = new Float32Array(model.rowLength * 3);
		if (property.getRGBTransferFunction()) for (let c = 0; c < numRows; ++c) {
	...
	publicAPI.updateOpacityTexture = () => {
		...
		const numComp = scalars.getNumberOfComponents();
		const numRows = property.getIndependentComponents() ? numComp : 1;
		const pwfTextureString = computeFnToString(property, property.getPiecewiseFunction, numRows);
		if (model.pwfTextureString === pwfTextureString) return;
		const opacityArray = new Float32Array(model.rowLength * numRows);
		const tmpTable = new Float32Array(model.rowLength);
		if (property.getPiecewiseFunction()) for (let c = 0; c < numRows; ++c) {
			...
			} else opacityArray.fill(1, c * model.rowLength, (c + 1) * model.rowLength);
		}
		else opacityArray.fill(1);
		const newTex = model.device.getTextureManager().getTexture({
	...
	publicAPI.updateUBO = () => {
		...
		if (publicAPI.getMTime() <= utime && model.renderable.getMTime() <= utime && actor.getMTime() <= utime && property.getMTime() <= utime && image.getMTime() <= utime && model.WebGPURenderer.getStabilizedTime() <= utime) return;
		...
		model.UBO.setArray("GlobalCenterPoint", [...centerPoint ?? [
			0,
			0,
			0
		], 1]);
		model.UBO.setValue("UseCenterPoint", centerPoint ? 1 : 0);
		model.UBO.setArray("BackgroundColor", model.renderable.getBackgroundColor());
		model.UBO.setArray("TangentDirection", [...model.renderable.getTangentDirection(), 0]);
		model.UBO.setArray("BitangentDirection", [...model.renderable.getBitangentDirection(), 0]);
		model.UBO.setArray("UniformOrientation", model.renderable.getUniformOrientation());
		model.UBO.setValue("UseUniformOrientation", model.renderable.getUseUniformOrientation() ? 1 : 0);
		...
		for (let i = 0; i < 6; ++i) {
			clipPlane.fill(0);
			if (i < numClipPlanes) model.renderable.getClippingPlaneInDataCoords(actor.getMatrix(), i, clipPlane);
			model.UBO.setArray(`ClipPlane${i}`, clipPlane);
		}
		const coincidentParameters = publicAPI.getCoincidentParameters();
		model.UBO.setValue("CoincidentFactor", coincidentParameters?.factor ?? 0);
		model.UBO.setValue("CoincidentOffset", 16e-6 * (coincidentParameters?.offset ?? 0));
		const projectionSamples = model.renderable.getProjectionSlabNumberOfSamples();
		const projectionThickness = model.renderable.getProjectionSlabThickness();
		const projectionStepLength = projectionSamples > 1 ? projectionThickness / (projectionSamples - 1) : 0;
		...
		for (let i = 0; i < Math.min(numComp, 4); ++i) {
			...
			componentMix[i] = property.getComponentWeight?.(i) ?? 1;
			...
			const colorWindow = Math.abs(cw) > 0 ? cw : 1;
			cScale[i] = volumeScale / colorWindow;
			cShift[i] = -cl / colorWindow + .5;
			const pwfun = property.getPiecewiseFunction(target);
			if (pwfun) {
				const range = pwfun.getRange();
				const length = range[1] - range[0];
				const mid = .5 * (range[1] + range[0]);
				const pwfLength = Math.abs(length) > 0 ? length : 1;
				pwfScale[i] = volumeScale / pwfLength;
				pwfShift[i] = -mid / pwfLength + .5;
			}
		}
	...
	publicAPI.updateBuffers = () => {
		...
		publicAPI.updateOpacityTexture();
		publicAPI.updateUBO();
		const filter = model.WebGPUImageSlice.getRenderable().getProperty().getInterpolationType() === InterpolationType.NEAREST ? "nearest" : "linear";
		if (!model.cprSampler || model.cprSampler.getOptions().minFilter !== filter) {
			model.cprSampler = Sampler_default.newInstance({ label: "cprSampler" });
	...
	publicAPI.replaceShaderImageCPR = (hash, pipeline) => {
		...
		code = ShaderCache_default.substitute(code, "//VTK::ImageCPR::Impl", [
	...
	model.shaderReplacements.set("replaceShaderImageCPR", publicAPI.replaceShaderImageCPR);
	publicAPI.replaceShaderClip = (hash, pipeline) => {
		const fDesc = pipeline.getShaderDescription("fragment");
		let code = fDesc.getCode();
		if (!model.renderable.getNumberOfClippingPlanes()) {
			code = ShaderCache_default.substitute(code, "//VTK::Clip::Impl", []).result;
			fDesc.setCode(code);
			return;
		}
		...
		code = ShaderCache_default.substitute(code, "//VTK::Clip::Impl", [...clipPlaneChecks]).result;
		fDesc.setCode(code);
	};
	model.shaderReplacements.set("replaceShaderClip", publicAPI.replaceShaderClip);
	publicAPI.replaceShaderCoincident = (hash, pipeline) => {
		const fDesc = pipeline.getShaderDescription("fragment");
		let code = fDesc.getCode();
		const coincidentParameters = publicAPI.getCoincidentParameters();
		if (!coincidentParameters || coincidentParameters.factor === 0 && coincidentParameters.offset === 0) {
			code = ShaderCache_default.substitute(code, "//VTK::Coincident::Dec", []).result;
			code = ShaderCache_default.substitute(code, "//VTK::Coincident::Impl", []).result;
			fDesc.setCode(code);
			return;
		}
		...
		code = ShaderCache_default.substitute(code, "//VTK::Coincident::Dec", []).result;
		code = ShaderCache_default.substitute(code, "var output : fragmentOutput;", [
			"var output : fragmentOutput;",
	...
	model.shaderReplacements.set("replaceShaderCoincident", publicAPI.replaceShaderCoincident);
	publicAPI.replaceShaderRenderEncoder = (hash, pipeline) => {
		if (hash.includes("sel")) {
			const fDesc = pipeline.getShaderDescription("fragment");
			fDesc.addOutput("vec4<u32>", "outColor");
			let code = fDesc.getCode();
			code = ShaderCache_default.substitute(code, "//VTK::RenderEncoder::Impl", ["output.outColor = vec4<u32>(mapperUBO.PropID, 0u, 0u, 0u);"]).result;
			fDesc.setCode(code);
			return;
		}
...
function extend(publicAPI, model, initialValues = {}) {
	...
	SimpleMapper_default.extend(publicAPI, model, initialValues);
...
var newInstance = macros_default.newInstance(extend, "vtkWebGPUImageCPRMapper");
...
//#endregion
export { ImageCPRMapper_default as default, extend, newInstance };
